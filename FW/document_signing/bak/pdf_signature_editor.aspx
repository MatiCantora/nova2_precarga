...
<%
    ...
    If accion = "ver_pagina" Then
        ...
        Dim version As New Ghostscript.NET.GhostscriptVersionInfo(New System.Version("9.20"), Server.MapPath(".") & "\gs\gsdll32.dll", "", Ghostscript.NET.GhostscriptLicense.GPL)
    ...
    Dim cod_binding As String = "-1"
    
    If modo = "sign_file" Then
        
        Dim f_id As String = nvUtiles.obtenerValor("f_id", "")
        cod_binding = nvUtiles.obtenerValor("cod_binding", "")
        Dim nombre_doc As String = ""
        Dim extension As String = ""
        nro_entidad = nvApp.operador.nro_entidad
        
        Dim rs As ADODB.Recordset = nvDBUtiles.DBOpenRecordset("SELECT f_nombre, f_ext FROM verRef_files WHERE f_id=" + f_id + "")
        If Not rs.EOF Then
        ...
        Me.contents("cod_binding") = cod_binding
    End If
    ...
    <script type="text/javascript">
        ...
        function onclick_firmar() {
            ...
            switch (extension) {
                case "pdf":
                    ...
                    var saveAs = $('saveAs').value

                    nvFW.error_ajax_request('/fw/document_signing/document_sign.aspx', {
                        parameters: { modo_firma: modo_firma, strXML: strXML, f_id: f_id, cod_binding: nvFW.pageContents.cod_binding, idCert: idCert, saveAs: saveAs },
                        method: 'POST',
                        onSuccess: function (err, transport) {
                            if (err.numError == 0) {
        ...
        function getFirmaParams() {
            ...
            if (visible) {
                ...
                var x1 = left * pdfWidth
                var y1 = pdfHeight - (bottom * pdfHeight)
                var x2 = right * pdfWidth
                var y2 = pdfHeight - (top * pdfHeight)
                ...
                firma_params["PDF_params"]["PDF_appereance"]["x1"] = x1.toFixed(4)
                firma_params["PDF_params"]["PDF_appereance"]["y1"] = y1.toFixed(4)
                firma_params["PDF_params"]["PDF_appereance"]["x2"] = x2.toFixed(4)
                firma_params["PDF_params"]["PDF_appereance"]["y2"] = y2.toFixed(4)
        ...
        function loadMenu() {
            ...
            Menus["vMenu"].CargarMenuItemXML("<MenuItem id='0' style='WIDTH: 100%'><Lib TipoLib='offLine'>DocMNG</Lib><icono></icono><Desc>Firmas: " + nombre_doc + "</Desc></MenuItem>")
        ...
        function loadSignFileMenu() {
            ...
            Menus["vMenu"].CargarMenuItemXML("<MenuItem id='0' style='WIDTH: 100%'><Lib TipoLib='offLine'>DocMNG</Lib><icono></icono><Desc>Firmas: " + nombre_doc + "</Desc></MenuItem>")
            Menus["vMenu"].CargarMenuItemXML("<MenuItem id='1'><Lib TipoLib='offLine'>DocMNG</Lib><icono>abm</icono><Desc>Configuracion Firma</Desc><Acciones><Ejecutar Tipo='script'><Codigo>verConfiguracion()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenu"].CargarMenuItemXML("<MenuItem id='2' style='WIDTH: 100%'><Lib TipoLib='offLine'>DocMNG</Lib><icono>guardar</icono><Desc>Firmar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>onclick_firmar()</Codigo></Ejecutar></Acciones></MenuItem>")
    ...
    <div id="divOpcionesFirma" style="width: 100%; display: none">
        <table id="botones_de_desplazamiento" style="width:40%;margin: 0 auto">
            <tr>
                ...
                <td style="width:1%"><input type="text" id="txtCurrentPage" style="width:100%"/></td>
                <td style="width:1%">
                    <p id="divCantPaginas">/15</p>
                </td>
                <td style="width:2%">
