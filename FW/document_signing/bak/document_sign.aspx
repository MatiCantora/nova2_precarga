...
<%
    ...
    If accion = "" Then
        ...
        rs = nvDBUtiles.DBOpenRecordset("select * from ref_files where f_id=" + f_id)
        If Not rs.EOF Then
        ...
        If modo_firma = "deferred_sign" Then
            ...
            Dim estimatedSize As Integer = nvUtiles.obtenerValor("estimatedSize", 25000) '8192
            ...
            ' (Ver prueba de arriba: cada vez que se genera un blanksigned con SignExternalContainer, appareance.SignDate es distinto, asi como tambien los datos de crl, ocsp, y tsa pueden cambiar)
            ' 
            ...
            Dim hash As Byte() = defSign.genHash(estimatedSize)
        
        
 
            ' ensamblar pdf con firma
            'Stop
            'Dim certt As New X509Certificate2()
            'certt.Import("D:\certificados\cert_martinez_gonzalo.pfx", "7777777", X509KeyStorageFlags.UserKeySet Or X509KeyStorageFlags.Exportable)
            'Dim signedHash As Byte() = defSign.signHash(hash, certt)
            'Dim signedPDF As Byte() = defSign.composeSignedPDF(Convert.ToBase64String(signedHash))
            'If Not signedPDF Is Nothing Then
            '    System.IO.File.WriteAllBytes("D:\salidaDeferredLocal.pdf", signedPDF)
            '    err.response()
            'End If
            ...
            rpending.element("save_as") = saveAs
