...
function tTable(nombreTabla, filtroXML, cabeceras, campos, callbackValidacion, callbackEliminar, callbackModificar, backgroundColorCallback, callbackOnComplete) {
    ...
    */
    ...
    this.validacion = callbackValidacion ? callbackValidacion : function (fila) {
		...
		// si esta eliminada, no hay que validar
		if (fila.eliminado == true) return true;
		
        for (var index_colum = 0; index_colum < this.cantColumnas; index_colum++) {
			
			// Si hay campos definidos como no nulos y se encuentran vacios
			...
			// Si hay campos clave/unicos y se encuentran repetidos
			if (this.campos[index_colum].unico){
				for (var index_fila = fila.indice - 1; index_fila > 0; index_fila--) {
					filaAux = this.getFila(index_fila);
					if (filaAux.eliminado == true) continue;
					if (filaAux[this.campos[index_colum].nombreCampo].toLowerCase() == fila[this.campos[index_colum].nombreCampo].toLowerCase())
						return false;
				}
            }
        }
    ...
    */
    ...
    function agregar_fila(valores_campos) {
        ...
        if (this.eliminable) {
            celdas[index_cell] = row.insertCell(index_cell);
            celdas[index_cell].id = 'campos_tb_' + this.nombreTabla + '_fila_' + this.cantFilas + '_columna_' + (index_cell)
            celdas[index_cell].innerHTML = '<center><img border="0"  onclick="' + this.nombreTabla + '.eliminar_fila(\'' + this.cantFilas + '\')" src="/FW/image/icons/eliminar.png" title="Eliminar fila" style="cursor:pointer" /></center>';
        ...
        if (this.editable) {
            if (valores_campos[this.cantColumnas] + 1) {
                ...
                celdas[index_cell].innerHTML = '<center><img border="0" onclick="' + this.nombreTabla + '.modificar_fila(\'' + this.cantFilas + '\')"src="/FW/image/icons/editar.png" title="Editar fila" style="cursor:pointer"/></center>';
                //celdas[index_cell].style.width = "12.5%";
                celdas[index_cell].style.align = "center";
            } else {
                celdas[index_cell] = row.insertCell(index_cell);
    ...
    function agregar_espacios_en_blanco_dir() {
        ...
        for (cell_index = 0, limit = this.campos.length; cell_index < limit; cell_index++) {
    ...
    function modificar_fila(row_index) {
        ...
        for (; index_cell < this.cantColumnas; index_cell++) {
            ...
            if (tieneId) {
                ...
                valor = celda.innerText.trim();
                this.valoresAnteriores[nombreCampoDef] = valor;
            }

            if (!esUnRadiobutton && !esUnCheckbox && !esUnico && esEditable) {
                celda.innerHTML = "";
        ...
        if (esUnRadiobutton || esUnCheckBox) {
            ...
            if (esUnCampoDef) {
                ...
                if (this.cantColumnas > index_cell){
                    fila_objeto[nombreCampo] = this.data[row_index][nombreCampo] ? this.data[row_index][nombreCampo] : ''
                }
            }
        }
                ...
                for (var i = 1; i < table_html.rows.length - 1; i++) { // filas
                    ...
                    for (var j = 0; j < this.campos.length; j++) { // columnas
                        if ((!this.data[i].tabla_control.existeEnBd || this.data[i].tabla_control.modificado) && !this.data[i].tabla_control.eliminado) {
                            if (this.campos[j].nombreCampo != nombreColumna)
                                continue;
    ...
    this.removeCamposDef = function removeCamposDef() {
        ...
        if (tabla)
            for (var i = 0, limit_i = tabla.rows.length - 1; i < limit_i; i++) { // filas
                for (var j = 0, limit_j = this.campos.length; j < limit_j; j++) { // columnas
    ...
    this.actualizarData = function () {
        for (var row_index = 1, limit = this.data.length; row_index < limit; row_index++) {
    ...
    this.table_load_html_ascy = function (tabla) {
        // Reinicializacion de los parametros básicos de la tabla
        ...
        rs.onComplete = function () {
            ...
            while (!rs.eof()) {
                ...
                for (var index_campos = 0, limit = tabla.campos.length; index_campos < limit; index_campos++) {
            ...
            nvFW.bloqueo_desactivar(null, 'cargando-' + tabla.nombreTabla);
    ...
    this.mostrar_tabla = function (tabla)
    {
        ...
        var srtHtml =
            ...
            '<table class="tb1" style="' + tabla.tHeader.style + '"  id="header_tbl_' + tabla.nombreTabla + '" > ' +
            '<tr class="tbLabel">';

        var existeColumnaDeOrden = false;

        for (var col_index = 0; col_index < tabla.campos.length; col_index++) {
        ...
        for (var col_index = 0; col_index < tabla.campos.length; col_index++) {
            celda = tabla.campos[col_index];
            // Cargamos valor por defecto campos de tipo texto.
            celda.nro_campo_tipo = celda.nro_campo_tipo ? celda.nro_campo_tipo : '104';
            var ordenHTML = '';

            // Valores de control 
            var tieneGetHtml     = celda.get_html                  ? true : false;
            var tieneCheckbox    = celda.checkBox                  ? true : false;
            var tieneRadioButton = celda.radioButton               ? true : false;
            var esOrdenable      = (celda.ordenable === undefined) ? true : false;

            // Para los getHtml(se puede especificar ordenable:true para permitirlo), Checkbox y radiobutton no se permite que sean ordenables.
            if (!existeColumnaDeOrden && celda.ordenable === true || !existeColumnaDeOrden && (!tieneGetHtml && !tieneCheckbox && !tieneRadioButton && esOrdenable)) {
                ordenHTML = '<img src="/FW/image/icons/up_a.png" onclick="return ' + tabla.nombreTabla + '.ordenar( \'' + celda.nombreCampo + '\' ,\'ASC\',\'' + celda.nro_campo_tipo + '\')" />' +
                '<img src="/FW/image/icons/down_a.png" onclick="return ' + tabla.nombreTabla + '.ordenar( \'' + celda.nombreCampo + '\',\'DESC\',\'' + celda.nro_campo_tipo + '\' )" />'
            }
            
            srtHtml += '<td style="text-align: center; width: ' + celda.width + ';">' +
                ordenHTML +
        ...
        srtHtml += tabla.eliminable ? '<td style="text-align: center; width: 7.5%;">Eliminar</td>' : "";
        srtHtml += tabla.editable ? '<td style="text-align: center; width: 7.5%;">Editar</td>' : "";
        srtHtml += '</tr></table></div>';


        srtHtml += '<div id="div_cuerpo_' + tabla.nombreTabla + '" style="overflow: auto; width: 100%; heigth: 100%;">' +
            '<table id="campos_tb_' + tabla.nombreTabla + '" class="tb1" style="' + this.tBody.style + '" ><tr style="visibility: collapse; line-height: 0;">';
            //'<table id="campos_tb_' + tabla.nombreTabla + '" class="tb1" style="' + this.tBody.style + '" ><tr style=" visibility: hidden ; height:0px">';

        for (var row_index = 0; row_index < tabla.cabeceras.length; row_index++) {
            srtHtml += '<td></td>';
        }
        ...
        srtHtml += '</tr></table></div>' +
            ...
            '.agregar_espacios_en_blanco_dir()" src="/FW/image/icons/agregar.png" title="agregar" style="cursor: pointer;"/></center></div></div>'

        var contenedor       = $(tabla.nombreTabla);
        contenedor.innerHTML = "";
        contenedor.insert({ top: srtHtml });
        ...
        */
        ...
        // Generamos el html para todas las filas segun los valores de campo enviados.
        for (var indice_fila = 1 ; indice_fila < tabla.data.length; indice_fila++) {
            ...
            for (; index_cell < tabla.campos.length; index_cell++) {
                ...
                celda         = tabla.campos[index_cell];
                // Revisamos si es un radioButton para asinar la funcion de generacion.
                if (celda.radioButton) {
                    celda.get_campo = tabla.funcionRadioButtonTable;
                    celda.get_html  = tabla.funcionRadioButtonTable;
                }

                if (celda.ordenButton) {
                    celda.editable    = false;
                    celda.get_campo   = "";
                    var indiceColumna = index_cell;
                    celda.get_html    = function (indice_fila) {
                ...
                // Revisamos si es un radiobuton
                if (celda.checkBox) {
                    celda.get_campo = tabla.funcionCheckBox;
                    celda.get_html  = tabla.funcionCheckBox;
                }
                else {
                    celda.get_campo = celda.get_campo ?
                        celda.get_campo :
                        default_get_campo;
                }

                if (celda.id) {
                    campo.id = tabla.data[indice_fila][celda.id];
                }

                celda.convertirValorBD = (!celda.convertirValorBD) ? 
                    function (valor) {
                        return valor
                    } : 
                    celda.convertirValorBD;

                campo.valor                = celda.convertirValorBD(tabla.data[indice_fila][celda.nombreCampo]);
                campo.nombre               = celda.nombreCampo;
    ...
    this.swap_fila = function (fila, columna, ord)
    {
        ...
        var nombreCampo                                     = this.campos[columna].nombreCampo

        if (!this.data[fila][nombreCampo])
            this.data[fila][nombreCampo] = parseInt(this.data[fila + ord][nombreCampo]) + 1 + ""

        var swapOrden                      = this.data[fila][nombreCampo]
        this.data[fila][nombreCampo]       = this.data[fila + ord][nombreCampo]
        this.data[fila + ord][nombreCampo] = swapOrden
    ...
    /**
     * Description
     * Carga el valor enviado en una celda verificando su tipo.
     * @method modCelda
     * @param {} celda
     * @param {} fila_objeto
     * @param {} row_index
    ...
    */
    ...
    function funcionCheckBox(check, nombreTabla) {
        ...
        if (check.valor == true || check.valor == "True" || check.valor == 1 || check.valor == "true")
        ...
        return '<input type="checkbox" onclick="' + nombreTabla + '.click_check(' + check.fila + ',' + check.columna + ')" name="vehicle" value="' + stringCheck;
    }
     ...
     */
    function funcionRadioButton(c, nombreTabla) {
        ...
        return '<input type="radio"  name="radiobutton_' + nombreTabla + ' value="' + stringRadio + '<br>';
    }
