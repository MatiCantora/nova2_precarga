  ...
  function tnvBrowserLog()
   ...
   //Controlar la salida de la aplicación por fuera de código
...
//                                 }) 
 function tnvFrameWork()
      ...
      this.bloqueo_msg = nvFW_bloqueo_msg;
      ...
      this.CSSExistClass = nvFW_CSSExistClass;
      ...
      this.addDebug = nvFW_addDebug;
      this.interOPSendMessage = nvFW_interOPSendMessage;
      this.interOPaddHandlerReceiveMessage = nvFW_interOPaddHandlerReceiveMessage;
      ...
      //Determina si se le emitirá un alerta al usuario cuando quiera salir de la aplicación por fuera del código. Por ejemplo recargando la página o haciendo hacia atras.
      this.alertUnload = false
 ...
 function nvFW_get_hash()
   {
    ...
    function nvFW_createFORM(tipo, formTarget, enctype, method)
	  {
	  if (!enctype)
	    enctype = ''
      if (!tipo)
      ...
      if (tipo == 'EXP' || tipo == 'INF' || tipo == 'FILE' || tipo == 'TRAN' || tipo == 'DTSX')
        ...
        strHTML = "<form name='" + id + "' id='" + id + "' style='display: none' method='" + method + "' action='" + action + "' target='" + formTarget + "' " + strEnctype + " >"
    ...
    function nvFW_reporte_ejecutar(exp_tipo, parametros)
      ...
      var requestMethod = !parametros.requestMethod ? 'POST' : parametros.requestMethod //requestMethod POST o GET
      ...
      var bloq_id = !parametros.bloq_id ? (Math.random() * 1000).floor() : parametros.bloq_id //Privado
      var bloq_msg = !parametros.bloq_msg ? "" : parametros.bloq_msg 
	  ...
	  if (metodo.toLowerCase() == 'httprequest')
        ...
        if (bloq_contenedor != null)
            this.bloqueo_activar(bloq_contenedor, bloq_id, bloq_msg)
        oXMLHttp.open(requestMethod, URL, async);
        ...
        if (exp_tipo == "EXP") 
          {
          FORM = nvFW_createFORM('EXP', formTarget, undefined, requestMethod)
        ...
        if (exp_tipo == "INF")
          {
          FORM = nvFW_createFORM('INF', formTarget, undefined, requestMethod)
        ...
        if (es_winPrototype) 
          ...
          if (winPrototype.bloquear)
            ...
            winPrototype.onShow = function(e)
                                        {
                                        miOBJETO.bloqueo_activar($(win.content.id), bloq_id, bloq_msg)
                                        }
            }
          var win = this.createWindow(winPrototype);
          if (winPrototype.center)
            win.showCenter(winPrototype.modal, winPrototype.centerTop, winPrototype.centerLeft)
          else
...
/*
	...
	function nvFW_transferenciaEjecutar(parametros)
      ...
      var include_file = !parametros.include_file ? false : parametros.include_file
      var include_file_format = !parametros.include_file_format ? "xml" : parametros.include_file_format
	  ...
	  if (metodo.toLowerCase() == 'httprequest')
        ...
        strParam += "&include_file=" + encodeURIComponent(include_file)
        strParam += "&include_file_format=" + encodeURIComponent(include_file_format)
        strParam += "&winModalID=" 

        strParam += "&accion=getbpmfile" 
        ...
        var URL = this.path.getXML //this.path.transferenciaEjutar
        if (bloq_contenedor != null)
            this.bloqueo_activar(bloq_contenedor, bloq_id)
        oXMLHttp.open('POST', URL + strParam, async);
        oXMLHttp.send(null)
        return false
        }
      else
    ...
    function nvFW_createWindow(parametros)
      ...
      var setHeightToContent  = parametros.setHeightToContent == undefined ? false : parametros.setHeightToContent
      if (setHeightToContent)
        {
        var handler_onShow = parametros.onShow
        parametros.onShow = function (win)
                              {
                              //Ajustar ventana al contenido vertical
                              var lastElement = win.content.lastElementChild
                              while (lastElement != null && lastElement.offsetHeight == 0)
                                {
                                lastElement = win.content.lastElementChild.previousElementSibling
                                }
                              var altoInner = lastElement.offsetHeight + lastElement.offsetTop
                              var altoContent = win.content.clientHeight
                              var dif = altoContent - altoInner
                              win.setSize(win.width, (win.content.getHeight() - dif) + 1, true)
                              win._recenter()
                              //Continuar con el manejador anterior
                              if (typeof(handler_onShow) == 'function')
                                handler_onShow(win)
                              }
      }
      
      //Ajustar el ancho de la ventana para que entre siempre en el contenido
      var setWidthMaxWindow  = parametros.setWidthMaxWindow == undefined ? false : parametros.setWidthMaxWindow
      if (setWidthMaxWindow)
        {
        var handler_onShow1 = parametros.onShow
        parametros.onShow = function (win)
                              {
                              var maxWidth = $$("BODY")[0].getWidth() - 20
                              if (win.width > maxWidth)
                                {
                                win.setSize(maxWidth, win.height, true)
                                win._recenter()
                                }
                              if (typeof(handler_onShow1) == 'function')
                                handler_onShow1(win)
                              }
        }
      parametros.padestroyOnClose = parametros.padestroyOnClose == undefined ? true : parametros.padestroyOnClose
      //Centrar la ventana en funcion de otro elemento
      parametros.centerFromElement  = parametros.centerFromElement == undefined ? null : parametros.centerFromElement
      parametros.centerHFromElement  = parametros.centerHFromElement == undefined ? null : parametros.centerHFromElement
      parametros.centerVFromElement  = parametros.centerVFromElement == undefined ? null : parametros.centerVFromElement
      //Definir el ancho de una ventana en porcentaje de otro elemento
      parametros.parentWidthPercent = parametros.parentWidthPercent == undefined ? undefined : parametros.parentWidthPercent
      parametros.parentWidthElement = parametros.parentWidthElement == undefined ? null : parametros.parentWidthElement
      //Definir el alto de una ventana en porcentaje de otro elemento
      parametros.parentHeightPercent = parametros.parentHeightPercent == undefined ? undefined : parametros.parentHeightPercent
      parametros.parentHeightElement = parametros.parentHeightElement == undefined ? null : parametros.parentHeightElement
    ...
    function nvFW_bloqueo_activar(contenedor, id, msg) 
      {
      if (!msg) msg = ""
      var objC = $(contenedor) //objeto contenedor
      ...
      //Crear div de transparencia
      var oDivTrans = doc.createElement("DIV")
      oDivTrans.setAttribute("id", "divBloq_trans_" + id)
      $(oDivTrans).addClassName("overlay_bloqueo")
      oDivTrans.style.zIndex = oDiv.style.zIndex + 1
      oDiv.appendChild(oDivTrans)
      oDivTrans.setStyle({top:"0px", left:"0px"})
      //oDivTrans.clonePosition(oDiv)
      oDiv._DivTrans = oDivTrans
      
      //Crear div de mensaje
      var oDivMsg = doc.createElement("DIV")
      oDivMsg.setAttribute("id", "divBloq_msg_" + id)
      $(oDivMsg).addClassName("overlay_bloqueo_msg")
      oDivMsg.style.zIndex = oDiv.style.zIndex + 2
      oDivMsg.innerHTML = msg
      oDiv.appendChild(oDivMsg)
      oDiv._DivMsg = oDivMsg
     ...
     return oDiv
      ...
      function nvFW_bloqueo_msg(id, msg) {
          var oDiv = document.getElementById("divBloq_" + id)
          if (oDiv == null) oDiv = $("divBloq_" + id) 
          if (oDiv != null) 
              {
              oDiv._DivMsg.innerHTML = msg
              }
      }
...
function nvFW_window_click(e)
  ...
  var exeptions = ["input", "a", "img"]
  if (exeptions.indexOf(el.tagName.toLowerCase()) < 0)
    e.preventDefault(); //Previene el comportamiento de "Touch to Search de chrome"
  var func = null
  switch (e.button)
    {
    case 0:
...
function nvFW_chargeJSifNotExist(strObject, jsURL, async, onComplete, forceCarge)
   ...
   if (!objetoEncontrado || forceCarge)
     ...
     if (async)
       ...
       //Carga sincrnica
       var code = ''
       if (!window.top._xhr_cache)
         if (window.localStorage != undefined)
           if (window.localStorage._xhr_cache != undefined)
             window.top._xhr_cache = JSON.parse(window.localStorage._xhr_cache)
           else
             window.top._xhr_cache = {}

       if (!window.top._xhr_cache[jsURL.toLowerCase()])
         {
         var xmlHttp 
         try
          {
         ...
         try
           ...
           window.top._xhr_cache[jsURL.toLowerCase()] = code
           if (window.localStorage != undefined)
             window.localStorage._xhr_cache = JSON.stringify(window.top._xhr_cache)
           }
         catch(e)
           {
       ...
       code = window.top._xhr_cache[jsURL.toLowerCase()]
...
function nvFW_addDebug(msg, reverse)
    {
    if (reverse == undefined) reverse = false
    if (!window.debug)
      {
      nvFW_chargeCSS("/FW/css/debug.css")
      nvFW.chargeJSifNotExist("", '/FW/script/debug.js')
      //nvFW.chargeJSifNotExist("", '/FW/script/extended_debug.js')
      showDebug()
      }
      var mom = new Date()
      window.debug(FechaToSTR(mom) + " " + HoraToSTR(mom) + " : " + msg, reverse)
...
function nvFW_CSSExistClass(selectorName)
  {
    //document.styleSheets[0].rules[0].selectorText
    for (var i=0; i < document.styleSheets.length; i++)
        for (var j=0; j < document.styleSheets[i].rules.length; j++)
            if (typeof document.styleSheets[i].rules[j].selectorText != 'undefined' && document.styleSheets[i].rules[j].selectorText.indexOf(selectorName) > -1) 
                return true
 ...
 function nvFW_chargeCSS(cssURL, IfnotExistSelector) 
   {
   var cargar =  IfnotExistSelector == undefined ? true : !this.CSSExistClass(IfnotExistSelector)
...
function nvFW_selection_clear()
  ...
  function nvFW_interOPSendMessage(action, message, debug_console) 
    {

    if (!debug_console) debug_console = false

    if (debug_console) nvFW.addDebug(action + ': ' + message)

    if (this.nvInterOP) 
      {
      if (typeof this.nvInterOP.sendMessage != 'undefined')
        {
        this.nvInterOP.sendMessage(action, message)
        }
      }
    ...
    try
      ...
      var sendToNative = { action: action, message: message };
      var stringifiedMessageObj = JSON.stringify(sendToNative);
      window.webkit.messageHandlers.cordova_iab.postMessage(stringifiedMessageObj);
      
      }
    catch (e)
      {
      if (debug_console) nvFW.addDebug('Error: ' + e);
      }
    }

  function nvFW_interOPaddHandlerReceiveMessage(handler)
    {
    if (!this.nvInterOP)
      this.nvInterOP = {}
    try
      {
      this.nvInterOP.checkIfHandlerExists("true")
      }
    catch(e)
      {

      }
      this.nvInterOP.receiveMessageHandler = handler;
    }
