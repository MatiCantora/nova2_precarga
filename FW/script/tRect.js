...
function tnvRect(options)
{
    ...
    this.minHeight = !options.minHeight ? 60 : options.minHeight;
...
function tnvRect_select(sel, e)
{
    ...
    if (this.allowSelect)
    {
        ...
        else
        {
            ...
            //Desactivar teclas
            if (Prototype.Browser.IE || Prototype.Browser.WebKit) {
                Event.stopObserving($(document), 'keydown', this._scut_key)
            } else {
                Event.stopObserving($(document), 'keypress', this._scut_key)
            }
        }
        this.selected = sel
...
//        }
    ...
    this.div.clonePosition(parent, {setWidth: false, setHeight: false, offsetLeft: x, offsetTop: y});
...
{
//         var el = Event.element(e)
//         rec = nvCtrls.getRectById(el.id)
//         oCB2.div.innerHTML += rec.type + "<br/>"

    if (Event.isLeftClick(e))
    {
        var x = Event.pointerX(e) - nvCtrls._drag_x
        var y = Event.pointerY(e) - nvCtrls._drag_y
