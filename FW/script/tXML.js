    ...
    function selectNodes(sXPath, xNode) 
      ...
      try
        {
        nsResolver = document.createNSResolver(contextNode)
        oResult = oEvaluator.evaluate(sXPath, xNode, nsResolver, XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
        }
      catch(ex3)
        {
        oResult = null
    ...
    function tXML_load(URL, strParametres, onComplete, headers)
      ...
      try
                                          ...
                                          if (oXMLHttp.readyState == 4 && miOBJETO.async)
                                            {
                                             if (oXMLHttp.status == 200) 
                                               ...
                                               if (oXMLHttp.status == 0) //status == 0 cuando no hay conexión
                                                 {
                                                 miOBJETO.parseError.numError = 1000
                                                 miOBJETO.parseError.description = "En estos momentos no se puede acceder al recurso solicitado.<br>Revise el estado de su conexión e intente nuevamente."
                                                 }
                                               else
                                                 {
        ...
        for (el in headers)
          ...
          if (el.toLowerCase() == "content-type")    
            setHeader = true
          }
        if (this.method.toUpperCase() == "POST")
          {
          if (!setHeader) oXMLHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded;charset=ISO-8859-1")
