    ...
    var v = function (d, l) {
        ...
        return {
            ...
            onShow: function () {
                ...
                // si el elemento ya esta creado se muestra, sino crearlo
                if ($("selImageType")) {
                    //debugger
                    $("selImageType").show()

                    if ($("selImageType").input_url_image.value == "") {
                        // limpiar el campos_defs "f_id"
                        campos_defs.clear("f_id")
                    }

                    // setear foco en el campo_def luego de 0.5 segundos, de otra manera el foco se pierde hacia otro control
                    setTimeout(function () {
                        campos_defs.items["f_id"].input_text.focus()
                    }, 500)
                }
            },
            onOk: function () {
            ...
            onLoad: function () {
                ...
                // crear el elemento global para opciones custom del NOVA
                if (l == "image") {
                    // Agregar nuevo selector
                    // Opcion 1: Interno (por defecto)
                    // Opcion 2: <otro> (opcion original del CKeditor)
                    var strHTML = "<select class='cke_dialog_ui_input_select' style='width:15% !important;float: left;margin-right:0.5em' id='selImageType' name='selImageType'>"
                    strHTML += "<option value='img_interna' default='true'>Interno</option>"
                    strHTML += "<option value='img_otro'>&lt;otro&gt;</option>"
                    strHTML += "</select>"

                    // div para el input URL image desde un campo_def
                    strHTML += "<div id='campoDefImagen' style='display:inline-block;width:80%'></div>"
                    // div con el input checkbox para ultima version de imagen
                    strHTML += "<div id='divCheck' style='display:block;width: 100%'>"
                    strHTML += "<input type='checkbox' id='chkLastVersion' name='chkLastVersion' checked='true' style='margin-top:0.8em;margin-bottom:0.8em'/><label for='chkLastVersion' style='margin-left: 0.5em'>Vínculo a la última version</label>"
                    strHTML += "</div>"

                    // capturar del elemento INPUT para la URL de imagen
                    var input_url_image = this.parts.dialog.$.select("input")[0]
                    // insertar al inicio del input el strHTML
                    input_url_image.insertAdjacentHTML("beforebegin", strHTML)

                    // observar el evento "change" para el select "selImageType"
                    $("selImageType").observe("change", function () {
                        // Imagen interna
                        if (this.value == "img_interna") {
                            // ocultar el input URL original y limpiar contenido
                            $("selImageType").input_url_image.disabled = true
                            $("selImageType").input_url_image.setValue("")
                            // mostrar el input con campo_def
                            $("selImageType").div_img_interna.show()
                            $("selImageType").div_img_interna.setStyle({ "display": "inline-block" })
                            // mostrar el div con el input check
                            $("divCheck").show()
                            // setear la propiedad CSS display correcta para el input URL
                            $("selImageType").input_url_image.setStyle({ "display": "block", "width": $("selImageType").width_url + "px" })
                        }
                        // Imagen externa
                        else {
                            // ocultar el input con campo_def y limpiarlo
                            $("selImageType").div_img_interna.hide()
                            campos_defs.clear("f_id")
                            // ocultar el div que contiene al input check
                            $("divCheck").hide()
                            // mostrar el input URL original
                            $("selImageType").input_url_image.disabled = false
                            $("selImageType").input_url_image.setStyle({ "display": "inline-block", "width": "80%" })
                        }
                    })

                    // cargar un campo_def tipo 90
                    // Aplicar un filtro solo para imágenes al file dialog
                    var file_dialog = {}
                    var filters = {}
                    filters[0] = {}
                    filters[0].titulo = "Archivos de imagen"
                    filters[0].filter = '*.jpg|*.jpeg|*.png|*.gif|*.bmp'
                    filters[0].max_size = 1024 * 1024
                    filters[0].inicio = true
                    file_dialog.filters = filters

                    campos_defs.add("f_id", { target: "campoDefImagen", nro_campo_tipo: 90, enDB: false, file_dialog: file_dialog })
                    ...
                    // evento onChange para el campo_def
                    campos_defs.items["f_id"].onchange = function () {
                        // ejecutar solo si existe un valor
                        if (campos_defs.get_value("f_id") != "") {
                            var urlString = "/fw/files/file_get.aspx?"
                            // verificar si el usuario require la ultima version
                            if ($("selImageType").chk_lastVersion.checked == true) {
                                urlString += "ref_files_path=" + campos_defs.get_desc("f_id")
                            }
                            else {
                                urlString += "f_id=" + campos_defs.get_value("f_id")
                            }

                            // setear nuevo valor a la URL de imagen
                            $("selImageType").input_url_image.value = urlString
                            $("selImageType").miTXTUrl.onChange()
                        }
                        // si el campo_def esta vacio, limpiar la URL
                        else {
                            $("selImageType").input_url_image.setValue("")
                            $("selImageType").miTXTUrl.onChange()
                            // limpiar los inputs de width y height luego de quitar la imagen
                            $("selImageType").size_elements[0].value = ""
                            $("selImageType").size_elements[1].value = ""
                        }
                    ...
                    // capturar referencias hacia el elemento padre (aqui el select #selImageType)
                    // input con la URL de la imagen
                    $("selImageType").input_url_image = input_url_image
                    // input con el campo_def (todo el div)
                    $("selImageType").div_img_interna = $("campoDefImagen")

                    // setear estios al input URL original para mejorar la visualizacion
                    $("selImageType").input_url_image.setStyle({ "display": "block" })
                    $("selImageType").input_url_image.setStyle({ "width": tablaWidth + "px" })

                    // obtener ancho de tabla padre y asignarlo al input URL, solo cuando el campo_def esta presente
                    var tablaPadre = document.getElementsByClassName("cke_dialog_ui_hbox cke_dialo_gimage_url")[0]
                    var tablaWidth = tablaPadre.clientWidth
                    // quitar el padding-right al TD contenedor
                    tablaPadre.select("td.cke_dialog_ui_hbox_first")[0].setStyle({ "padding-right": 0 })
                    // guardar la tabla padre para busquedas de elementos
                    $("selImageType").tablaPadre = tablaPadre
                    // ajustar el ancho del contenedor
                    $("selImageType").tablaPadre.select("div.cke_dialog_ui_labeled_content")[0].setStyle({ "width": tablaWidth + "px" })

                    // guardar referencias del ancho para el onchange
                    $("selImageType").width_url = tablaWidth

                    // referencia al checkbox de ultima version
                    $("selImageType").chk_lastVersion = $("chkLastVersion")
                    // checkbox activado por defecto
                    $("selImageType").chk_lastVersion.checked = true

                    // Matchear el evento onchange del campo_def con el del checkbox
                    $("selImageType").chk_lastVersion.onchange = campos_defs.items["f_id"].onchange

                    // obtener los input de WIDTH y HEIGHT (solo para limpiarlos al borrar una imagen)
                    $("selImageType").size_elements = document.getElementsByClassName("cke_dialog_ui_hbox")[5].select(".cke_dialog_ui_hbox_first input")

                    // agregar reglas CSS para ajustar altura final del contenedor
                    document.styleSheets[0].insertRule(".cke_dialog_ui_vbox_child {padding-bottom: 0 !important;}", 0)
                    document.styleSheets[0].insertRule(".cke_dialog_contents_body {padding-bottom: 0 !important; padding-top: 0 !important;}", 0)
                }
            },
            onHide: function () {
            ...
            contents: [{
                ...
                elements: [{
                    ...
                    children: [{
                        ...
                        widths: ["280px", "110px"],
                        align: "right",
                        className: "cke_dialo_gimage_url",
                        children: [{
                            id: "txtUrl",
                            type: "text",
                            ...
                            required: !0,
                            onLoad: function () {
                                // capturar el elemento THIS de éste contexto
                                $("selImageType").miTXTUrl = this
                            },
                            onChange: function () {
