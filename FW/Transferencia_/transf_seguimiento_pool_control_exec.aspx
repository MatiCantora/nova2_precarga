...
<%
    ...
    Me.contents("filtroVerTransf_log_det") = nvXMLSQL.encXMLSQL("<criterio><select vista='verTransf_log_det'><campos>id_transf_log_subproc,fe_fin_det,id_transf_log,id_transferencia, nombre, estado, estado_det, fe_inicio, fe_fin, id_transf_log_det, id_transf_det, transferencia,transf_tipo,numError, mensaje, comentario, debug_desc, debug_src,link, script, nombre_operador,operador_det,login_det,nombre_operador_det</campos><filtro></filtro><orden>fe_fin_det</orden></select></criterio>")
         ...
         try {
             ...
             rs.onComplete = function(rs) {
                 try {
                     ...
                     strHTML += '<tr class="tbLabel"><td style="width:15%" title=" Nro. Log Transf: ' + id_transf_log + '">&nbsp;Nro.</td><td style="width:30%" >Tarea</td><td style="width:15%">Estado</td><td>LINK</td><td style="width:20%">LOG</td><td style="width:6%">Param</td></tr>'
                     ...
                     while (!rs.eof()) {
                         ...
                         if ((rs.position == 0 || tranf_log_det['transf_tipo'] != 'DTS' && tranf_log_det['transf_tipo'] != 'INF' && tranf_log_det['transf_tipo'] != 'EXP') && tranf_log_det['estado_det'] == 'terminado' && tranf_log_det['id_transf_log_det'] > 0) {
                             img_parametro = "<img title='Parámetros' onclick='mostrar_transf_parametros(" + tranf_log_det['id_transf_log_det'] + ", \"" + "Parámetros Entrada: " + tranf_log_det['transferencia'] + "\", " + ((nvFW.pageContents.permiso_ver_parametros) > 0) + ",\"" + nvFW.pageContents.filtroVertransf_log_param + "\")' style='cursor:pointer;cursor:hand' src='/FW/image/transferencia/variable.png'/>"
                         ...
                         strHTML += "<tr><td style='width:15%;text-align:center' title='Nro. Log Transf. Det: " + tranf_log_det['id_transf_log_det'] + "'>" + tranf_log_det['id_transf_det'] + "</td><td style='width:30%'>" + tranf_log_det['transferencia'] + "</td><td style='width:15%;" + class_name + "'>" + txtEstado + " - " + tranf_log_det['login_det'] + "</td><td>" + link + "</td><td style='width:20%'>" + log + "</td><td style='width:6%;text-align:center'>" + img_parametro + "</td></tr>"
                         rs.movenext()
                     }
                                                                 ...
                                                                 onOk: function(w){
                                                                                   ...
                                                                                   try {
                                                                                      ...
                                                                                      err.error_from_xml(oXML)
                                                                                       ...
                                                                                       if (err.numError == 0) {
                                                                                            setTimeout("win.options.userData.onclose();win.close()",1000)
                                                                                       }
                                                                                       else
                                                                                          alert(err.mensaje)
