            ...
            if (extension == 'jscript' ||
                ...
                if (editor) {
                    ...
                    editor = CodeMirror.fromTextArea($(obj), {
                        ...
                        //            data.replaceSelection('nvEvaluator.Pizarra.value("Pre-calificación ""Tipo de empleo"" v1.0",3, "Empleado Público Nacional", 1001)' + selection);
                        //            if (!selection) {
...
function window_onresize() {
    try {
      ...
      //  $('cb_type').setStyle({ height: (alto - altoRight) + 'px' })
      //  $('cb_campos').setStyle({ height: (alto - altoRight) + 'px' })
        ...
        editor.setSize(($('divLeft').getWidth()) + 'px', (alto - altoLeft) + 'px')
        editor.refresh()
...
//***** sacar
//if (!objEntrante.parametros.length)
//{
//    objEntrante.parametros = []
//    objScriptEditar.parametros = []
//}
 
//if (!objEntrante.param_add.length)
//{
//    objEntrante.param_add = []
...
$('tbTarget').show()
  ...
  switch (objScriptEditar.protocolo.toUpperCase())
  {
     ...
     case 'SCRIPT':
          ...
          //    this.param_add[parametros[i]['parametro']] = "ajustarFecha(" + parametros[i]['parametro'] + ")"
       ...
       //   $('txt_script').value = dar_formato()
          ...
          setContent($('cmb_lenguaje').value, 'txt_script', $('txt_script').value)
    ...
    case 'XSL':
        ...
        setContent('xml', 'txt_script', $('txt_script').value)
    ...
    case 'MAILTO':
        ...
        setContent('txt', 'txt_body', $('txt_body').value)
    ...
    //    if ($('cb_protocolo')[0].value == "XML")
    //        $('cb_protocolo').remove(0);
    //    if ($('cb_protocolo')[3].value == "XSL")
    //        $('cb_protocolo').remove(3);

    //    $('cb_protocolo').selectedIndex = 2
    //    $('tbFiltro').hide()
    //    $('tbPFILE').show()
    //    $('tbPMAILTO').hide()

    //    $('txt_target').value = arTarget[0]['filename'] == undefined ? '' : arTarget[0]['filename']
    //break         
  }
...
function Aceptar() 
  ...
  switch ($('cb_protocolo')[$('cb_protocolo').selectedIndex].value)
    ...
    //case 'NAME':
    //    var string = 'NAME://' + $('txt_target').value
    //    var script =  objScriptEditar.string_to_script(string)   
    //break    
    }
        ...
        if (res != null) {
            ...
            Dialog.confirm(msj, {
                ...
                onOk: function (win_local) {
                    ...
                    //           script = replace(script, res[i], "' + " + replace(replace(res[i],"{",""),"}","") + " + '")
                    //     }
                    //  }
                    guardar(script, string)
                    _parent.close()
...
function validar_xml()
{
 ...
 if (!objXML.loadXML(strXML)) 
 {
   ...
   //strError += 'strText: ' + objXML.parseError.strText
    strError = 'Código: ' + objXML.parseError.numError + '</br>Descripción: ' + objXML.parseError.description
  }
  
  if (strError != '')  
   return 'Validación Erronea.</br>' + strError
 else 
   return ''
...
{ 
  ...
  //strXML = replace(strXML, "\\\\s*" + String.fromCharCode(13) + String.fromCharCode(10) + "\\\\s*", "") //Reemplazar retorno de carro y salto de linea
  //strXML = replace(strXML, "\\\\s*\n\\\\s*", "") //Reemplazar salto de linea
...
function protocolo_cambiar() 
  ...
  switch (protocolo)
    ...
    case 'MAILTO':
      ...
      if(!editor)
        setContent('text', 'txt_body', $('txt_body').value)
...
function salida_base() 
{
   ...
   if (strError != '') 
    ...
    if (objXML.loadXML(filtroXML)) {
        ...
        strHTML += "</tr>"
        strHTML += "<tr>"
        strHTML += "<td colspan='3' class='Tit4' style='text-align:left;width:30%'>Debe ingresar valores a los parámetros definidos en la consulta.</td>"
        strHTML += "</tr>"
        strHTML += "<tr>"
        ...
        strHTML += "<td style='width:30%; text-align:center; vertical-align:middle'>Parámetro</td>"
...
function btn_aceptar_ec_onclick()
{
    ...
    $('formExportar').action = 'editor_script.aspx?accion=Exportar&filtroXML='+ escape(filtroXML)
    $('formExportar').submit()
...
function seleccionar_plantillas()
{
         winPlantilla = nvFW.createWindow({
            ...
            minimizable: false,
            maximizable: false,
            title: "<b>Seleccione una plantilla para diseñar</b>",
            onShow: function (win) {
...
function onchange_lenguje() {
            ...
            if ($('cmb_lenguaje').value == "vb")
                setContent('vb', 'txt_script', $('txt_script').value)

            if ($('cmb_lenguaje').value == "js")
                setContent('js', 'txt_script', $('txt_script').value)

            if ($('cmb_lenguaje').value == "cs")
                setContent('cs', 'txt_script', $('txt_script').value)
     ...
     <style type="text/css">.CodeMirror-scroll { max-width: 100% }</style>
</head>
<body onload="return window_onload()" onresize="return window_onresize()"  style='background-color:white;width:100%;height:100%;overflow:hidden'>
    <form id="formExportar" name="formExportar" method="POST" target="_blank" style="display:none"></form>
                <div id="divLeft" style="overflow:hidden;display:inline-block;vertical-align:top;width:78%">
                               ...
                               <table id='tbFiltro' style="vertical-align:top">
                                    <tr>
                                        <td style="vertical-align:top">
                                            <textarea id='txt_script' onchange="campo_cargar(event)"  style="vertical-align:top"></textarea>
                                        </td>
                                    </tr>
                                    <tr class="contenedor" id="trFiltroPie">
                                       <td>
                                         <input type="button" style="width: 25%" name="btn_dar_formato" id='btn_dar_formato' value="Dar formato" onclick="dar_formato_onclick()" />
                                         <input type="button" style="width: 25%" name="btn_validar_xml" id='btn_validar_xml' value="Validar XML" onclick="btn_validar_xml()" />
                                         <input type="button" style="width: 25%" name="btn_exportar_reporte" id='btn_exportar_reporte' value="Ejecutar Consulta" onclick="salida_base()" />
                                         <input type="button" style="width: 23%" name="btn_plantillas" id='btn_plantillas' value="Plantillas" onclick="seleccionar_plantillas()" />
                                       </td>
                                    </tr>
                               </table>
                               <table class="tb1" id="tbPFILE" style="display:none">
                                                <tr>
                                                    <td class="Tit1" style="width: 5%">Target:</td>
                                                    <td colspan="6"><span style="display:inline-block;width:15%"><select id="seluri" style="width:100%"><option value="default" selected="selected">Default</option><option value="local">Local</option></select></span><span style="display:inline-block;width:85%"><input type="text" style="width: 100%" id="txt_target" ondragenter="return false" ondragover="return false" onchange="valTxt_target()" onblur="valTxt_target()"  ondrop='valTxt_target_ondrop(event)'/></span></td>
                                                </tr>
                                                <tr>
                                                    <td class="Tit1" style="width: 5%">Codificación:</td>
                                                ...
                                                <tr id="trCompri">
                                                   <td  style="width: 100%" colspan="8" > 
                                                    <table class="tb1">
                                                      <tr>
                                                         <td  style="width: 10%;white-space:nowrap"  class="Tit1">Compresión:</td>
                                                          <td style="width: 10%;">
                                                          ...
                                                          <td  style="width: 10%;white-space:nowrap"  class="Tit1">Protección Contraseña:</td>
                                                          ...
                                                          <td><img onclick="return alert(comp_pwd.value)" title="Ver contraseña" src="/fw/image/transferencia/ojo.png" style="cursor:hand;cursor:pointer;width:100%"/></td>
                ...
                <div id="divRight" style="overflow:hidden;display:inline-block;vertical-align:top;width:21%">
                               <table class="tb1 layout_fixed">
                                <tr class="diccionario">
                                 ...
                                 <tr class="tbLabel contenedor param_old">
                                        <td style="width: 50%" class="class_paramstros">Parámetros</td>
                                ...
                                <tr class="tbLabel contenedor ocultar">
                                    ...
                                    <td class="ocultar"><img src="/fw/image/transferencia/refresh.png" alt="" style="cursor:pointer" onclick="campo_cargar()">Campos/Parámetros</td>
                                </tr>
                                ...
                                <tr class="contenedor">
                                    <td colspan="2">
                                        <input type="text" id="txt_sel" style="width: 100%" />
                                    </td>
                                </tr>
                            </table>
     ...
     <div id="divPlantillas"  style="display:none">
         <table class="tb1" style="width:100%">
                 ...
                 <tr>
                     <td class="tit2" colspan="6" style="text-align:left">Parámetros:</td>
                 </tr>  
                 <tr style="padding-bottom:10px">
