        ...
        <script type="text/javascript">
            ...
            function onChangeIdTransferencia() {
                ...
                if (id) {
                    ...
                    /*if (confirm("¿Desea crear una nueva transferencia?")) {
                        if (confirm("¿Desea guardar los cambios actuales?")) {
                            guardar(false);
                        }
                        redirectToTransferABM();
                    }*/
...
//                    }
        ...
        <script type="text/javascript">
            ...
            function newTransfer(options) {
                ...
                Transferencia.transf_fe_creacion = $('transf_fe_creacion').value = options.transf_fe_creacion;
                Transferencia.transf_fe_modificado = $('transf_fe_modificado').value = options.transf_fe_modificado;
                Transferencia.nombre_operador = $('nombre_operador').value = options.nombre_operador;
...
//                options.style = 'width: ' + options.width + 'px ;height: ' + options.height + 'px; top: -500px; left: -500px;';
            ...
            function ctrl_newPaste() {
               ...
               //  xmlCopy = ""
                ...
                //    options.top = event.clientY - offset.top - options.height / 2;
                //    element.move(options.left, options.top)
                //    console.log(options.left + ' , '+ options.top)
                //});
                //Event.observe(oCB1.div, 'click', function (event) {
        ...
        <script type="text/javascript">
            ...
            function loadAnnotations(id_transferencia) {
                var rs = new tRS();
                //rs.open("<criterio><select vista='Transferencia_notas'><campos>*</campos><orden></orden><grupo></grupo><filtro><id_transferencia type='igual'>" + id_transferencia + "</id_transferencia></filtro></select></criterio>");
            ...
            function loadAnnotationsArrows() {
                Transferencia.annotations.each(function(annotation) {
                    var src;
                    var rs = new tRS();
                    //rs.open("<criterio><select vista='transferencia_notas_det'><campos>*</campos><filtro><id_transferencia_nota type='igual'>" + annotation.id_transferencia_annotation + "</id_transferencia_nota></filtro><orden></orden></select></criterio>");
           ...
           // var win;
            ...
            function makeXML(save_as, accion, save_as_transf_desc, save_as_transf_id) {
                ...
                Transferencia.timeout = $('timeout').value;
                    ...
                    detalle.relations.each(function (relation) {
                        ...
                        if (detalle.id == relation.src.id && relation.dest.type != 'annotation' && contar == true) {
                          ...
                          //  var className = relation.className == null ? '' : relation.className;
                            ...
                            xmldato += "<evaluacion><![CDATA[" + evaluacion + "]]></evaluacion>";
...
//            console.log(xmldato.replace(/'/g, '"'));
           ...
           /* function guardar(recargar, save_as, save_as_transf_desc, save_as_transf_id) {
                ...
                if(validation.valid){
                    ...
                    var xml = makeXML(save_as, 'guardar', save_as_transf_desc, save_as_transf_id);
                    ...
                    alert(validation.errors.join('<br>'), 450, 200);
                }
            }*/
                        ...
                        if (err.params.id_transferencia != -1 && recargar)
                         {
                            ...
                            //campos_defs.set_value('id_transferencia', $('id_transferencia_txt').value)
                ...
                confirm(strHTML,//"Se creará una copia idéntica de toda la transferencia.¿Desea continuar?",
                    {
                        ...
                        onOk: function (w) {
                            ...
                            if ($('transf_guardar_como').value > 0) {
                                ...
                                confirm("¿Desea reeemplazar el contenido de la transferencia <b>" + $("transf_guardar_como_desc").value + "</b>? <input type='hidden' id='save_as_transf_desc_text' value='"+ $('save_as_transf_desc_text').value  +"'>",//"Se creará una copia idéntica de toda la transferencia.¿Desea continuar?",
                                    {
                                        width: 300,
                                        height: "auto",
                                        className: "alphacube",
        ...
        <script type="text/javascript">
            ...
            function cargarFromXML(xmlStr) {
                ...
                /*
                xml.select('parametros parametro').each(function(parametro) {
                    var options = makeOptionsFromXML(parametro);
                    Transferencia.parametros.push(options);
                });
                xml.select('pools pool').each(function(pool) {
                    var options = makeOptionsFromXML(pool);
                    var c_pool = newPool(options, false);
                    pool.select('lanes lane').each(function(lane) {
                        var options = makeOptionsFromXML(lane);
                        newLane(options, c_pool);
                    });
                });

                var detalles = [];
                xml.select('detalles detalle').each(function(detalle) {
                    var options = makeOptionsFromXML(detalle);
                    options.title = options.transferencia;
                    cargar_parametros_det_from_xml(options, detalle);
                    var element = newElement(options);
                    detalles.push({
                        element: element,
                        _relations: detalle.select('relations relation')
                    });
                });

                detalles.each(function(detalle) {
                    detalle._relations.each(function(relation) {
                        var options = makeOptionsFromXML(relation);
                        options.evaluacion = relation.select('evaluacion')[0].innerHTML.replace('<!--[CDATA[','').replace('&lt;![CDATA[','').replace(']]-->', '').replace(']]&gt;', '');
                        options.title = relation.select('title')[0].innerHTML.replace('<!--[CDATA[','').replace('&lt;![CDATA[','').replace(']]-->', '').replace(']]&gt;', '');
                        var src = detalle.element;
                        var dest = Transferencia.detalle[options.dest_temp_id];
                        newArrow(src, dest, options);
                    });
                ...
                xml.select('annotations annotation').each(function(annotation) {
                    var options = makeOptionsFromXML(annotation);
                    var c_annotation = newAnnotation(options, false);
                    annotation.select('relations relation').each(function(relation) {
                        var options = makeOptionsFromXML(relation);
                        options.title = relation.select('title')[0].innerHTML.replace('<!--[CDATA[','').replace('&lt;![CDATA[','').replace(']]-->', '').replace(']]&gt;', '');
                        var src = ObtenerObjnvCtrls(options.id_transf_det);
                        newArrow(src, c_annotation, options);
            ...
            function cargar_parametros_det_from_xml(options, xmlElement) {
                if(options.transf_tipo !== undefined) {
                    ...
                    if (['XLS', 'EQV', 'USR', 'IUS', 'NOS','TRA'].indexOf(options.transf_tipo) != -1) {
                        ...
                        //    options.title = XMLText(NodeParametros_det[r].querySelector('title'));
                          
                        }


                   /*     xmlElement.selector('parametros_det parametro_det').each(function(parametro_det){
                            var obj = {};
                            for (var att_index in parametro_det.attributes) {
                                var attribute = parametro_det.attributes[att_index];
                                if (attribute.ownerElement != undefined) {
                                    obj[attribute.name] = attribute.value;
                                }
                            }
                            if (['XLS', 'EQV','NOS'].indexOf(options.transf_tipo) != -1) {
                                options.parametros_det.push(obj);
                            } else {
                                var parameter = getParameter(obj.parametro);
                                options.parametros_det[parameter.parametro] = {};
            ...
            function transferencia_imprimir(tipo_salida) {
                ...
                if ("<%= id_transferencia %>" != "") {
                    var rs = new tRS();
                    rs.open(nvFW.pageContents.filtroXML_operador_permisos, "", "<criterio><select><filtro><Permitir type='igual'>'" + Transferencia.nombre + " (<%= id_transferencia %>) imprimir'</Permitir></filtro></select></criterio>", "", "")
                ...
                var id_transferencia = parseInt('<%= id_transferencia %>');
                   // openWindow({ path: '/FW/Transferencia/Transferencia_pre_export_pdf.aspx?id_transferencia=' + id_transferencia + '&tipo_salida=' + tipo_salida, descripcion: 'Exportar Transferencia', height: 270, width: 350 })
...
//                        console.log(det);
            ...
            //function onclickSelUndo()
            //{
            //    if ($('sel_undo').selectedIndex == -1)
            //        return

            //    var value = $('sel_undo').options[$('sel_undo').selectedIndex].value
            //    Undo.undo(value)
            //}

            //function onclickSelRedo()
            //{

            //    if ($('sel_redo').selectedIndex == -1)
            //        return

            //    var value = $('sel_redo').options[$('sel_redo').selectedIndex].value
            //    Undo.redo(value)
            //}
            ...
            function abrir_versiones()
            {
                ...
                while (!rs.eof()) 
                {
                    ...
                    strHTML += "<img src='/fw/image/transferencia/seleccionar.png' onclick='return window_transferencia_abm("+ id_transferencia_txt.value +","+ rs.getdata("id_transf_version") +")' style='cursor:hand;cursor:pointer' border='0' align='absmiddle' hspace='1'/>"
                    strHTML += "</td>"
            ...
            var winVersion
            function window_transferencia_abm(id_transferencia,id_transf_version)
            {
    ...
    <body onload="windows_onload()" onresize="return window_onresize()" >
        <table class="tb1">
            <tr class="tbLabel">
                ...
                <td style="width:30%;" colspan="2">Nombre</td>
                ...
                <td style="width:5%;" title="Timeout">T.Out</td>
                <td style="width:15%;">Estado</td>
                <td style="width:10%;">Creado</td>
                <td style="width:10%;">Modificado</td>
                <td style="width:15%;">Operador</td>
            </tr>
            <tr>    
                <td id="topMenu"></td>
                <td style="width:5%;">
                    <input type="text" name="id_transferencia_txt" id="id_transferencia_txt" style="width:100%; text-align:center" onkeypress='return valDigito(event)' disabled="disabled" value="<% = id_transferencia %>"/>
                </td>
                <td style="width:30%;">
                    <input type="text" name="nombre" id="nombre" style="width:100%" />
                </td>
                <td style="width:5%;">
                    <input type="button" value="Versiones" style="width:100%;cursor:pointer" onclick="abrir_versiones()"/>
                </td>
                ...
                <td style="width:10%;">
                    <input type="text" name="transf_fe_creacion" id="transf_fe_creacion" style="width:100%; text-align:left;" disabled="disabled"/>
                </td>
                 <td style="width:10%;">
                     <input type="text" name="transf_fe_modificado" id="transf_fe_modificado" style="width:100%; text-align:left;" disabled="disabled"/>
                </td>
                <td style="width:15%;">
                     <input type="text" name="nombre_operador" id="nombre_operador" style="width:100%" disabled="disabled"/>
                </td>
