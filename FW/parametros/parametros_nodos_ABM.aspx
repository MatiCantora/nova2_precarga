    ...
    <script type="text/javascript" language="javascript">
        ...
        function cargar(){
            ...
            if(!rs.eof()){
                    ...
                    rs_param.open(filtroSeleccionarParametro, '', "<id_param type='igual'>'" + rs.getdata("id_param") + "'</id_param>", '', '')

                    if(rs_param.getdata("wrapper_file"))
                      campos_defs.set_value('wrapper_file', rs_param.getdata("wrapper_file"))
            }
              else{
        ...
        function eliminar() {  
            ...
            if (parametros.length > 0) {
               ...
               html += '<div class="alphacube_message">El módulo tiene parámetros asociados: ¿Desea eliminarlos?</div>'
               ...
               html += '<input style="width:27%" type="button" value="Eliminar módulo" onclick="eliminar_modulo()" class=" " />  ' 
...
//                alert('No es posible guardar los cambios si el parámetro esta codificado.');
//                return;
//            }
            if(!nvFW.tienePermiso("permisos_parametros",1)){ alert("No tiene permisos para guardar el parámetros") ; return }
            var strError = ''
        ...
        function procesarXML(eliminar_dependient) { //si se deben eliminar los dependientes se pasa el nro_par_nodo en negativo para tratarlo en la DB
            ...
            xmldato += " nro_permiso_grupo='" + campos_defs.value("nro_permiso_grupo") + "' nro_permiso='" + campos_defs.value('nro_permiso_dep') + /*"' wrapper_file='" + campos_defs.value('wrapper_file') +*/ "'>"
            ...
            xmldato += "<wrapper_file><![CDATA[" + campos_defs.value('wrapper_file') + "]]></wrapper_file>"
            xmldato += "</nodo>";
            ...
            parametros.each(function(arreglo, i) 
            {
                        ...
                        xmldato += " nro_permiso_grupo ='" + arreglo['nro_permiso_grupo'] + "' nro_permiso ='" + arreglo['nro_permiso_dep'] + /*"' wrapper_file ='" + arreglo['wrapper_file'] + */"'>"
        ...
        function parametros_cargar_arr(nro_par_nodo){            
            ...
            rs.onComplete = function (rs) {   
                while (!rs.eof()) { 
                    ...
                    vacio['wrapper_file'] = rs.getdata('wrapper_file')
        ...
        function eliminar_parametro(indice){
            if (!nvFW.tienePermiso("permisos_parametros", 1)) { alert("No tiene permisos para eliminar el parámetro"); return }
               nvFW.confirm('¿Desea eliminarlo?', { width: 350,
                                okLabel: "Si",
                                cancelLabel: "No",
                                onOk: function(win) {          
        ...
        function redraw(id_param) {
            ...
            if (!rs.eof()) {
                ...
                campos_defs.set_value('wrapper_file', '')
                        return
                    }
            else {
                var rs_param = new tRS()
...
<body onload="return window_onload()" onresize="window_onresize()" style="width:100%;height:100%;overflow:hidden">
    ...
    <div id="divMenuABM"></div>
        <script type="text/javascript" language="javascript">
         ...
         Menus["vMenuABM"].CargarMenuItemXML("<MenuItem id='2' style='width:10%'><Lib TipoLib='offLine'>DocMNG</Lib><icono>buscar</icono><Desc>Buscar Parámetro</Desc><Acciones><Ejecutar Tipo='script'><Codigo>seleccionar_parametro(false)</Codigo></Ejecutar></Acciones></MenuItem>")  
       ...
       <table class="tb1" style="width:100%">
         ...
         <tr id="trPath_paramValor">
            <td class="Tit1" style='width:8%'>Wrapper file:</td>
            <td colspan="3" ><%= nvFW.nvCampo_def.get_html_input("wrapper_file", enDB:=False, nro_campo_tipo:=104)%></td>
         </tr>
       </table>
    </div>   
    ...
    <script type="text/javascript" language="javascript">
     ...
     //Menus["vMenuABM1"].CargarMenuItemXML("<MenuItem id='1' style='width:10%'><Lib TipoLib='offLine'>DocMNG</Lib><icono>explorer</icono><Desc>Parámetros</Desc><Acciones><Ejecutar Tipo='script'><Codigo>seleccionar_parametro(true)</Codigo></Ejecutar></Acciones></MenuItem>")  
