...
<% 
    ...
    Me.contents("nro_par_nodo") = nvFW.nvUtiles.obtenerValor("nro_par_nodo", "")
    Dim stringSQL As String

    Me.contents("filtroParametros") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verParametros_nodos'><campos>*</campos><filtro>[<id_param type='igual'>'%idParam?%'</id_param>][<nro_par_nodo type='igual'>'%nro_par_nodo?%'</nro_par_nodo>][<nro_par_nodo_dep type='igual'>%nro_par_nodo_dep?%</nro_par_nodo_dep>][<par_nodo_tipo type='distinto'>'%par_nodo_tipo_distinto?%'</par_nodo_tipo>][<par_nodo_tipo type='igual'>'%par_nodo_tipo_igual?%'</par_nodo_tipo>]</filtro><orden>orden</orden></select></criterio>")
    Me.contents("filtroSeleccionarParametro") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='parametros_def'><campos>*</campos><filtro>[<id_param type='igual'>'%idParamRs?%'</id_param>]</filtro><orden></orden></select></criterio>")

    Me.contents("filtroParametrosHistorico") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='ver_parametros_value_log'><campos>*</campos><filtro></filtro><orden>momento</orden></select></criterio>")
    ...
    If (modo.ToUpper() = "MODIFICAR") Then
        ...
        Try
            ...
            For i As Integer = 0 To NODS.Count - 1
                ...
                Dim StrSQLparam As String = "if (exists(select 1 from [Parametros_value] where id_param = '" & id_param & "')) "
                StrSQLparam += "update [Parametros_value] set valor = '" & valor & "' where id_param = '" & id_param & "' "
                StrSQLparam += "else "
                StrSQLparam += "INSERT INTO [dbo].[Parametros_Value] ([id_param],[valor]) VALUES ('" & id_param & "','" & valor & "')"
                nvFW.nvDBUtiles.DBExecute(StrSQLparam)
...
<head>
    ...
    <script type="text/javascript" language="javascript">
     ...
     var nro_par_nodo = nvFW.pageContents.nro_par_nodo
     var filtroParametros = nvFW.pageContents.filtroParametros
     var tipo_nodo

     function window_onload(){
         var rs = new tRS();
         
         var rsparams = "<criterio><params nro_par_nodo='" + nro_par_nodo + "' /></criterio>"
         rs.open({
    ...
    function redraw(idParam){
        var rsparams = "<criterio><params idParam='" + idParam + "' /></criterio>"
        var rs = new tRS();
        rs.open({
            filtroXML: nvFW.pageContents.filtroParametros,
            params: rsparams
    ...
    function dibujar(){   
        ...
        var rsparams = "<criterio><params nro_par_nodo_dep='" + nro_par_nodo + "' par_nodo_tipo_distinto='p' /></criterio>"
        var rs = new tRS();
        rs.open({
            filtroXML: nvFW.pageContents.filtroParametros,
            params: rsparams
        ...
        if(tipo_nodo == 'P')
            rsparams = "<criterio><params nro_par_nodo='" + nro_par_nodo + "' par_nodo_tipo_igual='p' /></criterio>"
        else
            rsparams = "<criterio><params nro_par_nodo_dep='" + nro_par_nodo + "' par_nodo_tipo_igual='p' /></criterio>"  
    ...
    function showHistorico(idParam) {
        var poptions =
        {
            width: 500, height: "230",
            title: "<b>Histórico " + idParam + "</b>",
            maximizable: false,
            minimizable: false
        }

        poptions.url = "/FW/parametros/parametros_valor_log.aspx",
        poptions.userData = {}
        poptions.userData.param = idParam
        poptions.userData.filtro_historico = nvFW.pageContents.filtroParametrosHistorico
    ...
    function showValor(idParam, valor, encriptar, permiso_grupo, nro_permiso, _par_nodo) {
        ...
        var idParamRs = rs.getdata("id_param") 
        var rsparam = "<criterio><params idParamRs='" + idParamRs + "' /></criterio>"
        ...
        var idParam = rs.getdata('id_param')
        //var encriptar = rs.getdata('encriptar')
        //var permiso_grupo = rs.getdata('permiso_grupo')
        //var nro_permiso = rs.getdata('nro_permiso')
        //var _par_nodo = rs.getdata('par_nodo')
        //par_nodo = _par_nodo 
        ...
        if (nvFW.tienePermiso(permiso_grupo, nro_permiso)) {
            ...
            if (!rs_param.getdata("wrapper_file")) {

                if (encriptar == 'True') {
                    showValorEncriptado(idParam, valor)

                } else {
                    var htmlDialog = "<table class='tb1' style='height:30px !Important'>" +
                        "<tr>" +
                ...
                var w = window.nvFW != undefined ? window.nvFW : nvFW
                win_archivos_def_detalle_abm = w.createWindow({
                    className: 'alphacube',
                    url: rs_param.getdata("wrapper_file") + '?idParam=' + idParam,
                    title: '<b>Modificar valor</b>',
                    minimizable: true,
                    maximizable: false,
                    draggable: true,
                    resizable: true,
