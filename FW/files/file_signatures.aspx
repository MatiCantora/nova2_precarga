...
<%
    ...
    If modo = "getSignatures" Then
        ...
        Dim nro_entidad As String = op.nro_entidad
        ...
        If Not file Is Nothing Then
            ...
            Else
                ...
                For Each idpki In nvApp.PKIs.Keys
                    
                    Dim pki As tnvPKI = nvApp.PKIs(idpki)
        
                    ' clonar la pki y agregarle los certificados del usuario actual en la carpeta my
                    ' (la instancia pki, esta generada con la carpeta my con los certificados del servidor)
                    Dim myPKI As tnvPKI = pki.clone(tnvPKI.nveunmloadOptions.loadTrusted)
                    
                    ' la pki no carga la carpeta entidades solo las trusted
                    'Dim entidadesFolder As nvFW.pkiFolder = pki.folders("entidades")
                    Dim rsCert As ADODB.Recordset = nvFW.nvDBUtiles.DBOpenRecordset("Select * from [verEntidad_certificados] where carpeta_path='entidades' AND idpki='" & idpki & "' AND nro_entidad=" & nro_entidad & "")
                    myPKI.createFolder("my", False, True)
                    While Not rsCert.EOF
                        Dim cbytes As Byte() = rsCert.Fields("cert_bin").Value
                        Dim certMy As New X509Certificate2(cbytes)
                        myPKI.addCert("my", certMy)
                        rsCert.MoveNext()
                    End While
                    nvDBUtiles.DBCloseRecordset(rsCert)
                    

                    '  *********** validar firmas del documento
                    Dim signaturesStatus As Dictionary(Of String, nvFW.tnvSignVerifyStatus) = doc.verifyFileSignatures(myPKI, nvFW.enumnvRevocationOptions.OCSP_FAIL_CRL)
                    ...
                    For Each signName In signaturesStatus.Keys
                        ...
                        sigVal.signaturesStatus(signName).Add(myPKI.name, signaturesStatus(signName))
                    Next
