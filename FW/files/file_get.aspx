...
<% 
    ...
    Dim f_id As Integer = nvUtiles.obtenerValorType("f_id", 0, nvConvertUtiles.DataTypes.int)
    ...
    If (path <> "") Then
        ...
        For Each Appdir As tnvAppDir In nvApp.app_dirs.Values
            Dim dirs As String() = Appdir.path.Split(New Char() {";"})
            For Each dir As String In dirs
                '  If fisical_path.IndexOf(dir) >= 0 Then
                If System.IO.File.Exists(dir + path.replace("FILE://", "").replace("/", "\")) = True Then
                    fisical_path = dir + path.replace("FILE://", "").replace("/", "\")
                    cumple = True
                    Exit For
                End If
            Next
    ...
    If ref_files_path <> "" Or f_id <> 0 Then
