...
<%
    ...
    If Not op.tienePermiso("permisos_herramientas", 4) Then
        err.numError = -1
        err.mensaje = "No posee permisos para realizar esta acción. Consulte con el Administrador del Sistema."
        err.response()
    End If

    Dim base_f_nro_ubi_default = 2
    ...
    ' Si se define, sirve para controlar que el archivo a subir exista. Se usa dentro del archivo tUpload.js
    Me.contents("filtroFile") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verref_files'><campos>f_id</campos><filtro><borrado type='igual'>0</borrado><f_nombre type='igual'>'%f_nombre%'</f_nombre><f_ext type='igual'>'%f_ext%'</f_ext><f_depende_de type='igual'>%f_depende_de%</f_depende_de></filtro><orden></orden></select></criterio>")

    Me.contents("filtroFileVersiones") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verref_files'><campos>f_id, f_nombre, f_ext, f_nro_tipo, f_size, f_falta, nombre_operador</campos><filtro><f_nombre type='igual'>'%f_nombre%'</f_nombre><f_ext type='igual'>'%f_ext%'</f_ext><f_depende_de type='igual'>%f_depende_de%</f_depende_de></filtro><orden>f_falta desc</orden></select></criterio>")

    ' Consulta para obtener la si existe alguno de los archivos que se van a pegar en la carpeta destino
    Me.contents("filtroCheckFilesExist") = nvFW.nvXMLSQL.encXMLSQL("<criterio><procedure expire_minutes='1' cacheControl='None' AbsolutePage='1' CommandText='dbo.get_ref_files_exist'></procedure></criterio>")

    Me.contents("filtroFileCategorias") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='ref_file_db_categorias'><campos> distinct id_rfile_db_categoria as id, rfile_db_categoria as [campo] </campos><orden>[campo]</orden></select></criterio>")

    Try
        Select Case accion.ToLower()
            Case "disco_alta"

                ' El perfil de administrador es el unico que puede crear discos
                ' op.perfiles esta vacio, hay que cargarlo
               ' op.load(op.operador)
			    Dim esAdministrador As Boolean = False
                For i As Integer = 0 To op.perfiles.Count - 1
                    If op.perfiles(i).tipo_operador_desc.ToLower = "administrador" Then
                ...
                If Not esAdministrador Then
                    err.numError = 12
                    err.mensaje = "No posee permiso para dar de alta un disco. Comuniquese con el administrador"
                    err.response()
                End If
                ...
                Dim discosCount As Integer = rs.Fields("count").Value
                nvDBUtiles.DBCloseRecordset(rs)
                If discosCount > 0 Then
                    err.numError = 12
                ...
                strSQL = "INSERT INTO ref_file_permisos(f_id,tipo_operador,permiso) VALUES (" + CType(f_id, String) + ", 1, 31)" 'todos los permisos al perfil administrador
                cn.Execute(strSQL)
            ...
            Case "carpeta_alta"
                ...
                Dim folderCount As Integer = rs.Fields("count").Value
                nvDBUtiles.DBCloseRecordset(rs)
                If folderCount > 0 Then
                    err.numError = 12
                    err.mensaje = "Ya existe una carpeta con ese nombre"
                    err.response()
                End If
                ...
                If CType(f_nro_ubi, String) = "" Then
                    f_nro_ubi = f_nro_ubi_default
                End If
            ...
            Case "renombrar"
                Dim f_ids As String = nvUtiles.obtenerValor("f_ids")
                Dim f_nombres As String = nvUtiles.obtenerValor("f_nombres")

                Dim f_ids_list() As String = f_ids.Split(",")
                Dim f_nombres_list() As String = f_nombres.Split(",")

                For i As Integer = 0 To f_ids_list.Length - 1
                    Dim f_id As String = f_ids_list(i)
                    Dim f_nombre As String = f_nombres_list(i)
                    ...
                    Dim arr() As String = f_nombre.Split(".")
                    Dim f_ext As String = ""
                    If arr.Length > 1 Then
                        f_ext = arr.Last()
                        f_nombre = String.Join(".", arr.Take(arr.Length() - 1))
                    End If
            ...
            Case "eliminar"
                ...
                Dim f_ids As String = nvUtiles.obtenerValor("f_ids")
                Dim f_ids_list() As String = f_ids.Split(",")
                For Each f_id As String In f_ids_list
            ...
            Case "archivo_pegar"
                ...
                'pegar: si ya existe, lo pisa
                Dim f_cut_id As String = nvUtiles.obtenerValor("f_cut_id", "")
                Dim f_dest As String = nvUtiles.obtenerValor("f_dest", "")
                Dim arr() As String = f_cut_id.Split(",")

                For Each elem In arr

                    Dim rs As ADODB.Recordset = DBExecute("exec get_ref_files_exist '" + elem + "', " + f_dest + "")
                    Dim existe As Boolean = Not rs.EOF
                    Dim f_id_existente As String = ""
                    If existe Then
                        f_id_existente = rs.Fields("f_id_dest").Value
                    End If
                    DBCloseRecordset(rs)

                    ' antes de mover, borrar el existente 
                    If f_id_existente <> "" Then
                        nvDBUtiles.DBExecute("UPDATE ref_files SET borrado=1  where f_id=" + f_id_existente)
                    End If

                    ' mover
                    nvDBUtiles.DBExecute("UPDATE ref_files SET f_depende_de=" & f_dest & " WHERE f_id=" & elem)
                Next

        End Select

    Catch tabortEx As System.Threading.ThreadAbortException 'para obviar el error generado por err.response dentro del bloque try catch
    Catch ex As Exception
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script type="text/javascript">
                            ...
                            ok: function (win) {
                                ...
                                if ($('txt_nc').value != "") {
                                    ...
                                    nvFW.error_ajax_request('/fw/files/file_dialog.aspx', {
                                        bloq_contenedor_on: false,
                                        asynchronous: false,
                                        parameters: {accion: 'carpeta_alta', f_depende_de: file.f_id, f_nombre: filename.f_nombre, f_ext: filename.f_ext},
                                        onSuccess: function(err) {
                                            link_onclick(file.f_id, file.ref_files_path)
                                        },
                                        onFailure: function(err) {
            ...
            function renombrar() {
                ...
                var new_names = {}
                renombrar_dialog(0, ref_files, new_names)
            }


            function renombrar_dialog(index, ref_files, new_names) {

                if (index >= ref_files.length) {
                    var f_ids = []
                    var f_nombres = []
                    for (var f in new_names) {
                        f_ids.push(f)
                        f_nombres.push(new_names[f])
                    }
                    if (f_ids.length == 0) return

                    nvFW.error_ajax_request('/fw/files/file_dialog.aspx', {
                        parameters: { accion: 'renombrar', f_ids: f_ids.join(","), f_nombres: f_nombres.join(",") },
                        onSuccess: function (err) {
                            link_onclick(file.f_id, file.ref_files_path)
                        },
                        onFailure: function (err) {
                ...
                var frm = $('frmPreview')
                var f_id = ref_files[index]
                var file_sel = frm.contentWindow.files[f_id]
                ...
                nvFW.confirm("Renombrar archivo:</b> <div style='width:100%' id='divNC'><input id='txt_nc' style='width:100%' value='" + filename_actual + "' /></div>",
                   {
                       ...
                       cancel: function (win) {
                           win.close();
                           renombrar_dialog(++index, ref_files, new_names)
                           return
                       },
                       ok: function (win) {
                           if ($('txt_nc').value != "") {
                               ...
                               if (!filename_validar($('txt_nc').value)) {
                                   alert("Se han ingresado carcateres inválidos")
                                   return
                               }
                               //var filename = filename_parse($('txt_nc').value)

                               new_names[f_id] = $('txt_nc').value
                               win.close()
                               renombrar_dialog(++index, ref_files, new_names)
                           }
                           else {
            ...
            function eliminar() {
                ...
                if (nSelected == 1) {
                    var file_sel = frm.contentWindow.files[ref_files[0]]
                    var filename_actual = file_sel.f_ext != '' ? file_sel.f_nombre + '.' + file_sel.f_ext : file_sel.f_nombre
                    var msg = "¿Desea eliminar \"" + filename_actual + "\"?"
                } else {
                    var msg = "¿Desea eliminar los elementos seleccionados?"
                }
        ...
        <script type="text/javascript">
            ...
            function get_f_id_selected() {
                ...
                if (nSelected == 0) {
                    ...
                    alert("Seleccione sólo un elemento")
                    return -1
                }
                f_id_sel = ref_files[0]
                return f_id_sel
            }
            ...
            function btnFirmar_onclick(f_id_sel) {
                ...
                var file_seleccionado = $('frmPreview').contentWindow.files[f_id_sel]

                if (file_seleccionado != null) {

                    if (file_seleccionado.f_ext.toLowerCase() != 'pdf') {
                        alert("Actualmente, la firma digital sólo esta soportada para archivos PDF")
                        return
                    }

                    var win = window.top.nvFW.createWindow({
                        title: '<b>Firmar Documento</b>',
                        url: '/fw/document_signing/pdf_signature_editor.aspx?modo=sign_file&f_id=' + f_id_sel,
                        width: 750,
                        height: 650,
                        onClose: function () {
                            if (win.options.userData.retorno["success"]) {
                                // refrescar lista de archivos si se agrego nuevo
                                btnDetalle_onclick()
                            }
                        },
                        destroyOnClose: true
            ...
            function btnVersiones_onclick(f_nombre, f_ext, f_depende_de) {

                var filtroXML = nvFW.pageContents.filtroFileVersiones
                var params = "<criterio><params "
                params += "f_depende_de='" + f_depende_de + "' "
                params += "f_nombre='" + f_nombre + "' "
                params += "f_ext='" + f_ext + "' "
                params += "/>"
                params += "</criterio>"

                var filename = f_nombre + "." + f_ext

                nvFW.exportarReporte({
                    //Parémetros de consulta
                    filtroXML: filtroXML
                    , params: params
                    , parametros: '<parametros filename="' + filename + '" ext="' + f_ext + '"></parametros>'
                    , path_xsl: "report\\file_dialog\\verref_files\\file_versiones.xsl"
                    , salida_tipo: "adjunto"
                    ...
                    , winPrototype: { modal: true,
                        ...
                        center: true,
                        url: '/fw/enBlanco.htm',
                        title: '<b>Versiones del archivo</b>',
                        minimizable: false,
                        maximizable: true,
                        draggable: true,
                        width: 600,
            ...
            function cortar() {
                ...
                nvFW.bloqueo_activar($('frmPreview'), "b0d1")
                var frm = $('frmPreview')
                var f_id_sel = frm.contentWindow.f_id_sel
                ...
                window.top.f_cut_id = f_cut_id
                window.top.f_cut_source = file.f_id

                setTimeout(function () { nvFW.bloqueo_desactivar($('frmPreview'), "b0d1") }, 1000);
            ...
            var existing_f_id = []
            var existing_f_nombre = []

            function pegar() {
                
                if (window.top.f_cut_id && window.top.f_cut_id.length>0) {
                    
                    // si la carpeta origen es igual a la carpeta destino no hacer nada
                    if (window.top.f_cut_source == file.f_id)
                        return

                    var filtroWhere
                    filtroWhere = "<criterio><procedure>"
                    filtroWhere += "<parametros>"
                    filtroWhere += "<select><filtro></filtro><campos>*</campos><orden></orden></select>"
                    filtroWhere += "<ref_files>" + window.top.f_cut_id.join(",") + "</ref_files>"
                    filtroWhere += "<f_dest>" + file.f_id + "</f_dest>"
                    filtroWhere += "</parametros>"
                    ...
                    rs.onComplete = function () {
                        ...
                        while (!rs.eof()) {
                            existing_f_id.push(rs.getdata("f_id"))
                            existing_f_nombre.push(rs.getdata("f_nombre"))
                            rs.movenext()
                        }

                        if (existing_f_id.length > 0) {
                            showDialog(0)
                        } else {
                            pasteFiles()
                        }
                    ...
                    rs.open(nvFW.pageContents.filtroCheckFilesExist, '', filtroWhere, '', '')
                }
            }
            ...
            function showDialog(index) {
                ...
                if (index >= existing_f_id.length) {
                    pasteFiles()
                    return
                } 

                var f_nombre = existing_f_nombre[index]

                var strHTML = "El archivo " + f_nombre + " ya existe, desea sobreescribirlo?<br/>" +
                "<input type='button' value='Si' onclick='setOverwrite(" + index + ",1)'/><br/><input type='button' value='Si, recordar' onclick='setOverwrite(-1, 2)'/>" +
                "<br/><input type='button' value='Omitir' onclick='setOverwrite(" + index + ",3)'/>" +
                "<br/><input type='button' value='Omitir, recordar' onclick='setOverwrite(" + index + ",4)'/>" +
                "<br/><input type='button' value='Cancelar' onclick='setOverwrite(" + index + ",5)'></input>"
            ...
            function setOverwrite(index, selectedOption) {
                ...
                if (selectedOption == 3) {
                    var f_id = existing_f_id[index]
                    var idx = window.top.f_cut_id.indexOf(f_id)
                    if (idx > -1) {
                        window.top.f_cut_id.splice(idx, 1);
                    }
                    showDialog(++index)
                    return
                }
                ...
                if (selectedOption == 4) {
                    ...
                    for (var i = index; i < existing_f_id.length; i++) {
                        var f_id = existing_f_id[i]
                        var idx = window.top.f_cut_id.indexOf(f_id)
                        if (idx > -1) {
                            window.top.f_cut_id.splice(idx, 1);
                        }
            ...
            function pasteFiles() {
                ...
                var ref_files = window.top.f_cut_id.join(",")
                if (ref_files) {
                    nvFW.error_ajax_request('/fw/files/file_dialog.aspx', {
                        parameters: { accion: 'archivo_pegar', f_cut_id: ref_files, f_dest: file.f_id },
                        onSuccess: function (err) {
                            window.top.f_cut_id = []
                            window.top.f_cut_source = null
                            btnDetalle_onclick()
                        },
                        onFailure: function (err) {
                        }
    ...
    <body onload="return window_onload()" onresize="window_onresize()" style="height: 100%;
        ...
        <script language="javascript" type="text/javascript">
            ...
            vMenuDatosFile.loadImage("firma", "../image/icons/signature.png")
            vMenuDatosFile.loadImage("cortar", "../image/file_dialog/cortar.png")
            vMenuDatosFile.loadImage("pegar", "../image/file_dialog/pegar.png")
            ...
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='4'><Lib TipoLib='offLine'>DocMNG</Lib><icono>cortar</icono><Desc>Cortar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>cortar()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='5'><Lib TipoLib='offLine'>DocMNG</Lib><icono>pegar</icono><Desc>Pegar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>pegar()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='6'><Lib TipoLib='offLine'>DocMNG</Lib><icono>eliminar</icono><Desc>Eliminar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>eliminar()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='7'><Lib TipoLib='offLine'>DocMNG</Lib><icono>actualizar</icono><Desc>Actualizar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>btnActualizar_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='8'><Lib TipoLib='offLine'>DocMNG</Lib><icono>detalle</icono><Desc>Detalle</Desc><Acciones><Ejecutar Tipo='script'><Codigo>btnDetalle_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='9'><Lib TipoLib='offLine'>DocMNG</Lib><icono>miniatura</icono><Desc>Miniatura</Desc><Acciones><Ejecutar Tipo='script'><Codigo>btnMiniatura_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='10'><Lib TipoLib='offLine'>DocMNG</Lib><icono>seguridad</icono><Desc>Seguridad</Desc><Acciones><Ejecutar Tipo='script'><Codigo>btnSeguridad_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='11'><Lib TipoLib='offLine'>DocMNG</Lib><icono>propiedades</icono><Desc>Propiedades</Desc><Acciones><Ejecutar Tipo='script'><Codigo>btnPropiedades_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")
            Menus["vMenuDatosFile"].CargarMenuItemXML("<MenuItem id='12'><Lib TipoLib='offLine'>DocMNG</Lib><icono>firma</icono><Desc>Firmar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>btnFirmar_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")
        </script>
                                ...
                                <td style="width: 100%;">
                                    ...
                                    <script type="text/javascript">
                                         ...
                                         campos_defs.add('db_categorias', {enDB: false,
                                            ...
                                            filtroXML:  nvFW.pageContents.filtroFileCategorias,
        ...
        <iframe name="frmEnviar" id="frmEnviar" src="/fw/enBlanco.htm" style="display: none" frameborder="0"></iframe>
    </body>
</html>
