...
<head>
    ...
    <script type="text/javascript">
        ...
        function ABM_Cuentas() {
            var nro_entidad = parseInt($('nro_entidad').value, 10)
            
            if (nro_entidad === 0) {
                alert("Debe completar todos los campos de la Entidad y luego Aplicar los cambios antes de administrar las cuentas relacionadas.")
                return
            }

            //if ((permisos_web & 8388608) == 0) {
            if (!nvFW.tienePermiso("permisos_cuentas", 8)) { // "permisos_web" 24: Cambiar cuenta en propiedades
                alert('No posee permisos para acceder al ABM de cuentas')
                return
            }
            
            //nvFW.abrir_ventana_emergente('/FW/funciones/CuentaBanco_ABM.aspx?nro_entidad=' + nro_entidad, 'Cuentas Bancarias', 'permisos_cuentas', 8, 480, 900, false, false, false, true)
            winBanco = nvFW.createWindow( { className: 'alphacube',
                    async: false,
                    url: '/FW/funciones/CuentaBanco_ABM.aspx?id_tipo=' + nro_entidad + '&nro_ent_id_tipo=1',
            ...
            winBanco.options.userData = {}
            winBanco.showCenter(true)
        }


        var mostrarMenuFPagosCobros = false;
        function verFormas_pagos_cobros() {
            ObtenerVentana('frame_comentarios').location.href = '/fw/funciones/ent_pagos_tipo.aspx?nro_entidad=' + $nro_entidad.value
            mostrarMenuFPagosCobros = true
            ...
            <div id="divDatosPersonales" style="width: 100%; height: auto;">
                ...
                <script type="text/javascript">
                    if (nvFW.pageContents.nro_entidad != '') {
                        ...
                        vMenuContactos.loadImage("bank", "/FW/image/icons/banco.png");
                        ...
                        Menus["vMenuContactos"].CargarMenuItemXML("<MenuItem id='4'><Lib TipoLib='offLine'>DocMNG</Lib><icono>bank</icono><Desc>ABM Cuentas</Desc><Acciones><Ejecutar Tipo='script'><Codigo>ABM_Cuentas()</Codigo></Ejecutar></Acciones></MenuItem>")
                        Menus["vMenuContactos"].CargarMenuItemXML("<MenuItem id='5'><Lib TipoLib='offLine'>DocMNG</Lib><icono>bank</icono><Desc>Pagos y Cobros</Desc><Acciones><Ejecutar Tipo='script'><Codigo>verFormas_pagos_cobros()</Codigo></Ejecutar></Acciones></MenuItem>")
