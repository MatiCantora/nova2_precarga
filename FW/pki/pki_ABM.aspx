...
<%
    ...
    If modo = "VA" Or modo = "" Then
        ...
        Me.contents("consulta_ver_certificados") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verpki_certificados'><campos>*</campos><orden></orden><filtro><IDCert type='igual'>%IDCert%</IDCert></filtro></select></criterio>")
        Me.contents("consulta_ver_certificados_extensiones") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='PKI_Certificados_extensiones'><campos>*</campos><orden></orden><filtro><idcert type='igual'>%idcert%</idcert></filtro></select></criterio>")
    Else
        ...
        Try
            ...
            If modo = "A" Then
                If cer Is Nothing Then
                    Throw New Exception("Se debe especificar un certificado para el root de la PKI.")
                End If
                err = nvFW.nvPKIDBUtil.dbAddPKI(idpki, pki, pki_comentario, esConfiable, urlTsa, cer)
            ElseIf modo = "M" Then
                err = nvFW.nvPKIDBUtil.dbUpdatePKI(idpki, pki, pki_comentario, acraiz, esConfiable, urlTsa, cer)
            Else
                err = nvFW.nvPKIDBUtil.dbDeletePKI(idpki)
            End If
...
<head>
    ...
    <script type="text/javascript">
        ...
        function pki_certificado_cargar(acraiz) {
            ...
            var strHTML = ''
            var rs = new tRS()
            var params = "<criterio><params IDCert= '" + acraiz + "' /></criterio>"
            ...
            if (!rs.eof()) {
                ...
                var rsE = new tRS()
                var params = "<criterio><params idcert= '" + acraiz + "' /></criterio>"
                rsE.open(nvFW.pageContents.consulta_ver_certificados_extensiones, "", "", "", params);
                while (!rsE.eof()) {
