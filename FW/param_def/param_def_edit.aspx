...
param_def.each(function(arreglo,j)
{
    if (param_def[j].eliminar == false) {
        arreglo["campo_def"] != "" ? $('tipo_param' + j).options.selectedIndex = 1 : 0
...
function prototype_window(obj)
    ...
    //}
    Dialog.confirm(used + "¿Desea borrar el parámetro <b>"+ param_def[j].parametro + "</b>?", {   width: 400,
                                                                 className: "alphacube",
                                                                 okLabel: "Si",
                                                                 cancelLabel: "No",
                                                                 onOk: function(win){
...
function campo_parametro_validar()
{
 ...
 param_def.each(function(arreglo,j)
 {
     if (param_def[j].eliminar == false) {
         if ($('parametro' + j).value == "")
             strError = j + "-El parámetro " + (j + 1).toString() + " está vacio."
             ...
             strError = j + "-El parámetro '" + $('parametro' + j).value + "' necesita el nombre de la función en el campo valor por defecto."
         }
         else if ($('tipo_param' + j).options[$('tipo_param' + j).selectedIndex].text == 'Campo Def' && $('campo_def' + j).value == "") {
             strError = j + "-El parámetro '" + $('parametro' + j).value + "' necesita selección de campo def."
         }
     }
   });
...
<body onload="return window_onload()" onresize="return window_onresize()" onunload="window_onunload()" style="width:100% !Important; height: 100% !Important; overflow: hidden; margin: 0px; padding: 0px; ">
        ...
        <table class='tb1' id="tbCab">
         <tr class='tbLabel'>
           ...
           <td style='text-align:center;width:10%'>Parámetro</td>
           ...
           <td style='width:10%; text-align:center'>Parametría</td>
