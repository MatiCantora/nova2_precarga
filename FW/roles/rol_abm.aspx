    ...
    <script type="text/javascript">
        ...
        function window_onload() {
            ...
            if ($('nro_entidad').value != '') {
                ...
                buscar_entidad_roles_param($('nro_entidad').value, $('nro_rol').value)
                $('modo').value = 'M'
            }
            else {
                entidad_nueva()
        ...
        function entidad_nueva() {
            ...
            decidir_tabla('0')
            buscar_param_roles()
            $('frame_comentarios').src = "enBlanco.htm"
        }
        ...
        function CargarDatos(nro_entidad) {
            ...
            rs.onComplete = function (rs) {
                if (!rs.eof()) {
                    ...
                    $('localidad').value = rs.getdata('localidad')
            ...
            rs.open("<criterio><select vista='verEntidades'><campos>nro_entidad,razon_social,abreviacion,calle,postal,localidad,telefono,email,cuit,car_tel,nro_docu,tipo_docu,sexo,apellido,nombres,alias,persona_fisica</campos><orden></orden><filtro><nro_entidad type='igual'>" + nro_entidad + "</nro_entidad></filtro></select></criterio>")
        }

        function VerComentarios() {
            ObtenerVentana('frame_comentarios').location.href = '/fw/comentario/verCom_registro.aspx?nro_entidad=' + $('nro_entidad').value + '&nro_com_id_tipo=5&collapsed_fck=1&do_zoom=0&id_tipo=' + $('nro_entidad').value + '&nro_com_grupo=5'
        }
        ...
        function validarCUIT() {
            ...
            if (!validarFormatoCUIT(cuit)) {
                ...
                var rs = new tRS();
                rs.open("<criterio><select vista='Entidades'><campos>nro_entidad,Razon_social,cuit</campos><orden></orden><filtro><cuit type='like'>" + cuit + "</cuit></filtro></select></criterio>")
                ...
                if (rs.recordcount >= 1) {
                    ...
                    if (accion == 'alta') {
                        win_rol_seleccion = new Window({ className: 'alphacube',
                            url: 'rol_seleccion.asp?cuit=' + cuit + '&nro_entidad=' + nro_entidad + '&id_ventana=' + window.top.win.content.id,
