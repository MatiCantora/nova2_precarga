...
<%
    ...
    Me.contents("filtroverTransf_log_cab") = nvXMLSQL.encXMLSQL("<criterio><select vista='verTransf_log_cab'><campos>distinct tieneSubprocesos,esSubproceso,id_transf_log,fe_inicio,fe_fin,estado,id_transferencia,nombre,operador,Login, nombre_operador,resumen,time_seg</campos><filtro></filtro><orden></orden></select></criterio>")
    Me.contents("filtroverTransf_log_subproc") = nvXMLSQL.encXMLSQL("<criterio><select vista='verTransf_log_subproc'><campos>distinct 1 as esSubproceso,id_transf_log,fe_inicio,fe_fin,estado,id_transferencia,nombre,operador,Login, nombre_operador,resumen,time_seg</campos><filtro></filtro><orden></orden></select></criterio>")

    Me.contents("filtroverTransf_log_param") = nvXMLSQL.encXMLSQL("<criterio><select vista='verTransf_log_param'><campos>distinct id_transf_log,id_transferencia,nombre,estado,fe_inicio,fe_fin,parametro,valor,nombre_operador,operador,time_seg</campos><filtro></filtro><orden></orden></select></criterio>")
...
function mostrar_transf_return() {
   ...
   if(('actualizar' in win_seg.options.userData))
	 btnMostrar_transferencia()
...
{ 
  ...
  if (key == 13)
    setTimeout("btnMostrar_transferencia()",500)
}
