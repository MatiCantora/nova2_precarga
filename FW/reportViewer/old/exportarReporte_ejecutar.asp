...
function RSXMLtoExcelRs(rs,export_exeption)
  {
  //debugger
  ...
  try
    ...
    var path_modelo = Request.ServerVariables("APPL_PHYSICAL_PATH").Item + "fw\\reportViewer\\modelo_excel" + ext_modelo
    
    rptError.numError = '9'
    rptError.mensaje = 'exAPP.Workbooks.Open(path_modelo)'
    var exLibro = exAPP.Workbooks.Open(path_modelo)
    ...
    var path_tmp_i = 0
    var path_tmp  = Request.ServerVariables("APPL_PHYSICAL_PATH").Item + "fw\\reportViewer\\tmp\\tmp_exportar_excel" + path_tmp_i + ext_modelo
    while (fso.FileExists(path_tmp))
      {
       path_tmp_i++
       path_tmp  = Request.ServerVariables("APPL_PHYSICAL_PATH").Item + "fw\\reportViewer\\tmp\\tmp_exportar_excel" + path_tmp_i + ext_modelo
      }
      
    rptError.numError = '12'
    rptError.mensaje = 'exLibro.SaveAs(path_tmp)'
    exLibro.SaveAs(path_tmp) 
...
if(export_exeption.indexOf("RSXMLtoExcel") > -1)
  ...
  try
    {
    path_temp = RSXMLtoExcelRs(rs,export_exeption)
    DBCloseRecordset(rs)
    }
  catch(e)
    {
