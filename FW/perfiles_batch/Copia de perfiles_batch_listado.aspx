...
<head>
    ...
    function batch_descargar_excel(id_batch, nombre_excel){
        nvFW.error_ajax_request('/fw/perfiles_batch/perfiles_batch.aspx', {
            parameters: { accion: "exportar", id_batch: id_batch },
            onSuccess: function(er, transport){
                if (er.numError != 0){
                    nvFW.alert("Error al exportar archivo.")
                }
                else{     
                    nvFW.exportarReporte({
                        xml_data: er.params.strXML,
                        path_xsl: "report/excel_base.xsl",
                        filename: nombre_excel.split(".")[0]+ ".xls",
                        salida_tipo: "adjunto",
