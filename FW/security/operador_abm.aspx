...
<%
        ...
        Select Case accion.ToLower
            Case "get_operador"
                ...
                Try
                    ...
                    Dim cuenta_observacion As String = nvXMLUtiles.getNodeText(oXML, "criterio/observacion", "") 'cargamos las observaciones de la cuenta
                    Dim cuenta_set_password As Boolean = nvXMLUtiles.getNodeText(oXML, "criterio/login", "") = "" 'en el caso que el login no exista le decimos que setee la password

                    Dim use_credencial = nvXMLUtiles.getNodeText(oXML, "criterio/use_credencial", "")
                    Dim use_certificate = nvXMLUtiles.getNodeText(oXML, "criterio/use_certificate", "")
                    Dim use_jwt = nvXMLUtiles.getNodeText(oXML, "criterio/use_jwt", "")
                    Dim restriction_ip = nvXMLUtiles.getNodeText(oXML, "criterio/restriction_ip", "")
                ...
                Catch ex As Exception
                    ...
                    err.mensaje = "No se pudo realizar la acción solicitada. Detalle: " & ex.Message
                    err.debug_src = "operador_abm.aspx::get_operador"
                End Try
            ...
            Case "abm_operador"
                ...
                Try
                    ...
                    Dim forzar As Boolean = nvUtiles.obtenerValor("forzar", "False")
                    ...
                    If oXml.SelectSingleNode("criterio/cuenta_existe").InnerText.ToLower = "false" Then
                    ...
                    oXml.SelectSingleNode("criterio/forzar").InnerText = forzar.ToString.ToLower
                ...
                Catch ex As Exception
                    ...
                    err.parse_error_script(ex)
                    err.titulo = "Error en ABM operador"
                    err.mensaje = "No se pudo realizar la acción solicitada. Detalles: " & ex.Message
                    err.debug_src = "operador_abm.aspx::abm_operador"
                End Try
    ...
    Me.contents("filtroOperadorIp") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verOperador_ip_tipo'><campos>*</campos><filtro><operador type='igual'>" + operador + "</operador></filtro><orden></orden></select></criterio>")

    Dim verificacion As String = "La cuenta se valida contra <b>" & IIf(nvApp.ads_dc = "", "", nvApp.ads_dc)

    If Not IsNothing(nvApp.ads_dominio) AndAlso nvApp.ads_dominio <> "" Then
        verificacion &= "." & nvApp.ads_dominio
    End If

    verificacion &= "</b> mediante el protocolo <b>" & nvApp.ads_access & "</b>"

    Dim fechaHoy As String = Date.Today.ToString("dd/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture)
%>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function operador_cargar(forzar)
        {
            ...
            nvFW.error_ajax_request('operador_abm.aspx',
            {
                ...
                onSuccess: function(err)
                {
                    ...
                    if (objXML.loadXML(err.params.cuenta_observacion))
                    {
                        $('validador').insert({ top: XMLtoString(objXML.selectSingleNode(".")) });
                    }
                    ...
                    if (objXML.loadXML(err.params.operadorXML))
                    {
                        ...
                        $('use_credential').checked = XMLText(objXML.selectSingleNode("operador/use_credential")).toLowerCase() == "true";
                        $('use_clientCertificate').checked    = XMLText(objXML.selectSingleNode("operador/use_clientCertificate")).toLowerCase() == "true";
                        $('use_tokenJWT').checked    = XMLText(objXML.selectSingleNode("operador/use_tokenJWT")).toLowerCase() == "true";
                        $('restriction_ip').checked    = XMLText(objXML.selectSingleNode("operador/restriction_ip")).toLowerCase() == "true";

                        entidad_cargar();
                        
                        $login.value                    = XMLText(objXML.selectSingleNode("operador/login"));
                        $login.disabled                 = true;
                        $('cuenta_habilitada').checked  = err.params.enebleApplication.toLowerCase() == 'true';
                        $('cuenta_cambiar_pwd').checked = err.params.cuenta_cambiar_pwd.toLowerCase() == 'true';
                        cuenta_set_password = err.params.cuenta_set_password.toLowerCase() == 'true';
                        
                        $restriccion_ip = restriccion_ip_cargar()
                        if (forzar)
                        {
        ...
        function restriccion_ip_cargar() {

           var xmldato = "<ips>"
           var rs = new tRS()
           rs.open({filtroXML: nvFW.pageContents.filtroOperadorIp});

           while (!rs.eof()) {
                xmldato += "<ip IP='" + rs.getdata('mask')  + "'"
                xmldato += " mask='" + rs.getdata('IP') + "'"
                xmldato += " type='" + rs.getdata('ip_typeID') + "'></ip>"
                rs.movenext()
            }
            xmldato += "</ips>"
        ...
        function perfil_cargar(Node)
        {
            ...
            tabla_perfil.campos = [
                ...
                },
                {
                    ...
                    get_campo:      function(nombreTabla, id) {
                        ...
                        // Armo la fecha con el formato DD/MM/YYYY
                        // Si no estan completos rellenar con cero. Ejemplo: 3 => 03
                        campos_defs.set_value(nombreTabla + "_campos_defs" + id, '<% = fechaHoy %>');
                    }
                },
                {
                    nombreCampo:    "fe_baja", 
            ...
            for (var i = 0, cant_nodos = NOD.length; i < cant_nodos; i++)
            {
                ...
                for (var index_campos = 0, cant_campos = tabla_perfil.campos.length; index_campos < cant_campos; ++index_campos)
                {
                    ...
                    if (tabla_perfil.campos[index_campos].nombreCampo == 'fe_alta' || tabla_perfil.campos[index_campos].nombreCampo == 'fe_baja')
                    {
                        valor_campo = valor_campo.split(' ')[0];
                    }
            ...
            if ($('nro_entidad').value > 0)
            {
                ...
                if (!rs.eof())
                {
                    if (rs.getdata('persona_fisica').toLowerCase() == 'true')
                    {
                        ...
                        $nombre_operador.value = $apellido.value + ' ' + $nombres.value;
                    }
                    else
                    {
                        $tbEntidadPersonaFisica.hide();
                        $tbEntidadPersonaJuridica.show();
                        $tipo_cuitcuil.value   = rs.getdata('cuitcuil') != undefined ? rs.getdata('cuitcuil').toUpperCase() : "CUIT";
                        $nro_cuitcuil.value    = rs.getdata('cuit')         || "";
                        $razon_social.value    = rs.getdata('Razon_social') || "";
                        $nombre_operador.value = $razon_social.value;
                    }
                }
        ...
        function operador_nuevo()
        {
            ...
            $('use_credential').checked = false;
            $('use_clientCertificate').checked = false;
            $('use_tokenJWT').checked = false;
            $('restriction_ip').checked = false;
          ...
          /* for (var i = 0, cant_nodos = Nodesdatos.length; i < cant_nodos; i++)
            {
                if (selectSingleNode('@campo_def', Nodesdatos[i]).value != "")
                {
                    ...
                    {
                        strError += 'Debe seleccionar un valor al atributo <b>' + selectSingleNode('@campo_def', Nodesdatos[i]).value + '</b>.';
                    }
                }
            }
        ...
        function ajax_request(forzar)
        {
            forzar = forzar == undefined ? false : forzar
            ...
            xmldato += "<use_credential>" + $('use_credential').checked.toString() + "</use_credential>"
            xmldato += "<use_clientCertificate>" + $('use_clientCertificate').checked.toString() + "</use_clientCertificate>"
            xmldato += "<use_tokenJWT>" + $('use_tokenJWT').checked.toString() + "</use_tokenJWT>"
            xmldato += "<restriction_ip>" + $('restriction_ip').checked.toString() + "</restriction_ip>"
            ...
            nvFW.error_ajax_request('operador_abm.aspx', {
                ...
                onSuccess: function(err, transport)
                {
                    ...
                    // Si hay alguna ventana abierta, la cerramos
                    if (win_estado_cuenta != null)
                    {
                        win_estado_cuenta.close();
                    }
                ...
                onFailure: function(err)
                {
                    // Si numError es negativo => ALERT con mensaje de error que viene
                    // Si numError es positivo => CONFIRM preguntando si se fuerza el guardado
                    if (err.numError < 0)
                    {
                        alert(err.mensaje,
                        {
                    ...
                    {
                        nvFW.confirm('No se pudo acceder al Active directory (AD). Si lo desea, puede registrar los cambios que no afectan al AD. ¿Desea registrarlos?',
                        {
                            okLabel:     'Si',
                            cancelLabel: 'No',
                            onOk:        function (win)
        ...
        function window_onresize()
        {
            try {
              ...
              //  var tbtabla_perfil = $('tbtabla_perfil');
                ...
                $('tbtabla_perfil').setStyle({ height: (tamanio + "px") })
        ...
        function ComoGuardarEstadoCuenta()
        {
            ...
            win_estado_cuenta = nvFW.createWindow(
            {
                width:          700,
                height:         150,
                draggable:      false,
            ...
            win_estado_cuenta.getContent().innerHTML = $('divComoguardarEstadoCuenta').innerHTML;
            win_estado_cuenta.showCenter(true);
        }
        ...
        function restriccion_ip_abm() {

            var win = nvFW.createWindow(
            {
                url:            'operador_abm_ip.aspx?operador=' + $('operador').value,
                width:          600,
                height:         400,
                draggable:      true,
                resizable:      true,
                ...
                maximizable:    true,
                destroyOnClose: true,
                title:          "<b>Restriccion de IP</b>",
                onClose:        restriccion_ip_abm_return
            });

            win.options.userData          = {};
            win.options.userData.operador = operador;
            win.options.userData.xmlip    = "";
            win.showCenter(true);

        }

        function restriccion_ip_abm_return() {

            if (this.userData.xmlip != "")
                $restriccion_ip = this.userData.xmlip
...
<body onload="return window_onload()" onresize="window_onresize()" style="height: 100%; width:100%; vertical-align: top; overflow: hidden;">
    <form style="width: 100%; height: 100%; margin: 0;" autocomplete="off">
        ...
        <div id="divCab" style="width: 100%;">
            ...
            <script type="text/javascript">
                ...
                Menus["vOperador"].CargarMenuItemXML("<MenuItem id='0' style='WIDTH: 10%; text-align: center;'><Lib TipoLib='offLine'>DocMNG</Lib><icono>guardar</icono><Desc>Guardar</Desc><Acciones><Ejecutar Tipo='script'><Codigo>guardar_onclick()</Codigo></Ejecutar></Acciones></MenuItem>")                         
                ...
                Menus["vOperador"].CargarMenuItemXML("<MenuItem id='2' style='WIDTH: 10%; text-align: center;'><Lib TipoLib='offLine'>DocMNG</Lib><icono>clave</icono><Desc>Setear contraseña</Desc><Acciones><Ejecutar Tipo='script'><Codigo>login_setear_pwd(true)</Codigo></Ejecutar></Acciones></MenuItem>")
                Menus["vOperador"].CargarMenuItemXML("<MenuItem id='3' style='WIDTH: 10%; text-align: center;'><Lib TipoLib='offLine'>DocMNG</Lib><icono>nueva</icono><Desc>Nuevo</Desc><Acciones><Ejecutar Tipo='script'><Codigo>operador_nuevo()</Codigo></Ejecutar></Acciones></MenuItem>")
                       ...
                       <table class="tb1">
                        ...
                        <div id="divUseIdentify">
                            <table class="tb1">
                                <tr class="tbLabel" >
                                 ...
                                 <tr>
                                    <td style="width:40%"><input type="checkbox" name="solo_interfaces" id="solo_interfaces" style='width: 20px; text-align: center; border: none; cursor: pointer;' />Sólo interfaces.</td>
                                    <td><input type="checkbox" name="restriction_ip" id="restriction_ip" style='text-align:left;width: 20px; text-align: center; border: none; cursor: pointer;' />Restricciones IP. <button style="width:50px" onclick="restriccion_ip_abm()">...</button></td>
                                </tr>
                                </table>
                               <table class="tb1">
                                <tr class="tbLabel">
                                    <td colspan="3">
                                        <b>Canal de Autenticación:</b>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="width:40%">
                                        <input type="checkbox" name="use_credential" id="use_credential" style='width: 20px; text-align: center; border: none; cursor: pointer;' />Credenciales.
                                    </td>
                                    <td style="width:40%">
                                        <input type="checkbox" name="use_clientCertificate" id="use_clientCertificate" style='width: 20px; text-align: center; border: none; cursor: pointer;' />Certificado Cliente.
                                    </td>
                                    <td>
                                        <input type="checkbox" name="use_tokenJWT" id="use_tokenJWT" style='width: 20px; text-align: center; border: none; cursor: pointer;' />Token JWT.
                                    </td>
                                </tr>
                            </table>
                        </div>
                        <div id="divCuenta">
                            <table class="tb1">
                                ...
                                <tr>
                                    <td style="width:40%">
                                        <input type="checkbox" name="cuenta_habilitada" id="cuenta_habilitada" style='width: 20px; text-align: center; border: none; cursor: pointer;' />Cuenta habilitada.
                                    </td>
                                    <td>
                                        <input type="checkbox" name="cuenta_cambiar_pwd" id="cuenta_cambiar_pwd" style='width: 20px; text-align: center; border: none; cursor: pointer;' />El usuario debe cambiar la contraseña en el siguiente inicio de sesión.
                                    </td>
                                </tr>
            ...
            <table class="tb1" id="tbEntidadPersonaFisica">
                <tr>
                    <td class="Tit4" style="width: 10%;vertical-align: middle; text-align: center" colspan="2" disabled="disabled"><b>Documento</b></td>
                    <td class="Tit4" style="width: 5%;vertical-align: middle; text-align: center" disabled="disabled"><b>Sexo</b></td>
                    <td class="Tit4" style="width: 15%;vertical-align: middle; text-align: center" disabled="disabled"><b>CUIT/CUIL</b></td>
                    <td class="Tit4" style="width: 8%; vertical-align: middle; text-align: center" nowrap="nowrap" disabled="disabled"><b>Apellido</b></td>
                    <td class="Tit4" style="vertical-align: middle; text-align: center" nowrap="nowrap" disabled="disabled"><b>Nombre</b></td>
                </tr>
                <tr>
            ...
            <div id="divMemuAvanzado" style="margin: 0px; padding: 0px;"></div>
            <script type="text/javascript">
                var vMemuAvanzado = new tMenu('divMemuAvanzado', 'vMemuAvanzado');
                Menus["vMemuAvanzado"] = vMemuAvanzado
                Menus["vMemuAvanzado"].alineacion = 'centro';
                Menus["vMemuAvanzado"].estilo = 'A';
                Menus["vMemuAvanzado"].CargarMenuItemXML("<MenuItem id='0' style='WIDTH: 100%;font-weight:bold'><Lib TipoLib='offLine'>DocMNG</Lib><icono></icono><Desc>Datos Complementarias:</Desc></MenuItem>")
                vMemuAvanzado.MostrarMenu()
            </script>

            <div style="margin: 0px; padding: 0px;max-height:100px;overflow:auto">
            <table class="tb1">
                <%
                    For Each dato As Object In nvFW.nvApp.getInstance().operador.datos.values
                        Response.Write("<tr><td class='Tit2' style='width: 20%; vertical-align: middle; text-align: left; height: 21px;' nowrap='nowrap'>" & dato.label & ":</td>")
                        Response.Write("<td style='width: 100%;vertical-align: middle; text-align: center' nowrap='nowrap'>")
        ...
        <div id="tbtabla_perfil" style="width: 100%; overflow: auto;">
