...
<%
    ...
    If (modo.ToUpper() = "GUARDAR") Then
        ...
        Try
            ...
            Dim cmd As New nvFW.nvDBUtiles.tnvDBCommand("FW_permiso_nodos_abm", ADODB.CommandTypeEnum.adCmdStoredProc)
    ...
    Me.contents("filtroCargar") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista=""permiso_nodos""><campos>*</campos><filtro></filtro><orden></orden></select></criterio>")
    Dim filtroTieneDependientes = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista=""verPermiso_Nodos_Dep""><campos>*</campos><filtro></filtro><orden></orden></select></criterio>")
    Dim filtroPermisosCargar = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista=""verPermiso_Nodos""><campos>*</campos><filtro></filtro><orden>per_orden</orden></select></criterio>")
    Dim filtroNodoPadre = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista=""permiso_nodos""><campos>top 1 nro_per_nodo</campos><filtro></filtro><orden></orden></select></criterio>")
    Dim filtroPermisoAsociado = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista=""permiso_nodos""><campos>top 1 nro_per_nodo</campos><filtro></filtro><orden></orden></select></criterio>")
    Me.contents("filtroTipoNodo") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='permiso_nodos_tipos'><campos>distinct per_nodo_tipo as id, per_nodo_tipo_desc as [campo]</campos><orden>[campo]</orden><filtro><per_nodo_tipo type='distinto'>'R'</per_nodo_tipo></filtro></select></criterio>")
    Me.contents("filtroPermisoGrupo") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='Operador_permiso_grupo'><campos>distinct nro_permiso_grupo as id, permiso_grupo as [campo] </campos><orden>[campo]</orden></select></criterio>")
    Me.contents("filtroPermisoDetalle") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='operador_permiso_detalle'><campos>distinct nro_permiso as id, Permitir as [campo] </campos><orden>[campo]</orden></select></criterio>")
...
<head>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function per_nodo_tipo_onchange() {
            ...
            if( campos_defs.get_value("per_nodo_tipo") == 'P')
            {
            ...
            if( campos_defs.get_value("per_nodo_tipo") == 'M')
            {
        ...
        function cargar(){ 
            ...
            var i = 0
            var rs = new tRS();
            rs.open(nvFW.pageContents.filtroCargar, '', filtroWhere, '', '')
            if(!rs.eof())
                {
                 ...
                 if(es_nodo_padre($('nro_per_nodo').value) || campos_defs.get_value("per_nodo_tipo") == 'P')
        ...
        function eliminar() {
            ...
            if(es_nodo_padre($('nro_per_nodo').value)) {
                alert('El elemento tiene módulos o permisos asociados.')
                return
            }
        ...
        function permisos_cargar() 
        {
            ...
            {     
                while (!rs.eof()) 
                {
                    ...
                    vacio['nro_permiso_grupo'] = rs.getdata('nro_permiso_grupo') != null ? rs.getdata('nro_permiso_grupo') : 0
                    vacio['permiso_grupo'] = rs.getdata('permiso_grupo')
                    vacio['nro_permiso'] = rs.getdata('nro_permiso') != null ? rs.getdata('nro_permiso') : 0
        ...
        function permisos_dibujar() 
        {
            if (permisos.length >= 0) 
                ...
                var strHTML = '<table id="tb_permisos" class="tb1" style="width:100%; vertical-align: top;">'
                    ...
                    strHTML += '<td style="text-align:center">Descripción</td>'
    ...
    <script type="text/javascript" language="javascript">
     ...
     vMenuABM.MostrarMenu()
       ...
       <table class="tb1" style="width:100%">
          ...
          <tr id="trPer_nodo">
             <td style='width:20%' class="Tit1" >Descripción:</td>
             <td colspan="3"> 
          ...
          <td colspan="3" id="td_nro_permiso_grupo"><%= nvFW.nvCampo_def.get_html_input("nro_permiso_grupo", enDB:=False, nro_campo_tipo:=1, filtroXML:="<criterio><select vista='Operador_permiso_grupo'><campos>distinct nro_permiso_grupo as id, permiso_grupo as [campo] </campos><orden>[campo]</orden></select></criterio>", filtroWhere:="<nro_permiso_grupo type='igual'>%campo_value%</nro_permiso_grupo>")%></td>
          </tr> 
          <tr id="trPermiso">
          <td style='width:20%' class="Tit1">Permiso Detalle:</td>
          <td colspan="3" id="td_nro_permiso"><%= nvFW.nvCampo_def.get_html_input("nro_permiso_dep", enDB:=False, nro_campo_tipo:=1,filtroXML:="<criterio><select vista='operador_permiso_detalle'><campos>distinct nro_permiso as id, Permitir as [campo] </campos><orden>[campo]</orden></select></criterio>",filtroWhere:="<nro_permiso type='igual'>%campo_value%</nro_permiso>", depende_de:="nro_permiso_grupo", depende_de_campo:="nro_permiso_grupo")%></td>
          </tr> 
       </table>
    </div>   
    ...
    <div id="div_permisos_abm">
      <table id="tb_perfil_param_def_am" class="tb1" style="table-layout:fixed;">
        <tr class="tbLabel0">
            ...
            <td style="width:18%;text-align:center">Tipo</td>
            <td style="text-align:center" id="td_per_nodo_ABM_Cab">Descripción</td>
        ...
        <tr>
         ...
         <td style="width:18%;text-align:left" id="td_per_tipo">
            <script type="text/javascript">
                   campos_defs.add('per_nodo_tipo_ABM', {    despliega: 'arriba',
                                                             enDB: false,
                                                             target: 'td_per_tipo',
                                                             nro_campo_tipo: 1,
                                                             filtroXML: nvFW.pageContents.filtroTipoNodo,
         ...
         <td style="width:20%;text-align:left" id="td_nro_permiso_ABM">
            <script type="text/javascript">
                   campos_defs.add('nro_permiso_ABM', {      despliega: 'arriba',
                                                             ...
                                                             nro_campo_tipo: 1,
                                                             filtroXML: nvFW.pageContents.filtroPermisoDetalle,
