...
<%
    ...
    Dim Err As tError
    Dim oXMLLog As System.Xml.XmlDocument = Nothing
    ...
    Select Case accion.ToLower()
        ...
        Case "getbpmfile"
            ...
            Dim id_transferencia As Integer = nvUtiles.obtenerValorType("id_transferencia", 1021, nvConvertUtiles.DataTypes.int)
            ...
            Else
                Select Case accion.ToLower()
                    ...
                    Case "getterror"
                        ...
                        '------------------------------------------------------
                        '
                        ' NUEVA FUNCIONALIDAD -- ENCRIPTACION
                        '
                        '------------------------------------------------------
                        ' Si tenemos presente una clave pública del 
                        ' destinatario, la usamos para encriptar los parametros
                        ' del tError. De otro modo, el comportamiento es el de
                        ' siempre.
                        '------------------------------------------------------
                        'Dim destPublicKey As String = nvUtiles.obtenerValor("destPublicKey", "")

                        'If destPublicKey <> "" Then
                        '    Dim json As String = errRes.params.toJSON(nvConvertUtiles.nvJS_format.js_json)
                        '    Dim json_bytes As Byte() = System.Text.Encoding.UTF8.GetBytes(json)
                        '    Dim jsonRes_enc As nvFW.nvIDS.tnvDeviceEncData = nvFW.nvIDS.nvDevice.EncRsaAes256(destPublicKey, json_bytes)
                        '    ' Limpio los params originales
                        '    errRes.params.Clear()
                        '    ' Asigno la clave simetrica y los datos encriptados
                        '    errRes.params("simKey") = jsonRes_enc.simKeyB64
                        '    errRes.params("DeviceDataResEnc") = jsonRes_enc.encDataB64
                        'End If
