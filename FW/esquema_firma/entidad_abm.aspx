...
<%
    ...
    If modo.ToUpper() = "M" Then
        ...
        Try
            ...
            Dim rs As ADODB.Recordset = nvDBUtiles.DBOpenRecordset("SELECT top 1 * FROM entidad_funciones_rel WHERE origen=" & nro_entidad & " and destino=" & nro_funcion_entidad_destino & " and nro_funcion=" & nro_funcion)
            ...
            If Not exists Then
                nvFW.nvDBUtiles.DBExecute("Insert into entidad_funciones_rel(origen,destino,nro_funcion,activo) values('" + nro_entidad + "','" + nro_funcion_entidad_destino + "','" + nro_funcion + "','" + "1" + "')")
            Else
                Err.numError = -1
                Err.mensaje = "La funcion ya exitse"
            End If
    ...
    ElseIf modo.ToUpper() = "ELIMINAREF" Then 'modo entidad funcion
        
        Dim Err As New nvFW.tError()
        Dim nro_funcion_entidad_destino As String = nvFW.nvUtiles.obtenerValor("nro_funcion_entidad_destino", "")
        Try

            Dim rs As ADODB.Recordset = nvFW.nvDBUtiles.DBExecute("Delete from entidad_funciones_rel where origen = '" + nro_entidad + "' and destino = '" + nro_funcion_entidad_destino + "'")
            'rs = nvFW.nvDBUtiles.DBExecute("Insert into entidad_funciones_rel(origen,destino,nro_funcion,activo) values('" + nro_funcion_entidad_destino + "','" + nro_entidad + "','" + nro_funcion_inversa + "','" + "1" + "')")
        Catch ex As Exception

            Err.parse_error_script(ex)
            Err.titulo = "Error al eliminar la relacion entidad funcion"
            Err.mensaje = "No se pudo relizar el guardado." & vbCrLf & Err.mensaje
            Err.debug_src = "rol_abm.aspx"
        End Try
    ...
    Me.contents("filtroEntidadFunciones2") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='Entidad_funciones'><campos>distinct nro_funcion as id, funcion as [campo] </campos><orden>[id]</orden><filtro><aplica_pj type='igual'>'1'</aplica_pj></filtro></select></criterio>")
    Me.contents("filtroEntidadFunciones") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='Entidad_funciones'><campos>distinct nro_funcion as id, funcion as [campo] </campos><orden>[id]</orden><filtro><aplica_pf type='igual'>'1'</aplica_pf></filtro></select></criterio>")

    Me.contents("filtroCargarEntidadRelaciones") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verEntidadFuncionesRel'><campos>*</campos><orden></orden><filtro></filtro></select></criterio>")
    ...
    <script type="text/javascript">
        ...
        function window_onload()
        {
            ...
            if ($nro_entidad.value != '') {
                ...
                entidad_nueva()
                $modo.value = 'A'
                $('tabla_agregar_entidad_funciones').hide()
                $('divComentarioParametros').hide()
            }
        ...
        function window_onresize()
        {
            ...
            //var divComentariosHeight = $("divComentarios").getHeight()

            $("divComentarioParametros").setStyle({ 'height': (bodyHeight - divContentHeight) + "px" })
...
//            catch (e) {}
           ...
           // if ($('localidad').value == "")
            ...
            nvFW.error_ajax_request('entidad_abm.aspx', {
                ...
                onSuccess: function (err, transport) {
                    ...
                    $('tabla_agregar_entidad_funciones').show()
                    $('divComentarioParametros').show()
        ...
        function tabla_entidad_funcion_cargar() {
            ...
            if ($('nro_entidad').value != '')
                tabla_entidad_funciones.filtroWhere = "<origen type='igual'>" + $('nro_entidad').value + "</origen>";
            else
                tabla_entidad_funciones.filtroWhere = "<origen type='igual'>-1</origen>";
            ...
            tabla_entidad_funciones.eliminar_fila = function (num) {
                eliminar_relacion(tabla_entidad_funciones.data[num]["destino"]);
            }
        ...
        function entidad_funcion_guardar() {
            ...
            nvFW.error_ajax_request('entidad_abm.aspx', {
                ...
                onSuccess: function (err) {
                    if (err.numError == 0) {
                        tabla_entidad_funciones.refresh("<origen>" + $('nro_entidad').value + "</origen>");
                    }
                }
            });
        }

        function eliminar_relacion(destino) {
            nvFW.error_ajax_request('entidad_abm.aspx', {
                parameters: {
                    modo: "ELIMINAREF",
                    nro_entidad: $('nro_entidad').value,
                    nro_funcion_entidad_destino: destino
                },
                onSuccess: function (err) {
                    if (err.numError == 0) {
                        tabla_entidad_funciones.refresh("<origen>" + $('nro_entidad').value + "</origen>");
        ...
        function nuevo_esquema() {
            ...
            win =
                window.top.nvFW.createWindow({
                    className: 'alphacube',
                    url: '/fw/esquema_firma/pki_esquemas_de_firmas.aspx?nro_entidad=' + nro_entidad,
...
<body onload="return window_onload()" onresize="window_onresize()" style="width: 100%; height: 100%; overflow: hidden;">
    <form action="entidad_abm.aspx" method="post" name="form1" target="frmEnviar">
                    ...
                    <div id="div_funcion_fisica">
                        <script type="text/javascript">
                            campos_defs.add("funcion_fisica",
                                {
                                    filtroXML: nvFW.pageContents.filtroEntidadFunciones,
                                    nro_campo_tipo: 1,
                                    enDB: false
                                })
                            $("div_funcion_fisica").hide()
                    ...
                    <div id="div_funcion_juridica">
                        <script type="text/javascript">
                            campos_defs.add("funcion_juridica",
                                {
                                    filtroXML: nvFW.pageContents.filtroEntidadFunciones2,
                                    nro_campo_tipo: 1,
                                    enDB: false
                                })
                        </script> 
        ...
        <div id="divComentarioParametros" style="width: 100%; overflow:hidden">
            <iframe name="frame_comentarios" id="frame_comentarios" style="width: 100%; height: 100%; border: none;"></iframe>
        </div>
