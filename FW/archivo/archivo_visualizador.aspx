...
<%
    ...
    Dim nro_archivo_id_tipo = nvUtiles.obtenerValor("nro_archivo_id_tipo", "")
    Dim nro_def_archivo = nvUtiles.obtenerValor("nro_def_archivo", "")
    Dim nro_archivo = nvUtiles.obtenerValor("nroarchivo", "")
    Dim parametros = nvUtiles.obtenerValor("parametros", "")
    Dim etiquetas = nvUtiles.obtenerValor("etiquetas", "")
    Dim valores = nvUtiles.obtenerValor("valores", "")
    Dim valoresdesc = nvUtiles.obtenerValor("valoresdesc", "")
    Dim valoresdesc_antes = nvUtiles.obtenerValor("valoresdesc_antes", "")
    Dim ventana = nvUtiles.obtenerValor("ventana", "")
    ...
    Me.contents("verArchivos_parametros") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verArchivos_parametros'><campos>campo_def,parametro_valor,etiqueta</campos><grupo></grupo><filtro></filtro></select></criterio>")
    Me.contents("filtro_verArchivos") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verArchivos_idtipo'><campos>nro_archivo,archivo_descripcion as Descripcion,f_id,path</campos><filtro><nro_archivo_id_tipo type='igual'>" & nro_archivo_id_tipo & "</nro_archivo_id_tipo><id_tipo type='igual'>" & id_tipo & "</id_tipo><nro_archivo_estado type='igual'>1</nro_archivo_estado></filtro><orden>orden</orden></select></criterio>")
    ...
    If (modo = "G") Then
        ...
        Try
            ...
            For i As Integer = 0 To param.length
                ...
                Dim Rs = nvdbutiles.DBOpenRecordset(StrSQL_permisos)
...
<head>
    ...
    <script type="text/javascript">
        ...
        function cargar_archivos() {
            ...
            var filtro_nro_archivo = ""
            if ($('nro_archivo').value > 0)
                filtro_nro_archivo = "<nro_archivo type='igual'>" +$('nro_archivo').value + "</nro_archivo>"
            ...
            rs.open(nvFW.pageContents.filtro_verArchivos, "", filtro_nro_archivo )
            var i = 0
            while (!rs.eof()) {
                archivos.push({ nro_archivo: rs.getdata('nro_archivo'), path: rs.getdata('path'), f_id: rs.getdata('f_id')  })
        ...
        function cargar_parametros() {
            ...
            rs.open(nvFW.pageContents.verArchivos_parametros, "", "<id_tipo type='igual'>" + id_tipo + "</id_tipo><nro_param_grupo type='igual'>11</nro_param_grupo>")
            var i = 0
        ...
        function guardar_parametros() {
            ...
            nvFW.error_ajax_request('archivo_visualizador.aspx', {
        ...
        //        if ($('chk' + i).checked) {
        //            id = 'frame' + i
        //            $(id).src = '/fw/archivo/get_file.aspx?nro_archivo=' + archivos[i]
        //            //recalcula(i)
        //        }
        //    }
        //}
        ...
        function cargar_frame() {
            ...
            $('contenido').src = "/fw/files/file_get.aspx?path=" + archivos[lista_archivos_sel[archivo_actual]].path + "&f_id=" + archivos[lista_archivos_sel[archivo_actual]].f_id
...
<body onload="return window_onload()" style="width: 100%; height: 100%; overflow: auto">
    ...
    <input type="hidden" name="id_tipo" id="nro_archivo" value="<%=nro_archivo %>" />
    <table style="width: 100%; height: 100%" class="tb1">
        <tr style="height: 100%">
            ...
            <td style="vertical-align: top; width: 200px; height: 100%">
                ...
                <div style="width: 100%;display:none" id="divGuardar">      
                </div>
