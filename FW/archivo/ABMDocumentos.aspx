...
<%
    ...
    Dim nro_archivo_id_tipo As String = nvFW.nvUtiles.obtenerValor("nro_archivo_id_tipo", "")
    ...
    Dim salida_tipo As String = nvFW.nvUtiles.obtenerValor("salida_tipo", "estado")
    Dim isFisical As String = nvFW.nvUtiles.obtenerValor("isFisical", "0")

    Dim err As New nvFW.tError()
    err.salida_tipo = salida_tipo
    '*** nvCLiente *****/
    Dim xmlError As String = obtenerValor("xmlError", "0")
    Dim nro_img_origen As Integer = obtenerValor("nro_img_origen", enumOrigen.browser) 'por defecto viene del nova
    'Dim cant_hojas As Integer = obtenerValor("cant_hojas", 0)

    '***  nuevos parametros agregados a partir de la version de nvCLient 2.0****'
    Dim xmlInfo As String = obtenerValor("xmlInfo", "")
    Dim version As String = obtenerValor("version", "")
    Dim ComputerName As String = obtenerValor("ComputerName", "")

    If (modo = "http_post") Then
        Dim archivos As New Dictionary(Of Integer, tnvArchivo)
        Try
            ...
            If (accion = "ABM") Then
                ...
                If isFisical = 1 Then
                    ...
                    archivo = New tnvArchivo(id_tipo:=id_tipo, nro_archivo_id_tipo:=nro_archivo_id_tipo _
            , nro_def_archivo:=nro_def_archivo, nro_def_detalle:=nro_def_detalle, descripcion:=descripcion, xmlInfo:=xmlInfo _
            , xmlError:=xmlError, isFisical:=True)

                    archivo.save()

                    archivos_string = descripcion
                ...
                Else
                    ...
                    For Each campo In Request.Files.AllKeys
                        ...
                        Dim archivo As tnvArchivo
                        descripcion = descripciones.ToString().Split(",")(contador)
                        orden = ordenes.ToString().Split(",")(contador)
                        ...
                        If Request.Files(campo).FileName <> "" Then
                            ...
                            Dim ext As String = System.IO.Path.GetExtension(Request.Files(campo).FileName).ToLower
                            Dim size As String = Request.Files(campo).ContentLength
                            nro_def_detalle = nvFW.nvUtiles.obtenerValor("nro_def_detalle_" + orden, "")
                            nro_def_archivo = nvFW.nvUtiles.obtenerValor("nro_def_archivo_" + orden, "")

                            archivo = New tnvArchivo(BinaryData:=binaryData, id_tipo:=id_tipo, nro_archivo_id_tipo:=nro_archivo_id_tipo _
                                , nro_def_archivo:=nro_def_archivo, nro_def_detalle:=nro_def_detalle, descripcion:=descripcion, file_ext:=ext _
                                , nro_img_origen:=nro_img_origen, xmlInfo:=xmlInfo, xmlError:=xmlError, version:=version, ComputerName:=ComputerName, isFisical:=False)

                            archivo.save()
                            'If err.numError <> 0 Then
                            ' err.mostrar_error()
                            'End If
                            archivos.Add(contador, archivo)

                            archivos_string += archivo.filename.ToString().Replace(",", "") + ","
                            ids = ids & archivo.nro_archivo & ","

                        Else
                            archivo = New tnvArchivo
                            archivos.Add(contador, archivo)
                        End If
        ...
        Dim error_mensaje As String = ""
        Dim index As Integer
        For Each index In archivos.Keys
            strError = archivos(index).archivo_error.get_error_xml()
            Dim objError_det As New System.Xml.XmlDocument
            objError_det.LoadXml(strError)
            ...
            node_params.AppendChild(node_clon)
            If archivos(index).archivo_error.numError <> 0 Then
                error_count += 1
                error_mensaje = objError_det.SelectSingleNode("error_mensajes/error_mensaje/mensaje").InnerText & "<br>"
            End If
        Next

        Dim att As System.Xml.XmlAttribute = oXML.CreateAttribute("error_count")
        att.Value = error_count
        node_params.Attributes.Append(att)
        ...
        oXML.SelectSingleNode("error_mensajes/error_mensaje/@numError").Value = error_count
        oXML.SelectSingleNode("error_mensajes/error_mensaje/mensaje").InnerText = error_mensaje
    ...
    Me.contents("nro_archivo_id_tipo") = nro_archivo_id_tipo
    Me.contents("id_tipo") = id_tipo
    Me.contents("archivos_def") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='archivos_def_detalle'><campos>*</campos><orden>orden</orden><filtro><nro_def_archivo type='igual'>" & nro_def_archivo & "</nro_def_archivo>" & filtro_nro_def_detalle & "</filtro></select></criterio>")
...
mixpanel.init("1f3bfca051bd3bc6d01e636aace64c87");</script><!-- end Mixpanel -->
    ...
    <script type="text/javascript" language="javascript">
        ...
        var nro_archivo_id_tipo = nvFW.pageContents.nro_archivo_id_tipo;
            ...
            rs.onComplete = function (rs) {
                ...
                while (!rs.eof()) {
                    ...
                    if (rs.recordcount > 1) {
                        ...
                        $('divGuardar').setStyle({ width: '100%' })
                    }
                    else 
                        $('tdcheckbox').show()

                    var orden = rs.getdata('orden')
                    var file_filtro = rs.getdata('file_filtro')
                    var file_max_size = rs.getdata('file_max_size')
                    ...
                    tabla_archivos += '<tr><td '+ style_width +'> <input  style="width:99%" type="text"  id="text_' + rs.getdata("orden") + '" ondblclick="file_seleccionar(\'' + rs.getdata('archivo_descripcion') + '\')"';
                    ...
                    tabla_archivos += '<td><input type="file" orden="' + rs.getdata("orden") + '" archivo_descripcion="' + rs.getdata("archivo_descripcion") + '" style="width:99%;display:none" id="archivo_' + rs.getdata("archivo_descripcion") + '" name="archivo_' + rs.getdata("archivo_descripcion") + '" onchange="file_onchange(\'' + rs.getdata('archivo_descripcion') + '\',' + nro_orden + ',\''+ file_filtro + '\',\'' + file_max_size + '\')" ';
        ...
        function submit_archivos(isFisical) {
            ...
            oXML.onComplete = function () {
                ...
                if (res.loadXML(this)) {
                    ...
                    if (numError > 0) {
                        alert(XMLText(selectSingleNode('error_mensajes/error_mensaje/mensaje', res.xml)))
        ...
        function guardar_fisico() {
            ...
            submit_archivos(true)
             ...
             /* confirm("¿Desea guardar el documento como físico?", {
                                width: 400,
                                height: "auto",
                                className: "alphacube",
                                okLabel: "Si",
...
<body onload='return window_onload()' onkeypress='return key_Enter()' onresize="return window_onresize()"  style='width:100%;height:100%;overflow: hidden'>
    ...
    <div id="divButton" style="width:100%">
         <table class="tb1">
             <tr>
                 <td class="Tit4" style="vertical-align:middle;width:30%;display:none" id="tdcheckbox"><div style="display:inline-block;vertical-align:middle"><input type="checkbox" style="vertical-align:bottom;border:0px !important ;padding:0px !important ;margin:0px !important " onclick="return onchange_cambiar(event)"/></div><div style="display:inline-block;margin-left:5px;vertical-align:middle;">&#32;&#32; Marcar como <b>no digitalizado</b></div></td> 
                 <td><div id="divGuardarFisico" style="display:none;width:100%"></div><div id="divGuardar" style="display:inline;width:100%"></div></td> 
             </tr>
        </table>
    </div>
