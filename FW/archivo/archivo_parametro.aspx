...
function parseFecha(strFecha)
 ...
 function window_onload() {
  ...
  var rs = new tRS();
  rs.open(strCriterio, "", "<NOT><etiqueta type='isnull'/></NOT><nro_archivo_estado type='igual'>1</nro_archivo_estado><nro_def_detalle type='igual'>" + nro_def_detalle + "</nro_def_detalle><nro_archivo_id_tipo type='igual'>" + nro_archivo_id_tipo + "</nro_archivo_id_tipo><id_tipo type='igual'>" + id_tipo + "</id_tipo>","","")
  var strHTML = '<table class="tb1 highlightOdd highlightTROver layout_fixe" style="width:100%"><tr class="tbLabel0"><td>Par&aacute;metro</td><td>Valor</td></tr>'
  while (!rs.eof())
    {  
       strHTML += '<tr><td title="'+ rs.getdata('etiqueta') +'"><strong>'+ rs.getdata('etiqueta') +'</strong></td><td>'+rs.getdata('parametro_valor')+'</td></tr>'
       rs.movenext()
    }  
   strHTML += '</table>'
...
function actualizar_start() {
    winActualizar = Dialog.info('Actualizando', { className: "alphacube", width: 250,
