...
<%
    ...
    'Me.contents("filtroPerfiles") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verArchivos_def_perfil' PageSize='14' AbsolutePage='1' cacheControl='Session'><campos>*</campos><orden>id</orden><filtro></filtro></select></criterio>")
    Me.contents("filtroPerfiles") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verArchivos_def_perfil' ><campos>*</campos><orden>id</orden><filtro></filtro></select></criterio>")
    Me.contents("filtroDetalle") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='archivos_def_detalle'><campos>COUNT(*) as cant_detalles</campos><filtro><perfil type='igual'>%id%</perfil></filtro></select></criterio>")
%>
...
<head>
    ...
    <script type="text/javascript">
        ...
        var filtroPerfiles = nvFW.pageContents.filtroPerfiles
    ...
    function buscar_archivos_def_perfil()
    {
        ...
        var desc_def_perfil = campos_defs.get_value('desc_def_perfil')
        var filtro = ''

        if (nro_def_perfil != '') {
            filtro = "<id type='igual'>" + nro_def_perfil + "</id>"
        } else if (desc_def_perfil != '') {
            filtro = "<archivo_def_perfil type='like'>%" + desc_def_perfil + "%</archivo_def_perfil>"
        }
...
<table class="tb1" id="tb_definicion_perfil_archivos">
     <tr>
       <td style="width:80%">
          <table class="tb1">
            <tr class="tbLabel">
            ...
            <tr>                
                <td style='width:20%; padding:0;'><%= nvFW.nvCampo_def.get_html_input("nro_def_perfil", enDB:=False, nro_campo_tipo:=100)%></td>
                <script type="text/javascript">
                    $('nro_def_perfil').addEventListener('keydown', function (e) {
                        if (e.key === 'Enter') {
                           buscar_archivos_def_perfil()
                        }
                    })
                </script>
                <td style='width:80%; padding:0;'><%= nvFW.nvCampo_def.get_html_input("desc_def_perfil", enDB:=False, nro_campo_tipo:=104)%></td>
                <script type="text/javascript">
                    $('desc_def_perfil').addEventListener('keydown', function (e) {
                        if (e.key === 'Enter') {
                           buscar_archivos_def_perfil()
                        }
                    })
...
<iframe name="iframe_definicion_perfil_archivos" id="iframe_definicion_perfil_archivos" style='width: 100%; height: 100%; overflow: auto; border:none; max-height:700px' frameborder="0" src="/fw/enBlanco.htm"></iframe>
</form>
</body>
</html>