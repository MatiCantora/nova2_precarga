...
<head>
    ...
    var paramsGlob = []
        ...
        vButtonItems[0]["nombre"] = "Editar";
        vButtonItems[0]["etiqueta"] = "Editar";
        vButtonItems[0]["imagen"] = "editar";
        vButtonItems[0]["onclick"] = "return editarFiltros('RPT')";

        var vListButton = new tListButton(vButtonItems, 'vListButton')
        vListButton.loadImage("editar", "/fw/image/icons/editar.png")

    function editarFiltros() {
        var w = window.nvFW != undefined ? window.nvFW : nvFW
        win_archivos_def_detalle_abm = w.createWindow({
            className: 'alphacube',
            url: '/fw/def_archivos/archivos_def_det_filt_up.aspx' + '?idParam=' + $('file_filtro').value,
            title: '<b>Modificar filtros</b>',
            minimizable: true,
            maximizable: false,
            draggable: true,
            resizable: true,
 ...
 var f = 0
    ...
    function actualizar_archivos_def_detalle() 
    {
        ...
        if (!params) {
            ...
            paramsGlob.each(function (arreglo, i) {
                ...
                if (arreglo['nro_def'] == $('nro_def_detalle').value) {
                    paramsGlob[c] = ''
                }
                c += 1
            })
            ...
            params.each(function (arr, i) {
                ...
                parametro = new tParam_def({
                    parametro: arr.parametro,
                    tipo_dato: arr.tipo_dato,
                    etiqueta: arr.etiqueta,
                    requerido: arr.requerido,
                    editable: arr.editable,
                    visible: arr.visible,
                    valor_defecto: arr.valor_defecto
                })
                ...
                var parObj = {
                    ...
                    'visible': arr.visible,
                    'valor_defecto': arr.valor_defecto,
                    'nro_def': $('nro_def_detalle').value
                }



                paramsGlob.push(parObj)  
            });
            strXML += "</param_def>"
            ...
            paramsXML.push(paramsABM)
            sessionStorage.setItem("paramsXML", JSON.stringify(paramsXML))
            sessionStorage.setItem("paramsGlob", JSON.stringify(paramsGlob))
        }
    }
   ...
   ///////////////////////////////////
    function mostrar_param(params) {
        var flag = 0
        paramsGlob = JSON.parse(sessionStorage.getItem("paramsGlob"))
        if (paramsGlob != '') {
            var strHTMLparams = '<table class="tb1 highlightOdd highlightTROver layout_fixe" style="width:100%"><tr></tr>'
            paramsGlob.each(function (arreglo, i) {
                if (arreglo['nro_def'] == $('nro_def_detalle').value) {
                    strHTMLparams += '<tr><td style="width:40%">' + arreglo['parametro'] + '</td>'
                    strHTMLparams += '<td style="width:60%">' + arreglo['etiqueta'] + '</td></tr>'
                    flag = 1
                }
            })
            strHTMLparams += '</table>'
        ...
        function saveFilters(valor) {
            $('file_filtro').value = valor
        }
...
<form name="frmArchivosDefDetalleABM" action="/fw/def_archivos/archivos_def_detalle_ABM.aspx" method="post" style='width:100%;height:100%;overflow:hidden'>
    ...
    <table class="tb1" id="tb_archivos_def_detalle3">
        ...
        <tr>
            <td style="vertical-align:middle; text-align:left"><input readonly style="width:84%;margin-right:1%" type="text" name="file_filtro" id="file_filtro" value="" placeholder="Ej. .pdf | .html"/><div style="width:15%; display:inline-block; height:17px" id="divEditar"></div></td>
            <td style="vertical-align:middle; text-align:center">
                    <script type="text/javascript">
    ...
    <script type="text/javascript">
        ...
        Menus["vMenuABMParams"].CargarMenuItemXML("<MenuItem id='0' style='WIDTH: 100%'><Lib TipoLib='offLine'>DocMNG</Lib><icono></icono><Desc>Parámetros</Desc></MenuItem>")
    ...
    <div id="parametros_detalle" style="width:100%;overflow-y:auto;max-height:100px"></div>
</form>
</body>
</html>
