...
<%
    ...
    'Me.contents("filtroTipos") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='archivos_def_tipo' PageSize='14' AbsolutePage='1' cacheControl='Session'><campos>*</campos><orden>nro_archivo_def_tipo</orden><filtro></filtro></select></criterio>")
    Me.contents("filtroTipos") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='archivos_def_tipo'><campos>*</campos><orden>nro_archivo_def_tipo</orden><filtro></filtro></select></criterio>")
    Me.contents("filtroDetalle") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='archivos_def_detalle'><campos>COUNT(*) as cant_detalles</campos><filtro><nro_archivo_def_tipo type='igual'>%nro_archivo_def_tipo%</nro_archivo_def_tipo></filtro></select></criterio>")
%>
...
<head>
    ...
    <script type="text/javascript">
        ...
        var filtroTipos = nvFW.pageContents.filtroTipos
    ...
    {
        var desc_archivo_def_tipo = campos_defs.get_value('desc_def_tipos3')
        ...
        } else if (desc_archivo_def_tipo != ''){
            filtro = "<archivo_def_tipo type='like'>%" + desc_archivo_def_tipo + "%</archivo_def_tipo>"
        }
...
<table class="tb1" style="width:100%" id="tb_definicion_tipo_archivos">
     <tr>
       <td style="width:80%">
          <table class="tb1" style="width:100%">
               ...
               <tr> 
                    <td style='width:20%; padding:0;'><%= nvFW.nvCampo_def.get_html_input("nro_def_tipos3", enDB:=False, nro_campo_tipo:=100)%></td>
                    <script type="text/javascript">
                    $('nro_def_tipos3').addEventListener('keydown', function (e) {
                        if (e.key === 'Enter') {
                        buscar_archivos_def_tipo()
                        }
                    })
                   </script>
                   <td style='width:80%; padding:0;'><%= nvFW.nvCampo_def.get_html_input("desc_def_tipos3", enDB:=False, nro_campo_tipo:=104)%></td>
                   <script type="text/javascript">
                     $('desc_def_tipos3').addEventListener('keydown', function (e) {
                        if (e.key === 'Enter') {
                        buscar_archivos_def_tipo()
                        }
                     })
...
<iframe name="iframe_definicion_tipo_archivos" id="iframe_definicion_tipo_archivos" style='width: 100%; height: 100%; overflow: auto; border:none; max-height:700px;' frameborder="0" src="/fw/enBlanco.htm"></iframe>
</form>
</body>
</html>