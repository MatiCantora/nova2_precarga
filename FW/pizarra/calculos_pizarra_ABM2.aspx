...
<head>
    ...
    <script type="text/javascript">
                ...
                if (Number(fila["id_calc_piz_det"]) > -1) {
                    ...
                    while (contador > 0) {
                        ...
                        if (nro_campo_tipo != 105) {
                            ...
                            if (fila["dato" + j + "_desde"] != undefined && fila["dato" + j + "_desde"] != '')
                                evaluar.push("campos_defs.set_value('" + name_desde + "', '" + fila["dato" + j + "_desde"] + "', true);")
                            ...
                            if (habilitar_hasta) { 
                                ...
                                if (fila["dato" + j + "_hasta"] != undefined && fila["dato" + j + "_hasta"] != '')
                                    evaluar.push("campos_defs.set_value('" + name_hasta + "', '" + fila["dato" + j + "_hasta"] + "', true);")
                    ...
                    if (campo_def_valor != "") {
                        ...
                        if (fila['valor_1'] != undefined && fila['valor_1'] != '')
                            evaluar.push("campos_defs.set_value('valor_1_" + i + "', '" + fila["valor_1"] + "');")
                        ...
                        for (var ii = index_inicio; ii < PizarraExtra["valores"].length + index_inicio; ii++) {
                            ...
                            if (extra["campo_def"] != "") {
                                ...
                                if (fila['valor_' + ii] != undefined && fila['valor_' + ii] != '')
                                    evaluar.push("campos_defs.set_value('valor_" + ii + "_" + i + "', '" + fila['valor_' + ii] + "');")
                        ...
                        if (valores_campos_filtro.length) {
                            for (var i = 1, max = PizarraDef.length; i <= max; i++)
                                if (valores_campos_filtro[i - 1] != undefined && valores_campos_filtro[i - 1] != '')
                                    campos_defs.set_value("filtro_dato_" + i, valores_campos_filtro[i - 1])
        ...
        function pizarra_cubo_crear()
        {
            ...
            if (datos.length == 0) {
                alert('<br/>El/los dato/s no están definidos como <b>campos defs</b> o tienen seteado el item <b>"tiene hasta"</b>.<br/><br/>Para proseguir debe seleccionar un campo def o bien deseleccione el item "tiene hasta".', {
            ...
            if (!isNaN(total) && total > 0) {
                try {
                    ...
                    if (html != '') {
                        ...
                        $div_pizarra_det.select('table tbody')[0].childElements().each(function (tr, pos) {
	                        if (pos > 2) tr.remove()
                        })
                        try {
