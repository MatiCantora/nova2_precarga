...
<%
        ...
        Select Case modo.ToLower()
            ...
            Case "cubo_get_combinaciones"
                ...
                If str_campos_defs <> "" Then
                    ...
                    For Each cd_dato As String In campos_defs
                        strSQL = "SELECT filtroXML FROM campos_def WHERE campo_def='" & cd_dato & "'"

                        Try
                            rs = nvFW.nvDBUtiles.DBExecute(strSQL)

                            If Not rs.EOF Then
                                Array.Resize(filtros_XML, filtros_XML.Length + 1)
                                filtros_XML(filtros_XML.Length - 1) = rs.Fields("filtroXML").Value.ToString.Replace("""", "")
                            End If
                        ...
                        Catch ex As Exception
                            ...
                            err.parse_error_script(ex)
                            err.numError = 105
                            err.mensaje = "Error al recuperar los valores de 'filtroXML' de datos"
                    ...
                    Dim nombres_tablas As String = ""
                    Dim nombres_columnas_id As String = ""
                    Dim aux As String = ""
                    Dim ini As Integer = 0
                    Dim fin As Integer = 0

                    If filtros_XML.Length > 0 Then
                        For Each filtroXML As String In filtros_XML
                            aux = filtroXML.ToLower
                            ini = aux.IndexOf("vista='") + "vista='".Length
                            fin = aux.IndexOf("'>") - ini
                            nombres_tablas &= aux.Substring(ini, fin).Trim() & ","

                            ini = aux.IndexOf("<campos>") + "<campos>".Length
                            fin = aux.IndexOf(",") - ini
                            nombres_columnas_id &= aux.Substring(ini, fin).Replace("distinct ", "").Replace(" as id", "").Trim() & ","
                        Next

                        ' Quitar comas (',') al final
                        nombres_tablas = nombres_tablas.Remove(nombres_tablas.Length - 1, 1)
                        nombres_columnas_id = nombres_columnas_id.Remove(nombres_columnas_id.Length - 1, 1)
                    End If

                    If nombres_columnas_id <> "" AndAlso nombres_tablas <> "" Then
                        Dim sql_cross_join As String = "SELECT " & nombres_columnas_id & " FROM " & nombres_tablas & " ORDER BY " & nombres_columnas_id
                        Dim cols() As String = nombres_columnas_id.Split(",")
    ...
    '-----------------------------------------------------------
    Me.contents("filtroPizarra") = nvXMLSQL.encXMLSQL("<criterio><select vista='calc_pizarra_cab'><campos>*</campos></select></criterio>")
    ...
    If nro_calc_pizarra <> 0 Then
        Dim strSQL As String = "SELECT TOP 1 b.permiso_grupo, c.permiso AS valor, a.nro_permiso_ver, a.nro_permiso_editar, a.calc_pizarra " _
            ...
            & " WHERE c.operador = dbo.rm_nro_operador() AND a.nro_calc_pizarra = " & nro_calc_pizarra
        Dim rs As ADODB.Recordset = nvDBUtiles.DBExecute(strSQL)
...
<head>
    ...
    <script type="text/javascript">
        /*----------------------------------------
        | Cambia el estado de un input hacia
    ...
    <script type="text/javascript">
        ...
        function pizarra_datos_cargar()
        {
            ...
            if (Pizarra['tipo_dato'] == "bit")
                tipoDatoOnchange("tipo_dato", "bit")
        ...
        function generarXML_pizarra_extras()
        {
            if (PizarraExtra["valores"].length > 0) {
                ...
                PizarraExtra["valores"].forEach(function(salida) {
        ...
        function tipo_dato_def_seleccionar()
        {
            PizarraDef.each(function (arreglo, i) {
                if (arreglo["tipo_dato_def"] != '') {
                    $('tipo_dato_def_' + i).value = arreglo["tipo_dato_def"]

                    if (arreglo["tipo_dato_def"] == "bit")
                        tipoDatoOnchange('tipo_dato_def_' + i, "bit")
                }
            });
        }
        ...
        function pizarra_datos_det_cargar(filtros) {
            ...
            while (!rs.eof()) { 
                ...
                while (contador > 0) {
                    tiene_hasta = PizarraDef[j - 1].tiene_hasta == 1
        ...
        function pizarra_nuevo_dato()
        {
            ...
            if (indice >= datos_maximo) {
                alert('Se pueden agregar hasta ' + datos_maximo + ' datos.', { title: "<b>Información</b>", width: 300, height: 70 })
                return
            }

            var dato_orden = indice > 0
                            ? parseInt(PizarraDef[indice - 1]['dato_orden']) + 1
                            : 1
        ...
        function pizarra_nuevo_dato_detalle()
        {
            ...
            PizarraDet.each(function (arreglo, i) {
                ...
                while (contador > 0) {
                    ...
                    PizarraDet[i]['dato' + j + '_desde'] = j <= cant_datos_ant 
                                                            ? $('dato' + j + '_desde_' + i).value 
                                                            : ''
                    
                    if (PizarraDef[j - 1].tiene_hasta == 1)
                        PizarraDet[i]['dato' + j + '_hasta'] = j <= cant_datos_ant 
                                                                ? $('dato' + j + '_hasta_' + i).value 
                                                                : ''
        ...
        var cantidad_filas_det = 0


        function getCampo_def_options(campo_def, target, nro_campo_tipo, posicion_fila)
        {
            var options   = {}
            ...
            if (nro_campo_tipo == 0 || nro_campo_tipo == 2) {
                ...
                var nombre_cdef_custom = target.substr(3, target.length)
                ...
                if (campos_defs.items[campo_def] == undefined)
                    campos_defs.add(campo_def, { target: "return_html" }) // con este target creo el campo_def y evito un "document.write"
        ...
        function pizarra_det_dibujar()
        {
            ...
            cantidad_filas_det = 0
        
            $div_pizarra_det.innerHTML = ""     // limpia el HTML del contenedor de detalles

            strHTML = "<table class='tb1' style='table-layout: fixed;'>"
            ...
            for (var i = 1; i <= cant_datos; i++) {
                ...
                if (campo_def != "") {
                    eval_filtro.push("campos_defs.add('" + id_input + "', getCampo_def_options('" + campo_def + "', 'td_" + id_input + "', 2, 0));")
                }
                else {
                    var nro_campo_tipo = +arTipoDato[tipo_dato].nro_campo_tipo

                    if (nro_campo_tipo != 105) {
                        eval_filtro.push("campos_defs.add('" + id_input + "', getCampo_def_options('', 'td_" + id_input + "', " + nro_campo_tipo + ", 0));")
                    }
                    else {
            ...
            strHTML +=         "<input type='button' id='btnFiltrar' name='btnFiltrar' onclick='pizarra_filtrarDatos()' title='Aplicar filtro' value='Filtrar' style='width: 100%;' />"
            strHTML +=       "</td>"
            strHTML +=       "<td style='min-width: 45px; max-width: 45px;'>"
            strHTML +=         "<input type='button' id='btnResetear' name='btnResetear' onclick='pizarra_resetearFiltro()' title='Resetear filtro' value='' style='width: 100%;' />"
            strHTML +=       "</td>"
            strHTML +=     "</tr>"
            strHTML +=   "</table>"
            strHTML += "</td>"
            ...
            PizarraDet.each(function (arreglo, i) {
                ...
                if (arreglo["id_calc_piz_det"] >= 0) {
                    ...
                    strHTML += "<tr>"
                    strHTML += "<td>"
                    strHTML += "<input type='text' name='id_calc_piz_det_" + i + "' id='id_calc_piz_det_" + i + "' value='" + arreglo["id_calc_piz_det"] + "' style='width: 100%; text-align: center;' disabled />"
                    strHTML += "</td>"
                    ...
                    while (contador > 0) {
                        ...
                        if (campo_def == "")
                            nro_campo_tipo = +arTipoDato[tipo_dato_def].nro_campo_tipo
                        ...
                        arreglo["dato" + j + "_desde"] = ["money", "decimal"].indexOf(tipo_dato_def) != -1 && arreglo["dato" + j + "_desde"] != "" 
                                                            ? parseFloat(arreglo["dato" + j + "_desde"]).toFixed(2)
                                                            : arreglo["dato" + j + "_desde"]

                        if (nro_campo_tipo != 105) {
                            evaluar.push("campos_defs.add('" + name_desde + "', getCampo_def_options('" + campo_def + "', 'td_" + name_desde + "', " + nro_campo_tipo + ", " + i + "));")
                            
                            if (arreglo["dato" + j + "_desde"] != undefined && arreglo["dato" + j + "_desde"] != '')
                                evaluar.push("campos_defs.set_value('" + name_desde + "', '" + arreglo["dato" + j + "_desde"] + "', true);")
                            
                            evaluar.push("campos_defs.habilitar('" + name_desde + "', " + (es_cubo ? habilitar_dato : habilitar) + ");")
                            ...
                            if (habilitar_hasta) { 
                                ...
                                // Si tipo de dato es MONEY => parsear el valor a punto decimal
                                arreglo["dato" + j + "_hasta"] = ["money", "decimal"].indexOf(tipo_dato_def) != -1 && arreglo["dato" + j + "_desde"] != "" ? parseFloat(arreglo["dato" + j + "_hasta"]).toFixed(2).replace(",", ".") : arreglo["dato" + j + "_hasta"]
                            
                                evaluar.push("campos_defs.add('" + name_hasta + "', getCampo_def_options('" + campo_def + "', 'td_" + name_hasta + "', " + nro_campo_tipo + "));")

                                if (arreglo["dato" + j + "_hasta"] != undefined && arreglo["dato" + j + "_hasta"] != '')
                                    evaluar.push("campos_defs.set_value('" + name_hasta + "', '" + arreglo["dato" + j + "_hasta"] + "', true);")

                                evaluar.push("campos_defs.habilitar('" + name_hasta + "', " + (es_cubo ? habilitar_dato : habilitar) + ");")
                            }
                        }
                        else {
                            ...
                            evaluar.push("$('" + name_desde + "').checked = " + (arreglo["dato" + j + "_desde"] != undefined && arreglo["dato" + j + "_desde"] != "" ? (arreglo["dato" + j + "_desde"] == "0" ? "false" : "true") :  "false") + ";")
                            evaluar.push("cambiarEstado('" + name_desde + "', " + (es_cubo ? habilitar_dato : habilitar) + ");")
                        }
                    ...
                    arreglo["valor_1"] = ["money", "decimal"].indexOf(tipo_dato_valor) != -1 ? parseFloat(arreglo["valor_1"]).toFixed(2).replace(",", ".") : arreglo["valor_1"]
                    
                    nro_campo_tipo = +arTipoDato[Pizarra["tipo_dato"]].nro_campo_tipo

                    if (campo_def_valor != "") {
                        evaluar.push("campos_defs.add('valor_1_" + i + "', getCampo_def_options('" + campo_def_valor + "', 'td_valor_1_" + i + "', 0, " + i + "));")
                        
                        if (arreglo['valor_1'] != undefined && arreglo['valor_1'] != '')
                            evaluar.push("campos_defs.set_value('valor_1_" + i + "', '" + arreglo["valor_1"] + "');")

                        evaluar.push("campos_defs.habilitar('valor_1_" + i + "', " + habilitar + ");")
                    }
                    else {
                        // nro_campo_tipo == 105 => dibujar un checkbox
                        if (nro_campo_tipo != 105) {
                            evaluar.push("campos_defs.add('valor_1_" + i + "', getCampo_def_options('', 'td_valor_1_" + i + "', " + (+arTipoDato[tipo_dato_valor].nro_campo_tipo) + "));")
                            ...
                            evaluar.push("$('valor_1_" + i + "').checked = " + (arreglo["valor_1"] != undefined && arreglo["valor_1"] != "" ? (arreglo["valor_1"] == "0" ? "false" : "true") : "false") + ";")
                    ...
                    if (PizarraExtra != null && PizarraExtra["valores"] != null) {
                        ...
                        for (var ii = index_inicio; ii < PizarraExtra["valores"].length + index_inicio; ii++) {
                            ...
                            // Parsear tipo MONEY a punto decimal
                            arreglo["valor_" + ii] = ["money", "decimal"].indexOf(tipo_dato_valor) != -1 ? parseFloat(arreglo["valor_" + ii]).toFixed(2).replace(",", ".") : arreglo["valor_" + ii]
                            nro_campo_tipo         = parseInt(arTipoDato[tipo_dato_valor].nro_campo_tipo)   // actualizacion

                            strHTML += "<td>" + draw_pre_pos(extra["prefijo"], "td_valor_" + ii + "_" + i, extra["posfijo"]) + "</td>"

                            if (extra["campo_def"] != "") {
                                evaluar.push("campos_defs.add('valor_" + ii + "_" + i + "', getCampo_def_options('" + extra["campo_def"] + "', 'td_valor_" + ii + "_" + i + "', 0, " + i + "));")
                                
                                if (arreglo['valor_' + ii] != undefined && arreglo['valor_' + ii] != '')
                                    evaluar.push("campos_defs.set_value('valor_" + ii + "_" + i + "', '" + arreglo['valor_' + ii] + "');")
                                
                                evaluar.push("campos_defs.habilitar('valor_" + ii + "_" + i + "', " + habilitar + ");")
                            }
                            else {
                                if (nro_campo_tipo != 105) {
                                    evaluar.push("campos_defs.add('valor_" + ii + "_" + i + "', getCampo_def_options('', 'td_valor_" + ii + "_" + i + "', " + (+arTipoDato[tipo_dato_valor].nro_campo_tipo) + "));")
                                    ...
                                    evaluar.push("$('valor_" + ii + "_" + i + "').checked = " + (arreglo["valor_" + ii] != undefined && arreglo["valor_" + ii] != "" ? (arreglo["valor_" + ii] == "0" ? "false" : "true") : "false") + ";")
                    ...
                    if (!es_cubo)
                        strHTML += "<img alt='Eliminar' title='Eliminar Detalle' src='/FW/image/icons/eliminar.png' style='cursor: pointer;' onclick='pizarra_eliminar_detalle(" + i + ", " + habilitar + ")' />"
                    else
                        strHTML += "&nbsp;"
            ...
            if (eval_filtro.length > 0) {
                eval_filtro.forEach(function(item) {
                    eval(item)
                })

                if (eval_title.length) {
                    eval_title.forEach(function(item) {
                        eval(item)
                    })
                }
        ...
        function pizarra_det_dibujar_nueva_fila()
        {
            ...
            var containerDetalles  = $div_pizarra_det.select('table tbody')[0]
            // HTML de salida
            var strHTML            = "<tr>"
            // Variable para realizar evaluacion de codigo luego de insertar el HTML en el DOM
            var eval_cdef          = []
            ...
            if (arreglo["id_calc_piz_det"] == 0) {
                ...
                while (cant_datos > 0) {
                    ...
                    if (nro_campo_tipo != 105) {
                        eval_cdef.push("campos_defs.add('" + name_desde + "', getCampo_def_options('" + campo_def + "', 'td_" + name_desde + "', " + nro_campo_tipo + ", " + i + "));")

                        // ¿Habilitar columna HASTA?
                        if (habilitar_hasta) { 
                            strHTML += "<td>" + draw_pre_pos(prefijo, "td_" + name_hasta, posfijo) + "</td>"

                            eval_cdef.push("campos_defs.add('" + name_hasta + "', getCampo_def_options('" + campo_def + "', 'td_" + name_hasta + "', " + nro_campo_tipo + "));")
                        }
                    }
                    else {
                        var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='" + name_desde + "' id='" + name_desde + "' style='cursor: pointer;' /></td></tr></table>"
                ...
                if (campo_def_valor != "")
                    eval_cdef.push("campos_defs.add('valor_1_" + i + "', getCampo_def_options('" + campo_def_valor + "', 'td_valor_1_" + i + "', 0, " + i + "));")
                else {
                    nro_campo_tipo = +arTipoDato[Pizarra["tipo_dato"]].nro_campo_tipo

                    if (nro_campo_tipo != 105)
                        eval_cdef.push("campos_defs.add('valor_1_" + i + "', getCampo_def_options('', 'td_valor_1_" + i + "', " + (+arTipoDato[tipo_dato_valor].nro_campo_tipo) + "));")
                    else {
                        var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='valor_1_" + i + "' id='valor_1_" + i + "' style='cursor: pointer;' /></td></tr></table>"
                ...
                if (cant_salidas_extra > 0) {
                    ...
                    for (var ii = index_inicio; ii < cant_salidas_extra + index_inicio; ii++) {
                        ...
                        nro_campo_tipo  = parseInt(arTipoDato[tipo_dato_valor].nro_campo_tipo)   // update

                        strHTML += "<td>" + draw_pre_pos(salida["prefijo"], "td_valor_" + ii + "_" + i, salida["posfijo"]) + "</td>"

                        if (salida["campo_def"] != "")
                            eval_cdef.push("campos_defs.add('valor_" + ii + "_" + i + "', getCampo_def_options('" + salida["campo_def"] + "', 'td_valor_" + ii + "_" + i + "', 0, " + i + "));campos_defs.habilitar('valor_" + ii + "_" + i + "', " + habilitar + ");")
                        else {
                            if (nro_campo_tipo != 105)
                                eval_cdef.push("campos_defs.add('valor_" + ii + "_" + i + "', getCampo_def_options('', 'td_valor_" + ii + "_" + i + "', " + (+arTipoDato[tipo_dato_valor].nro_campo_tipo) + "));campos_defs.habilitar('valor_" + ii + "_" + i + "', " + habilitar + ");")
                            else {
                                // Dibuja un CHECKBOX
                                var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='valor_" + ii + "_" + i + "' id='valor_" + ii + "_" + i + "' style='cursor: pointer;' /></td></tr></table>"
            ...
            if (eval_cdef.length == 0) {
                ...
                eval_cdef.forEach(function(item) {
                    eval(item)
                })
                
                setTimeout(function() { nvFW.bloqueo_desactivar(null, "bloq_detalles") }, 5)
            }
        }
        ...
        function pizarra_eliminar_detalle(indice, habilitar)
        {
            ...
            nvFW.confirm("¿Desea eliminar el detalle seleccionado?", {
                ...
                ok:          function (win) {
                    ...
                    PizarraDet[i]['id_calc_piz_det'] *= -1
                    ...
                    if (!filtro_activado) {
                        ...
                        //---------------------------------
                        valores_camposFiltro.length = 0
                        
                        for (var i = 1, max = PizarraDef.length; i <= max; i++)
                            valores_camposFiltro.push($F("filtro_dato_" + i))

                        pizarra_det_dibujar() // Re-dibujar los Detalles

                        // Aplicar el filtrado
                        if (valores_camposFiltro.length) {
                            for (var i = 1, max = PizarraDef.length; i <= max; i++)
                                if (valores_camposFiltro[i - 1] != undefined && valores_camposFiltro[i - 1] != '')
                                    campos_defs.set_value("filtro_dato_" + i, valores_camposFiltro[i - 1])

                            // Muestro los campos de filtrado
                            filtro_oculto = true
                            pizarra_verFiltro()
                            pizarra_filtrarDatos() // Lanzar la funcion de filtrado
                        }
                    }
        ...
        function actualizar_datos_def()
        {
            if (PizarraDef.length > 0) {
                PizarraDef.each(function (arreglo, i) {
                    arreglo['etiqueta']          = $('etiqueta_' + i).value
                    arreglo['calc_pizarra_dato'] = $('calc_pizarra_dato_' + i).value
                    arreglo['tipo_dato_def']     = $('tipo_dato_def_' + i).value
                    arreglo['tiene_hasta']       = $('tiene_hasta_' + i).checked == true ? 1 : 0
                    arreglo['limite']            = $('limite_' + i).value
                    arreglo['campo_def']         = $('campo_def_' + i).value
                    arreglo['prefijo']           = $('prefijo_' + i).value
                    arreglo['posfijo']           = $('posfijo_' + i).value
                    arreglo['ancho']             = $('width_' + i).value
                });
            }
        }
            ...
            PizarraDet.each(function (detalle, i) {
                ...
                while (contador > 0) {
                    if (detalle['id_calc_piz_det'] >= 0) {
                        detalle['dato' + j + '_desde'] = $('dato' + j + '_desde_' + i).value

                        if ($('dato' + j + '_desde_' + i).value == "")
                            datos_completos = false

                        if (PizarraDef[j - 1].tiene_hasta == 1) {
                            detalle['dato' + j + '_hasta'] = $('dato' + j + '_hasta_' + i).value

                            if ($('dato' + j + '_hasta_' + i).value == "")
                                datos_completos = false
                        }
                    }
                ...
                if (detalle['id_calc_piz_det'] >= 0) {
                    if ($('valor_1_' + i) != null) {
                        detalle['valor_1'] = $('valor_1_' + i).type != "checkbox" ? $('valor_1_' + i).value : $('valor_1_' + i).checked ? "1" : "0"

                        if ($('valor_1_' + i).value == "" && $('valor_1_' + i).type != "checkbox")
                            datos_completos = false
                    }
                    
                    if ($('valor_2_' + i) != null) {
                        detalle['valor_2'] = $('valor_2_' + i).type != "checkbox" ? $('valor_2_' + i).value : $('valor_2_' + i).checked ? "1" : "0"

                        if ($('valor_2_' + i).value == "" && $('valor_2_' + i).type != "checkbox")
                            datos_completos = false
                    }

                    if ($('valor_3_' + i) != null) {
                        detalle['valor_3'] = $('valor_3_' + i).type != "checkbox" ? $('valor_3_' + i).value : $('valor_3_' + i).checked ? "1" : "0"

                        if ($('valor_3_' + i).value == "" && $('valor_3_' + i).type != "checkbox")
                            datos_completos = false
                    }
        ...
        function pizarra_guardar_wrapper(nuevo_nombre_pizarra)
        {
            ...
            // Si es ALTA de pizarra (nro_calc_pizarra == 0), verificar Nombre y Tipo de Dato
            else {
        ...
        function pizarra_guardar(nuevo_nombre_pizarra, perfil_permisos)
        {
            ...
            var xmldato = generarXML(false, nuevo_nombre_pizarra, perfil_permisos)
            ...
            nvFW.error_ajax_request('/FW/pizarra/calculos_pizarra_ABM.aspx', {
                ...
                onSuccess: function(err, transport) {
                    if (err.numError != 0) {
                        ...
                        if (filtro_activado) {
                            if (valores_camposFiltro.length > 0) {
                                ...
                                filtro_oculto = true
                                pizarra_verFiltro()
                                pizarra_filtrarDatos() // Lanzar el filtrado
                            }
                        }
        ...
        function eliminar()
        {
            ...
            var xmldato = ""
            xmldato += "<calc_pizarra_cab nro_calc_pizarra='" + $('txt_nro_calc_pizarra').value * (-1) + "' etiqueta='" + $('etiqueta_cab').value + "' calc_pizarra='" + $('calc_pizarra').value + "' prefijo='" + $('prefijo').value + "' posfijo='" + $('posfijo').value + "' tipo_dato='" + $('tipo_dato').value + "' campo_def='" + $('campo_def_cab').value + "'>"
        ...
        function obtener_select_tipoDato(name_id) {
            // si el arreglo con los valores no esta listo, lo cargo
            if (!arTipoDato) {
                arTipoDato = {}
                var rs = new tRS()
                ...
                while (!rs.eof()) {
                    ...
                    arTipoDato[id]                   = {}
                    arTipoDato[id]["id"]             = id
                    arTipoDato[id]["campo"]          = rs.getdata("campo")
                    arTipoDato[id]["nro_campo_tipo"] = rs.getdata("nro_campo_tipo")
                    arTipoDato[id]["adoXML"]         = rs.getdata("adoXML")
            ...
            for (el in arTipoDato) {
                var arr = arTipoDato[el]
        ...
        function pizarra_filtrarDatos()
        {
            ...
            for (var i = 1; i <= cant_datos; i++) {
                arFiltro[i] = []
                arFiltro[i]["valor"]       = $F("filtro_dato_" + i) != "" ? $F("filtro_dato_" + i) : null
                arFiltro[i]["tipo_dato"]   = (PizarraDef[i - 1].tipo_dato_def).toLowerCase()
                arFiltro[i]["tiene_hasta"] = PizarraDef[i - 1].tiene_hasta == 1
                arFiltro[i]["campo_def"]   = PizarraDef[i - 1].campo_def != ""
                arFiltro[i]["limite"]      = PizarraDef[i - 1].limite
            }
            ...
            for (var fila = 0, max = PizarraDet.length; fila < max; fila ++) {
                ...
                for (var dato = 1; dato <= cant_datos; dato++) {
                    ...
                    if (arFiltro[dato].valor != null) {
                        // tiene_hasta => comparacion between
                        if (arFiltro[dato].tiene_hasta) {
                            // between
                            var desde = PizarraDet[fila]["dato" + dato + "_desde"] 
                            var hasta = PizarraDet[fila]["dato" + dato + "_hasta"] 
                            var valor_filtro = arFiltro[dato].valor

                            switch (arFiltro[dato].tipo_dato) {
                                case "datetime":
                            ...
                            switch (arFiltro[dato].tipo_dato) {
                                ...
                                case "money":
                                    ...
                                    if (arFiltro[dato].campo_def) {
                                        hidefila = eval("[" + valor_filtro + "]").indexOf(desde) == -1 ? true : false
                                    }
                                    else {
                                        var limite = arFiltro[dato].limite
                                        // caso trivial
                                        valor_filtro = +valor_filtro
            ...
            window.setTimeout(function() { cargar_indices_visibles() }, 100)
            // mostrar todos los que tengan ID = 0
            show_new_items()
            // limpio el bloqueo
            nvFW.bloqueo_desactivar(null, "bloq_filtro")
        }
        ...
        function cargar_indices_visibles(limpiar)
        {
            ...
            if (limpiar)
                ...
                $$("input[name=id_calc_piz_det]").each(function(elemento) {
                    if (elemento.up("tr").style.display != "none" && elemento.value != "0")
                        indices_visibles.push(+elemento.value)
                })
            }
        }
        ...
        {
            $$("input[name=id_calc_piz_det]").each(function(item) {
                if (item.value == "0") {
                    $(item).up("tr").show()
                }
            })
        ...
        function pizarra_resetearFiltro()
        {
            var cant_datos    = PizarraDef.length
            ...
            pizarra_filtrarDatos() // llamada al filtrado

            window.setTimeout(function() { cargar_indices_visibles(true) }, 100)
        }
        ...
        function getTimeStamp()
        {
            ...
            return timestamp.getFullYear().toString() + 
                ...
                rellenar_izq(timestamp.getDate(),      2, '0') + "_" + 
        ...
        function getExcelTipo(tipo_dato, esCampoDef)
        {
            return esCampoDef ? 'string' : arTipoDato[tipo_dato].adoXML
            //if (esCampoDef)
            //    return "string"
            //else
            //    return arTipoDato[tipo_dato].adoXML 
        }
        ...
        function obtener_xmlADO()
        {
            ...
            PizarraDef.each(function(dato, pos) {
                columnas[nro_columna] = {}
                tipo_dato             = getExcelTipo(dato["tipo_dato_def"], dato.campo_def != "")
            ...
            tipo_dato                    = getExcelTipo(Pizarra["tipo_dato"], Pizarra.campo_def != null  && Pizarra.campo_def != '')
            precision                    = tipo_dato == "float" ? 2 : 0
            ...
            if (PizarraExtra['valores'].length > 0) {
                ...
                PizarraExtra['valores'].each(function (salida_extra, index) {
                    ...
                    tipo_dato                    = getExcelTipo(salida_extra['tipo_dato'], salida_extra['campo_def'] != null && salida_extra['campo_def'] != '')
                    precision                    = tipo_dato == 'float' ? 2 : 0
        ...
        {
            return (event.keyCode || event.which) == 13 && pizarra_filtrarDatos()
            //var key = isIE ? event.keyCode : event.which
            //return key == 13 && pizarra_filtrarDatos()
        }
        ...
        function generarXML(actualizar_datos, nuevo_nombre_pizarra, perfil_permisos)
        {
            ...
            if (perfil_permisos != undefined && perfil_permisos != null && perfil_permisos != {}) {
            ...
            if (!xml_comparacion.original.xml_def)
                xml_comparacion.original.xml_def = definicion.replace(/\s*/g, '')
            else
                xml_comparacion.nuevo.xml_def = definicion.replace(/\s*/g, '')
            ...
            PizarraDet.each(function(arreglo, i) {
                ...
                while (contador > 0) {
                    detalle += "dato" + j + "_desde='" + arreglo['dato' + j + '_desde'] + "' "
                    detalle += "dato" + j + "_hasta='" + (arreglo['dato' + j + '_hasta'] == null ? '' : arreglo['dato' + j + '_hasta']) + "' "

                    contador--
                    j++
                }

                detalle += " valor_1='" + arreglo['valor_1'] + "'"

                if (arreglo['valor_2'] != undefined)
                    detalle += " valor_2='" + arreglo['valor_2'] + "'"
                else
                    detalle += " valor_2=''"

                if (arreglo['valor_3'] != undefined)
                    detalle += " valor_3='" + arreglo['valor_3'] + "'"
                else
                    detalle += " valor_3=''"
            ...
            if (!xml_comparacion.original.xml_det)
                xml_comparacion.original.xml_det = detalle.replace(/\s*/g, '')
            else
                xml_comparacion.nuevo.xml_det = detalle.replace(/\s*/g, '')

            // Cabecera cierre
            cabecera_cierre += "</calc_pizarra_cab>"

            if (!xml_comparacion.original.xml_cab)
                xml_comparacion.original.xml_cab = (cabecera_apertura + cabecera_cierre).replace(/\s*/g, '')
            else
                xml_comparacion.nuevo.xml_cab = (cabecera_apertura + cabecera_cierre).replace(/\s*/g, '')
            ...
            if (win.options.userData.valores_extra) {
                ...
                if (valores_diferentes) {
                    ...
                    if (win.options.userData.valores_eliminados) {
                        ...
                        PizarraDet.forEach(function(detalle) {
        ...
        function pizarra_cubo_crear()
        {
            ...
            var datos = pizarra_cubo_verificar_datos()

            if (datos.length == 0) {
                alert('<b>El/los dato/s no están definidos como campos defs o tienen seteado el item "tiene hasta".<br>Para proseguir debe seleccionar un campo def o bien deseleccione el item "tiene hasta".', {
                    title:  '<b>Generación de CUBO</b>',
                    width:  350,
                    height: 120
                })

                // Si fallo la creacion de la pizarra CUBO vuelvo a false
                Pizarra['cubo'] = false
                return
            }
        ...
        function pizarra_cubo_verificar_datos()
        {
            var datos = []
        ...
        function pizarra_cubo_dibujar(combinaciones, total)
        {
            ...
            if (!isNaN(total) && total > 0) {
                try {
                    ...
                    eval(combinaciones) // Desde aqui está disponible "combinaciones[][]"
                    ...
                    combinaciones.each(function (fila, pos) {
                        ...
                        for (; indice < cant_datos; indice += 1) {
                            ...
                            evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("' + PizarraDef[indice].campo_def + '", "td_' + name + '", 0));') // Crear
                            
                            if (fila[indice] != undefined && fila[indice] != '')
                                evaluar.push('campos_defs.set_value("' + name + '", "' + fila[indice] + '");') // Setear solo si hay valor
                        ...
                        if (Pizarra.campo_def != '') {
                            evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("' + Pizarra.campo_def + '", "td_' + name + '", 0, ' + pos + '));') // Crear
                        }
                        else {
                            nro_campo_tipo = Number(arTipoDato[Pizarra['tipo_dato']]['nro_campo_tipo']) || 104

                            if (nro_campo_tipo != 105) {
                                evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("", "td_' + name + '", ' + nro_campo_tipo + ', ' + pos + '));') // Crear
                            }
                            else {
                                // Dibuja un CHECKBOX
                        ...
                        if (cant_salidas_extra > 0) {
                            ...
                            PizarraExtra['valores'].each(function (salida, idx) {
                                ...
                                if (salida['campo_def'] != '') {
                                    evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("' + salida['campo_def'] + '", "td_' + name + '", 0, ' + pos + '));') // Crear
                                }
                                else {
                                    nro_campo_tipo = Number(arTipoDato[salida['tipo_dato']]['nro_campo_tipo']) || 104
                    ...
                    // Insertar el HTML
                    if (html != '')
                        $div_pizarra_det.select('table tbody')[0].insert({ bottom: html })
        ...
        {
            $div_pizarra_det.select('img[alt=Eliminar]').each(function (imagen) { imagen.replace('&nbsp;') })
        }
        ...
        function make_combinations(arreglos)
        {
            ...
            arreglos.each(function (arreglo) {
                combinations.push(arreglo.join('_'))
            })
        ...
        function get_current_combinations()
        {
            var amount_of_data = PizarraDef.length
            var combinations   = []
            var combination    = []
            
            PizarraDet.each(function(detail) {
                combination = []

                for (var i = 1; i <= amount_of_data; i += 1) {
                    combination.push(detail['dato' + i + '_desde'])
                }

                combinations.push(combination.join('_'))
            })
        ...
        function pizarra_cubo_add_combination(actuales, nuevas)
        {
            ...
            nuevas.each(function (combination) {
                valores_def = combination.split("_")
                ...
                for (var indice = 0; indice < cant_datos; indice += 1) {
                    ...
                    if (valores_def[indice] != undefined && valores_def[indice] != '')
                        evaluar.push('campos_defs.set_value("' + name + '", "' + valores_def[indice] + '");') // Setear solo si hay datos
                ...
                if (Pizarra['campo_def'] != '') {
                    evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("' + Pizarra['campo_def'] + '", "td_' + name + '", 0, ' + nro_fila + '));') // Crear
                }
                else {
                    nro_campo_tipo = Number(arTipoDato[Pizarra['tipo_dato']]['nro_campo_tipo']) || 104

                    if (nro_campo_tipo != 105) {
                        evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("", "td_' + name + '", ' + nro_campo_tipo + ', ' + nro_fila + '));') // Crear
                    }
                    else {
                        // Dibuja un CHECKBOX
                        var html_chk = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='" + name + "' id='" + name + "' style='cursor: pointer;' /></td></tr></table>"
                ...
                if (cant_salidas_extra > 0) {
                    ...
                    PizarraExtra['valores'].each(function (salida, idx) {
                        ...
                        if (salida['campo_def'] != '') {
                            evaluar.push('campos_defs.add("' + name + '", getCampo_def_options("' + salida['campo_def'] + '", "td_' + name + '", 0, ' + nro_fila + '));') // Crear
                        }
                        else {
                            nro_campo_tipo = Number(arTipoDato[salida['tipo_dato']]['nro_campo_tipo']) || 104
            ...
            if (evaluar.length > 0) {
                evaluar.each(function (item) {
                    setTimeout(function() { eval(item) }, 10)
                })
            }
        }
...
<body onload="window_onload()" onresize="window_onresize()" style="width: 100%; height: 100%; overflow: hidden; background-color: white;">
    ...
    <script>nvFW.bloqueo_activar($$("body")[0], "bloq_pizarra", "Cargando pizarra...")</script>
    ...
    <form action="/FW/pizarra/calculos_pizarra_ABM.aspx" method="post" name="form1" target="frmEnviar" style="width: 100%; height: 100%; margin: 0;" novalidate="novalidate" autocomplete="off">
        ...
        <div id="div_pizarra" style="display: none;">
            <table id="tb_pizarra" class="tb1">
                <tr class="tbLabel">
                    ...
                    <td style="width: 10%; text-align: center;">Ancho px</td>
                </tr>
                <tr>
                    <td style="width: 4%">
        ...
        <div id="divPizarraDet" style="display: none">
            ...
            <script type="text/javascript">
                ...
                Menus["vMenuPizarraDet"].CargarMenuItemXML("<MenuItem id='0' ><Lib TipoLib='offLine'>DocMNG</Lib><icono>filtrar</icono><Desc>Filtro</Desc><Acciones><Ejecutar Tipo='script'><Codigo>pizarra_verFiltro()</Codigo></Ejecutar></Acciones></MenuItem>")
                Menus["vMenuPizarraDet"].CargarMenuItemXML("<MenuItem id='1' style='WIDTH: 100%; text-align:center;'><Lib TipoLib='offLine'>DocMNG</Lib><icono></icono><Desc>Detalles</Desc></MenuItem>")
