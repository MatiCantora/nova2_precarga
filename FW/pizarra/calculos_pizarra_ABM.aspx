...
<%
    ...
    ' Operador
    Dim operador = nvFW.nvApp.getInstance().operador
            ...
            Case "m"
                ...
                If strXML <> "" Then
                    Try
                        ...
                        Dim cmd As New nvDBUtiles.tnvDBCommand("calculo_pizarra_abm", ADODB.CommandTypeEnum.adCmdStoredProc)
    ...
    Dim nro_permiso As Integer = 2      ' Permiso para crear / editar estructura Pizarra
    Dim permiso_edicion_estructura As Boolean = operador.tienePermiso("permisos_pizarra_gral", 2)
    Dim permiso_edicion_detalles As Boolean = False 'nvFW.nvApp.getInstance().operador.tienePermiso(permiso_grupo, Math.Pow(2, nro_permiso_editar - 1))
    Dim calc_pizarra As String = ""     ' nombre de la pizarra

    If nro_calc_pizarra <> 0 Then
        Dim strSQL As String = "SELECT TOP 1 calc_pizarra, a.nro_permiso_ver, a.nro_permiso_editar, b.permiso_grupo, " &
            "dbo.rm_tiene_permiso(b.permiso_grupo, nro_permiso_editar) AS tiene_permiso_editar, c.permiso AS valor " &
            "FROM calc_pizarra_cab a " &
            "INNER JOIN operador_permiso_grupo b On b.nro_permiso_grupo = a.nro_permiso_grupo " &
            "INNER JOIN verOperador_permisos c ON b.nro_permiso_grupo = c.nro_permiso_grupo " &
            "WHERE nro_calc_pizarra=" & nro_calc_pizarra & " and habilitada=1"

        Dim rs As ADODB.Recordset = nvDBUtiles.DBExecute(strSQL)

        If Not rs.EOF Then
            ' Agregar dinámicamente el grupo de permiso y su valor
            ...
            Me.contents("pizarra_permiso_grupo") = permiso_grupo
            Me.contents("pizarra_nro_permiso_ver") = rs.Fields("nro_permiso_ver").Value
            Me.contents("pizarra_nro_permiso_editar") = rs.Fields("nro_permiso_editar").Value
            permiso_edicion_detalles = rs.Fields("tiene_permiso_editar").Value.ToString.ToLower = "true"
        End If
    ...
    If permiso_grupo <> "" Then Me.addPermisoGrupo(permiso_grupo)
%>
<html>
<head>
    <title><% = titulo %></title>
    ...
    <script type="text/javascript">
        ...
        function window_onload()
        {
            ...
            //seteo de datos por default de paginado
            campos_head.id_exp_origen = 0
            campos_head.cacheID = ''
            campos_head.pagina_cambiar = pizarra_cargar_vector_detalles
            campos_head.nvFW = nvFW
            nvFW.reporte_parametro[campos_head.id_exp_origen] = {}
        ...
        function pizarra_datos_cargar()
        {
            ...
            if (!rs.eof())
            {
                ...
                Pizarra.paginado = rs.getdata('paginado', 'false').toLowerCase() == 'true';
                Pizarra.page_size = rs.getdata('page_size', '0');
            }
            ...
            $('paginado').checked = Pizarra.paginado == null ? true : Pizarra.paginado;
            $('page_size').value = Pizarra.page_size == null ? '' : Pizarra.page_size;

            // visualiza a nivel de front el comportamiento de la definicion del paginado
            onchange_paginado()
        ...
        function checkSelector(checkboxElement)
        {
            ...
            // Actualizar el vector de Definiciones y redibujar los detalles
        ...
        function pizarra_eliminar_dato(indice)
        {
            ...
            nvFW.confirm("¿Desea eliminar el dato seleccionado?",
            {
                ...
                ok: function (win)
                {
                    ...
                    var dato_eliminado = PizarraDef.splice(i, 1);
                    var dato_posicion  = parseInt(dato_eliminado[0].dato_orden);
                    ...
                    $div_pizarra_det.innerHTML = '';
                    pizarra_eliminar_dato_detalle(dato_posicion);
                    pizarra_dibujar_detalles();
                    //try
                    //{
                    //    _pizarra_eliminar_dato_detalle();
                    //}
                    //catch (e) { }
                }
            });
        ...
        function pizarra_cargar_vector_detalles(AbsolutePage, filtros)
        {
            /*-----------------------------------------------------------------
            | Filtros (opcional)
            |------------------------------------------------------------------
            ...
            else
            {
                ...
                if (Pizarra.paginado) {
                    ...
                    filtroPaginado = " AbsolutePage='" + AbsolutePage + "' PageSize='" + Pizarra.page_size + "' "
                }

                var filtroWhere = "<criterio><select " + filtroPaginado + " ><filtro><nro_calc_pizarra type='igual'>" + nro_calc_pizarra + "</nro_calc_pizarra>" + filtros + "</filtro><orden>" + orden + "</orden></select></criterio>"
                nvFW.reporte_parametro[campos_head.id_exp_origen].filtroWhere = filtroWhere
        ...
        //function _pizarra_eliminar_dato_detalle()
        //{
        //    var contador = 0;
        //    var j = 0;
        //    var aux = [];
        ...
        function pizarra_eliminar_dato_detalle(dato_posicion)
        {
            var cant_datos      = PizarraDef.length + 1;  // tenemos que contar el dato que se elimino, ya que esta en los detalles
            var detalles_nuevos = [];
            var detalle_nuevo   = [];
            var idx_real        = 1;
            ...
            PizarraDet.each(function (detalle) {
                ...
                // datos
                for (var pos = 1; pos <= cant_datos; pos++)
                {
                    if (pos === dato_posicion)
                        continue;

                    detalle_nuevo['dato' + idx_real + '_desde'] = detalle['dato' + pos + '_desde'];

                    if (detalle['dato' + pos + '_hasta']) 
                        detalle_nuevo['dato' + idx_real + '_hasta'] = detalle['dato' + pos + '_hasta'];

                    // orden
                    detalle_nuevo['orden'] = detalle['orden'];
                    // id
                    detalle_nuevo['id_calc_piz_det'] = detalle['id_calc_piz_det'];

                    idx_real++;
                }

                // valores
                detalle_nuevo['valor_1'] = detalle['valor_1'];
                detalle_nuevo['valor_2'] = detalle['valor_2'];
                detalle_nuevo['valor_3'] = detalle['valor_3'];
                
                detalles_nuevos.push(detalle_nuevo);
            });

            PizarraDet = detalles_nuevos;
        }
        ...
        function pizarra_nuevo_detalle()
        {
            ...
            if (!nvFW.tienePermiso(nvFW.pageContents.pizarra_permiso_grupo, nvFW.pageContents.pizarra_nro_permiso_editar))
            {
        ...
        function pizarra_dibujar_detalles(rs)
        {
            ...
            {
                pizarra_dibujar_detalles_lineal(rs);      // Dibuja la pizarra en forma Lineal (estandar)
            }
        }


        function pizarra_dibujar_detalles_lineal(rs)
        {
            ...
            if (rs) {
                ...
                campos_head.cacheControl = selectSingleNode('xml/params/@cacheControl', rs.objXML.xml) == null ? 'none' : selectSingleNode('xml/params/@cacheControl', rs.objXML.xml).value
                campos_head.recordcount = selectSingleNode('xml/params/@recordcount', rs.objXML.xml) == null ? '1' : selectSingleNode('xml/params/@recordcount', rs.objXML.xml).value
                campos_head.PageCount = selectSingleNode('xml/params/@PageCount', rs.objXML.xml) == null ? '0' : selectSingleNode('xml/params/@PageCount', rs.objXML.xml).value
                campos_head.PageSize = selectSingleNode('xml/params/@PageSize', rs.objXML.xml) == null ? '30' : selectSingleNode('xml/params/@PageSize', rs.objXML.xml).value
                campos_head.AbsolutePage = selectSingleNode('xml/params/@AbsolutePage', rs.objXML.xml) == null ? '1' : selectSingleNode('xml/params/@AbsolutePage', rs.objXML.xml).value
            }
            ...
            for (var i = 1; i <= cant_datos; i++)
            {
                ...
                else
                {
                    ...
                    {
                        var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='" + id_input + "' id='" + id_input + "' style='margin: 0; cursor: pointer;' /></td></tr></table>";
            ...
            PizarraDet.each(function (fila, i)
            {
                if (Number(fila.id_calc_piz_det) > -1)
                {
                    ...
                    while (contador > 0)
                    {
                        ...
                        fila["dato" + j + "_desde"] = ["money"].indexOf(tipo_dato_def) != -1 && fila["dato" + j + "_desde"] != ""
                        ...
                        if (nro_campo_tipo != 105)
                        {
                            ...
                            if (habilitar_hasta)
                            {
                                ...
                                // Si tipo de dato es MONEY => parsear el valor a punto decimal
                                fila["dato" + j + "_hasta"] = ["money"].indexOf(tipo_dato_def) != -1 && fila["dato" + j + "_desde"] != "" ? parseFloat(fila["dato" + j + "_hasta"]).toFixed(2).replace(",", ".") : fila["dato" + j + "_hasta"];

                                fila["dato" + j + "_hasta"] = ["decimal"].indexOf(tipo_dato_def) != -1 && fila["dato" + j + "_desde"] != "" ? fila["dato" + j + "_hasta"].replace(",", ".") : fila["dato" + j + "_hasta"];
                        ...
                        {
                            var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='" + name_desde + "' id='" + name_desde + "' style='margin: 0; cursor: pointer;' /></td></tr></table>";
                    ...
                    fila.valor_1 = ["money"].indexOf(tipo_dato_valor) != -1 ? parseFloat(fila.valor_1).toFixed(2).replace(",", ".") : fila.valor_1;
                    fila.valor_1 = ["decimal"].indexOf(tipo_dato_valor) != -1 ? fila.valor_1.replace(",", ".") : fila.valor_1;
                    ...
                    else
                    {
                        ...
                        {
                            // Dibujar un checkbox
                            var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='valor_1_" + i + "' id='valor_1_" + i + "' style='margin: 0; cursor: pointer;' /></td></tr></table>";
                    ...
                    if (PizarraExtra && PizarraExtra["valores"].length)
                    {
                        ...
                        for (var ii = index_inicio; ii < PizarraExtra.valores.length + index_inicio; ii++)
                        {
                            ...
                            fila["valor_" + ii] = ["money"].indexOf(tipo_dato_valor) != -1 ? parseFloat(fila["valor_" + ii]).toFixed(2).replace(",", ".") : fila["valor_" + ii];
                            fila["valor_" + ii] = ["decimal"].indexOf(tipo_dato_valor) != -1 ? fila["valor_" + ii].replace(",", ".") : fila["valor_" + ii];
                            ...
                            else
                            {
                                ...
                                {
                                    // Dibuja un CHECKBOX
                                    var html = "<table class='tb1'><tr><td style='text-align: center;'><input type='checkbox' name='valor_" + ii + "_" + i + "' id='valor_" + ii + "_" + i + "' style='margin: 0; cursor: pointer;' /></td></tr></table>";
            ...
            //agrega el paginado
            if (Pizarra.paginado == true)
            {
                $('tbPie').innerHTML = "";
                $('tbPie').insert({ top: campos_head.paginas_getHTML() });
            }
                ...
                for (var columna_pos = 0; columna_pos < cantidad_columnas + 1; columna_pos++)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Dibujar un checkbox
                                var html = '<table class="tb1"><tr><td style="text-align: center;"><input type="checkbox" name="' + campo_def_custom + '" id="' + campo_def_custom + '" style="margin: 0; cursor: pointer;" /></td></tr></table>';
        ...
        function pizarra_guardar_wrapper(nuevo_nombre_pizarra)
        {
            ...
            else
            {
                ...
                if (nvFW.tienePermiso("permisos_pizarra_gral", 3))
                {
        ...
        function pizarra_guardar(nuevo_nombre_pizarra, perfil_permisos)
        {
            ...
            // Verificar si hay combinaciones repetidas (Modo: cubo o matriz)
            if (Pizarra.cubo)
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (chequeo.seccion.indexOf('cab') > -1 || chequeo.seccion.indexOf('def') > -1)
                    {
                        ...
                        if (!nvFW.tienePermiso('permisos_pizarra_gral', 2))
                        {
                            nvFW.bloqueo_desactivar(null, "guardar");
        ...
        function window_onresize()
        {
            try
            {
                ...
                var div_tbPie_h = $('tbPie').getHeight();

                
                var valor_h = modo_edicion
                    ? body_h - divPizarraDatos_h - div_pizarra_h - divPizarraDef_h - div_pizarra_def_h - divPizarraDet_h - div_tbPie_h - dif
                    : body_h - divPizarraDatos_h - divPizarraDet_h - div_tbPie_h - dif;
        ...
        function eliminar()
        {
            ...
            var xmldato = "";
            xmldato += "<calc_pizarra_cab nro_calc_pizarra='" + $('txt_nro_calc_pizarra').value * (-1) + "' etiqueta='" + $('etiqueta_cab').value + "' calc_pizarra='" + $('calc_pizarra').value + "' prefijo='" + $('prefijo').value + "' posfijo='" + $('posfijo').value + "' tipo_dato='" + $('tipo_dato').value + "' campo_def='" + $('campo_def_cab').value + "' auto_ordenada='" + ($('auto_ordenada').checked ? '1' : '0') + "' paginado='" + ($('paginado').checked ? '1' : '0') + "' page_size='" + $('page_size').value + "'>";
        ...
        function pizarra_filtrar_datos()
        {
            ...
            //var filtros = "<dato1_desde type='igual'>'" + ar_filtro[1].valor + "'</dato1_desde>"
            //pizarra_cargar_vector_detalles(null,filtros)
        ...
        function obtener_xmlADO()
        {
                ...
                PizarraDet.each(function (detalle, i)
                {
                    ...
                    else
                    {
                        ...
                        for (; icol < cant_columnas; icol++)
                        {
                            ...
                            {
                                xml_ado += nombre_columnas[icol] + '="' + ($('valor_' + isalida + '_' + i).checked ? 'SI' : 'NO') + '" ';
                            }
            ...
            else
            {
                PizarraDet.each(function (arr, i)
                {
                    ...
                    for (; icol < cant_columnas; icol++)
                    {
                        campo_name = 'valor_' + isalida + '_' + i;

                        // Checkear si la salida es un campo_def
                        try
                        {
                            xml_ado += nombre_columnas[icol] + '="' + replace(campos_defs.get_desc(campo_name),'"','\'') + '" ';
                        }
                        catch (e)
                        {
                            xml_ado += nombre_columnas[icol] + '="' + ($(campo_name).checked ? 'SI' : 'NO') + '" ';
                        }
        ...
        function onchange_paginado() {
            
            if ($('paginado').checked == true)
                $('page_size').disabled = false

            if ($('paginado').checked == false) {
                $('page_size').value = ""
                $('page_size').disabled = true
            }
    ...
    <form method="post" name="form1" target="frmEnviar" style="width: 100%; height: 100%; margin: 0;" novalidate="novalidate" autocomplete="off">
        ...
        <div id="div_pizarra" style="display: none;">
            <table id="tb_pizarra" class="tb1">
                <tr class="tbLabel">
                    ...
                    <td style="width: 6%; text-align: center" title="Paginado">Con Paginado</td>
                    <td style="width: 6%; text-align: center" title="Cantidad de registro por página">Reg. x Página</td>
                ...
                <tr>
                    ...
                    <td style="width: 6%; text-align: center;">
                        <input type="checkbox" name="paginado" id="paginado" title="Auto ordenado" style="cursor: pointer;" onclick="return onchange_paginado()" />
                    </td>
                    <td style="width: 6%; text-align: center;">
                        <input type="text" id="page_size" name="page_size" title="Cantidad de registro por página" value="" disabled="disabled"  style="width: 100%" />
                    </td>
        ...
        <div id="divPizarraDet" style="display: none;">
            ...
            <script type="text/javascript">
                ...
                <% If Not load_from_log Then %>
                vMenuPizarraDet.loadImage("filtrar", "/FW/image/icons/filtrar.png");
