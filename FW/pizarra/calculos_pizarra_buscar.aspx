...
<% 
    ...
    Dim permiso_grupo As String = "permisos_pizarra_gral"
    Dim nro_permiso_ver As Integer = 1
    Dim nro_permiso_editar As Integer = 2

    If Not operador.tienePermiso(permiso_grupo, nro_permiso_ver) Then
        Response.Redirect("/FW/error/httpError_403.aspx?subtitulo=403 Forbidden", True)
        'Response.Write("<p>No tiene permiso de acceso al módulo de <b>Pizarras</b>. Consulte con el Administrador del sistema.</p>")
        'Response.End()
    End If

    Dim tiene_permiso_creacion As Boolean = operador.tienePermiso("permisos_pizarra_gral", nro_permiso_editar)
    Dim desplegarPizarra As Boolean = nvUtiles.obtenerValor("desplegarPizarra", "true").ToString.ToLower = "true"
    Me.contents.Add("desplegarPizarra", desplegarPizarra)
    Me.contents("filtro_pizarra") = nvXMLSQL.encXMLSQL("<criterio><select vista='verPizarras'><campos>*</campos></select></criterio>")
%>
<html>
<head>
    ...
    <script type="text/javascript">
        ...
        {
            // Agregar evento al campo_def para ver cuando se ingresa un ENTER
            campos_defs.items["nro_calc_pizarra_text"].input_hidden.addEventListener('keypress', enter_onkeypress);
        ...
        function buscar_onclick()
        {
            ...
            if (campos_defs.get_value('nro_calc_pizarra_text'))
            {
                filtroWhere += "<nro_calc_pizarra type='igual'>" + campos_defs.get_value('nro_calc_pizarra_text') + "</nro_calc_pizarra>";
            }
...
<body onload="window_onload()" onresize="window_onresize()" style="width: 100%; height: 100%; overflow: hidden; background-color: white;">
    ...
    <table class="tb1" id="tablaMenu">
        <tr class="tbLabel">
            <td class="Tit1" style="width: 10%; text-align: center;">Nro</td>
            ...
            <td class="Tit1" style="width: 20%; text-align: center;">Tipo dato</td>
            <td class="Tit1" style="width: 20%; text-align: center;">&nbsp;</td>
        </tr>
        <tr>
            <td style="width: 10%">
                <% = nvFW.nvCampo_def.get_html_input("nro_calc_pizarra_text", enDB:=False, nro_campo_tipo:=100) %>
                <%--<input style="width: 100%;" id="nro_calc_pizarra_text" name="nro_calc_pizarra_text" type="text" value="" onkeypress="return enter_onkeypress(event)" />--%>
            </td>
            <td style="width: 30%">
