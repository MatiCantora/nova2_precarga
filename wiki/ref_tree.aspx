...
<%
    ...
    Me.contents("filtroDragAndDrop") = nvFW.nvXMLSQL.encXMLSQL("<criterio><procedure CommandText='drag_and_drop_ref'><parametros><nodo_id_drag>%nodo_id_drag%</nodo_id_drag><nodo_id_drop>%nodo_id_drop%</nodo_id_drop><copiar>%copiar%</copiar></parametros></procedure></criterio>")
...
//        }
        ...
        function nodo_ref_onclick(nodo_id) {
            ...
            if (nro_ref != '') {
                ...
                // se controla si puede ver la referencia.
                // TODO: no se deberia mostrar los nodos del arbol en principio
                /*if (!window.top.wikiPermisoRef.tienePermisoReferencia(4, nro_ref)) {
                    alert("No tiene permisos para ver la referencia")
                    return
                }*/
        ...
        function ref_mostrar(nro_ref, version, destino) {
            ...
            window.top.nvFW.exportarReporte({
                ...
                parametros: '<parametros><id_ref_auto>' + id_ref_auto + '</id_ref_auto><app_path_rel>' + window.top.app_path_rel + '</app_path_rel><permisos_referencias>' + window.top.permiso_grupos["permisos_referencias"] + '</permisos_referencias></parametros>',
                winPrototype: {
                    modal: true,
                    center: true,
                    bloquear: true,
...
<body onload="window_onload()" style="margin: 0px; padding: 0px; height: 99%;">
    <div id="Div_vTree_0" style="width:100%; height: 100%; overflow-y: visible; padding: 0 0 0 5px;"></div>
