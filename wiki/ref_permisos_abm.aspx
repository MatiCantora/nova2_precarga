...
<%
    ...
    Me.contents("filtroOperadoresGrupo") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verOperadores_operador_tipo'><campos>*</campos><orden></orden><filtro><tipo_operador type='igual'>%tipo_operador%</tipo_operador></filtro></select></criterio>")
    ...
    If (modo.ToUpper = "M") Then
        ...
        Else
            ...
            err.mensaje = "No hay ningún referencia seleccionado"
        End If
        err.response()
    End If
...
<head>
    ...
    <script type="text/javascript">
        ...
        function guardar_ref_permisos() {
            ...
            if (permisos_particulares.length == 0 && hereda_permisos == '0') {
                alert('Es una referencia con permisos propios,</br>debe definir permisos al menos</br>para su perfil')
                return
            }

            // Si se pierde la administracion de permisos y lectura, deberan ser reestablecidos por un admin
            var indicep = -1
            permisos_particulares.each(function (arreglo, i) {
                if (arreglo['permiso'] >= 24) {
                    indicep = i
                }
            });
            ...
            if (indicep == -1 && hereda_permisos == '0') {
                ...
                //alert('Debe asignarse como mínimo los permisos<br/> de lectura y administración de permisos<br/> porque sino se pierde la vinculación con la misma')
                nvFW.confirm('No se han asignado permisos<br/> de lectura y administración de permisos<br/> Sólo los usuarios con permisos globales <br/> podrán administrar esta referencia <br/> ¿Desea continuar?', {
                    width: 300,
                    onOk: function (win) {
                        guardar()
                        win.close()
        ...
        function ver_operadores(tipo_operador, tipo_operador_desc)
        {
            ...
            strHTML += "<td style='width: 65px; text-align: center; font-weight: 700;'>N°</td>"
            strHTML += "<td style='text-align: center; font-weight: 700;'>Operador</td>"
            strHTML += "</tr>"
            strHTML += "</table>"
            //strHTML += "<div style='height:130px;overflow:auto'>"
            strHTML += "<div style='overflow: auto;'>"
            strHTML += "<table class='tb1 highlightOdd highlightTROver'>"

            var rs = new tRS();
            var filtroXML = nvFW.pageContents.filtroOperadoresGrupo
            var params = "<criterio><params tipo_operador='" + tipo_operador + "'/></criterio>"
            rs.open(filtroXML, '', '', '', params)  
            ...
            while (!rs.eof()) {
                if (rs.getdata('nombre_operador') != null) {
                    strHTML += "<tr>"
                    strHTML += "<td class='Tit1' style='width: 65px; text-align: right;'>" + rs.getdata("operador") + "&nbsp;</td>"
                    strHTML += "<td style='text-align: left;'>&nbsp;" + rs.getdata('apellido') + ", " + rs.getdata('nombres') + "</td>"
                    strHTML += "</tr>"
                }
