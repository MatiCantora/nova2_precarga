...
<%
    ...
    If (modo.ToUpper = "M") Then
        If (nro_ref <> "") Then
            ...
            Dim permiso As Integer = rs.Fields("permiso").Value
            nvFW.nvDBUtiles.DBCloseRecordset(rs)
            
            Dim storeprocedure As String = "ref_permisos_abm"
            
            Dim mascara As Integer = 16
            Dim result As Integer
            result = permiso And mascara
...
<head>
    ...
    <script type="text/javascript">
        ...
        function guardar_ref_permisos() {                   
            ...
            //    strError += 'La referencia deben tener definición de Permisos Particulares<br />';
            ...
            if ((nro_ref != "") && (permisos_heredados.length == 0 || indiceh == -1) && (permisos_particulares.length > 0) && permiso_propios < 24)
                strError += 'Debe asignarse como mínimo los permisos<br/> de lectura y administración de permisos<br/> porque sino se pierde la vinculación con la misma';
            ...
            strXML += "<ref_permisos nro_ref = '" + arreglo['nro_ref'] + "' tipo_operador = '" + arreglo['tipo_operador'] + "' permiso = '" + arreglo['permiso'] + "' nro_operador = '" + arreglo['nro_operador'] + "' />"
            });
            strXML += "</permisos>"
            strXML += "</ref>"

            nvFW.error_ajax_request('ref_permisos_abm.aspx', {
            parameters: { modo: 'M', nro_ref: nro_ref, strXML: escape(strXML) },
