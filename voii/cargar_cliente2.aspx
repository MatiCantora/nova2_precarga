...
<%
    ...
    If tipdoc > 0 And nrodoc > 0 Or nro_entidad > 0 Then
        ...
        Me.contents("filtro_vinculos") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='VOII_entidad_vinculos_consolidada' cn='BD_IBS_ANEXA'><campos>vinc_razon_social as razon_social_vinc, vinc_tipocli, vinc_tipdoc as tipo_docu_vinc, vinc_tipdoc_desc as documento_vinc, vinc_nrodoc as nro_docu_vinc, vincliclinom as vinc_tipo, tipvinclidesc as vinc_grupo, clivinfecalta as vinc_desde, clivinfecven as vinc_hasta, vinc_nro_entidad as nro_entidad_vinc</campos><filtro><tipdoc type='igual'>" & tipdoc.ToString & "</tipdoc><nrodoc type='igual'>" & nrodoc.ToString & "</nrodoc></filtro><orden>vinc_razon_social ASC</orden></select></criterio>")
...
<head>
    ...
    <script type="text/javascript">
        ...
        var _cuit_cuil, _tiporel, _nro_entidad;
        var _params;
        var _nro_def_archivo, _def_archivo
        var filtro_vinculosXML = nvFW.pageContents.filtro_vinculos
                    ...
                    if (!rs.eof()) {
                        ...
                        while (!paramRs.eof()) {
                            var param = new tParam_def({
                                ...
                                editable: paramRs.getdata("editable") === "True"
                            });
                        ...
                        $$('#verCliente')[0].onclick = function (event) { verEntidad(event, nvFW.pageContents.nro_entidad, rs.getdata("tipdoc"), rs.getdata("nrodoc"), rs.getdata("tipocli"), rs.getdata("razon_social")); };
                    }
        ...
        function mostrarVinculos() {
            if (!frame.vinculo.cargado) {
                var url = '/FW/entidades/vinculos/ent_vinculos_listar.aspx'
                //if ((nvFW.pageContents.nro_entidad > 0) && typeof (filtro_vinculosXML == "undefined")) {
                    //Entidad de Nv
                    url += "?nro_entidad=" + nvFW.pageContents.nro_entidad
                //}
                //else {
                    //IBS
                    //url += "?url_ver_vinculo=cargar_cliente.aspx"
        ...
        function mostrarArchivos(reload) {
            if (!frame.archivo.cargado || reload) {
                ...
                if (rsAl.recordcount >= 1) {
                    ...
                    frame.archivo.elemento.src = '/FW/archivo/mostrar_def_archivos.aspx?nro_archivo_id_tipo=2&nro_def_archivo=' + _nro_def_archivo + '&id_tipo=' + (nvFW.pageContents.nro_entidad != 0 ? nvFW.pageContents.nro_entidad : _nro_entidad)
                    frame.archivo.cargado = true
                }
                else {
                    nvFW.confirm("Esta entidad no posee una definición de archivos<br>¿Desea crearla?",
                        {
                            okLabel: 'Si',
                            cancelLabel: 'No',
                            onOk: function (win) {
        ...
        function mostrarContactos() {
            if (!frame.contacto.cargado) {
                if (nvFW.pageContents.nro_entidad > 0)
                    frame.contacto.elemento.src = '/FW/entidades/contactos/Contacto_ABM.aspx?nro_entidad=' + nvFW.pageContents.nro_entidad
                else
        ...
        function verEntidad(evento, nro_entidad, tipdoc, nrodoc, tipocli, nombre) {
            var url_destino = "/voii/cargar_cliente.aspx"
            if (nro_entidad > 0) {
                //Entidad de Nv
                url_destino += "?nro_entidad=" + nro_entidad + "&tipdoc=" + tipdoc + "&nrodoc=" + nrodoc + "&tipocli=" + tipocli + "&titulo=" + nombre
            }
            else {
                //IBS
                url_destino += "?tipdoc=" + tipdoc + "&nrodoc=" + nrodoc + "&tipocli=" + tipocli + "&titulo=" + nombre
            }
        ...
        function editarEntidad() {
            ...
            if (nvFW.pageContents.nro_entidad) {
                var win_entidad_abm = window.top.nvFW.createWindow({
                    url: '/FW/entidades/entidad_abm.aspx?nro_entidad=' + nvFW.pageContents.nro_entidad,
...
<body onload="window_onload()" onresize="window_onresize()" style="width: 100%; height: 100%; overflow: hidden;">
    ...
    <div  id="datos_cliente">
        ...
        <script type="text/javascript">
            var pMenu = new tMenu('divMenuPrincipal', 'pMenu');
            ...
            <tr>
                <td>
                    ...
                    <% if tipocli = 1 Then %>
                        ...
                        <tr>
                            <td width="20px"><img id="verCliente" title="Ver Cliente" alt="Ver" src="../FW/image/icons/ver.png" style="cursor:pointer" /> </td>
    ...
    <script type="text/javascript">
        ...
        Menus["vMenu"].CargarMenuItemXML("<MenuItem id='3'><Lib TipoLib='offLine'>DocMNG</Lib><icono>contactos</icono><Desc>Contactos</Desc><Acciones><Ejecutar Tipo='script'><Codigo>mostrarContactos()</Codigo></Ejecutar></Acciones></MenuItem>")
        Menus["vMenu"].CargarMenuItemXML("<MenuItem id='4'><Lib TipoLib='offLine'>DocMNG</Lib><icono>vinculos</icono><Desc>Vínculos</Desc><Acciones><Ejecutar Tipo='script'><Codigo>mostrarVinculos()</Codigo></Ejecutar></Acciones></MenuItem>")
        ...
        //Menus["vMenu"].CargarMenuItemXML("<MenuItem id='5'><Lib TipoLib='offLine'>DocMNG</Lib><icono>prestamo</icono><Desc>Prestamos</Desc><Acciones><Ejecutar Tipo='script'><Codigo>mostrarPrestamos()</Codigo></Ejecutar></Acciones></MenuItem>")
