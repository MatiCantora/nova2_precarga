...
<%
    ...
    If accion.ToLower() = "capturar" Then
        ...
        If nro_com_estado <> "" Then
            cmd.addParameter("@nro_com_estado", ADODB.DataTypeEnum.adVarChar, , , nro_com_estado)
        End If

        If nro_com_tipo <> "" Then
            cmd.addParameter("@nro_com_tipo", ADODB.DataTypeEnum.adVarChar, , , nro_com_tipo)
        End If
    ...
    'Filtros Comentarios Pendientes
    Me.contents("filtro_pendientes") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verCom_registro_pizarra'><campos>COUNT(*) AS cant_pendientes</campos><orden></orden><filtro><pizarra_valor type='igual'>'" & op.operador.ToString() & "'</pizarra_valor></filtro></select></criterio>")
    ...
    Me.contents("filtro_comentarios_pendientes") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verCom_registro_pizarra'><campos>nro_com_grupo, nro_com_estado, id_tipo, nro_com_id_tipo, fecha, comentario, operador, nro_com_tipo, com_tipo, com_estado, nombre_operador, nro_registro, bloqueado, bloq_operador</campos><filtro><pizarra_valor type='igual'>'" & op.operador.ToString() & "'</pizarra_valor></filtro><orden>fecha desc</orden></select></criterio>")
    Me.contents("filtro_comentarios") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verCom_registro'><campos>nro_com_grupo, nro_com_estado, id_tipo, nro_com_id_tipo, fecha, comentario, operador, nro_com_tipo, com_tipo, com_estado, nombre_operador, nro_registro, bloqueado, bloq_operador</campos><filtro></filtro><orden>fecha desc</orden></select></criterio>")
    'Me.contents("filtro_comentarios") = nvFW.nvXMLSQL.encXMLSQL("<criterio><select vista='verCom_registro'><campos>nro_com_grupo, nro_com_estado, id_tipo, nro_com_id_tipo, fecha, comentario, operador, nro_com_tipo, com_tipo, com_estado, nombre_operador, nro_registro, contact, user_contact_type, bloqueado, bloq_operador, titulo, nombre_aspx</campos><filtro>[<nro_com_id_tipo type='in'>%nro_com_id_tipo?%</nro_com_id_tipo>][<id_tipo type='in'>%id_tipo?%</id_tipo>]</filtro><orden>fecha desc</orden></select></criterio>")

    Me.contents("filtroComTipo") = nvXMLSQL.encXMLSQL("<criterio><select vista='com_tipos'><campos>nro_com_tipo as id, com_tipo as [campo] </campos><orden>[campo]</orden><filtro><nro_com_tipo type='sql'>dbo.rm_tiene_permiso('permisos_com_tipo',nro_permiso) = 1</nro_com_tipo></filtro></select></criterio>")
    Me.contents("filtroComTipo2") = nvXMLSQL.encXMLSQL("<criterio><select vista='com_tipos'><campos>nro_com_tipo as id, com_tipo as [campo] </campos><orden>[campo]</orden><filtro></filtro></select></criterio>")
    Me.contents("filtroComEstados") = nvXMLSQL.encXMLSQL("<criterio><select vista='com_estados'><campos>nro_com_estado as id, com_estado as campo</campos><orden>campo</orden><filtro></filtro></select></criterio>")
    ...
    Me.addPermisoGrupo("permisos_comentarios")
...
<head>
    ...
    <script type="text/javascript">
        ...
        function window_onload() {
            ...
            //Menu Comentarios
            if (!nvFW.tienePermiso('permisos_comentarios', 9)) {    //menucoment
                $('tbComentario').hide();
            }
            //Boton Pendientes
            if (!nvFW.tienePermiso('permisos_comentarios', 8)) {    //verpend
                $('tbPendientes').hide();
            } else {
            ...
            if (!nvFW.tienePermiso('permisos_comentarios', 7)) {    //capt
                $('tbCaptura').hide();
                $('tbCapturar').hide();
                $('tbBloqueados').hide();
            } else {
        ...
        function mostrar_filtros_capturar() {

            if ($('trFiltrosCapturar').style.display == 'none') {
                $('trFiltrosCapturar').show();
                $('imgFiltrosCapturar').src = '/fw/image/mnusvr/menos.gif'
            } else {
                $('trFiltrosCapturar').hide();
                $('imgFiltrosCapturar').src = '/fw/image/mnusvr/mas.gif'
            }
        }
        ...
        function buscarComentario() {
            ...
            if (campos_defs.get_value("fecha_desde") == "") {
                ...
                winFramePrincipal.frameElement.onload = function () {
                    winFramePrincipal.frameElement.onload = undefined
        ...
        function cargarPendientes() {
            ...
            winFramePrincipal.frameElement.onload = function (win) {
                winFramePrincipal.frameElement.onload = undefined
                ObtenerVentana("frame_consulta").nvFW.exportarReporte({
        ...
        function capturar_comentario() {
            top.nvFW.error_ajax_request('buscar.aspx', {
                parameters: {
                    accion: 'capturar',
                    nro_com_estado: campos_defs.get_value('nro_cap_estados'),
                    nro_com_tipo: campos_defs.get_value('nro_cap_tipo1')
                ...
                onFailure: function (err) {debugger},
                bloq_msg: 'Cargando..'
            })
        }
...
<body onload="window_onload()" style='width: 100%; height: 100%; overflow: hidden;'>
    <form name="frmBuscar" id="frmBuscar" style="margin: 0;" autocomplete="off">
        <table class="tb1" cellpadding="0" cellspacing="0">
            <tr class="tbLabel0">
                <td style="text-align: center; font-weight: bold !important;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Búsqueda Cliente</td>
                        ...
                        <td>
                            <script>
                                campos_defs.add('nro_com_tipo', {
                                    enDB: false,
                                    filtroXML: nvFW.pageContents.filtroComTipo2,
                                    nro_campo_tipo: 2,
                                    filtroWhere: "<nro_com_tipo type='in'>%campo_value%</nro_com_tipo>"
                                });
                            </script>
                        </td>
                    </tr>
    ...
    <%--Busqueda de capturas--%>
    <table id="tbCaptura" class="tb1" cellpadding="0" cellspacing="0">
        <tr class="tbLabel0">
            <td style="text-align: center; font-weight: bold !important; background-color: silver !important;"><span style="float: left">
                <img src="/fw/image/mnusvr/mas.gif" border="0" align="absmiddle" hspace="1" id="imgFiltrosCapturar" onclick="mostrar_filtros_capturar()"></span>Captura</td>
        </tr>
        <tr style="display: none" id="trFiltrosCapturar">
            <td>
                ...
                <table class="tb1">
                    ...
                    <tr>
                        <td>
                            <script>
                                campos_defs.add('nro_cap_tipo1', {
                                    enDB: false,
                                    filtroXML: nvFW.pageContents.filtroComTipo,
                                    nro_campo_tipo: 1,
                                    filtroWhere: "<nro_com_tipo type='in'>%campo_value%</nro_com_tipo>"
                    ...
                    <tr>
                        <td>
                            <script>
                                campos_defs.add('nro_cap_estados', {
                                    enDB: false,
                                    filtroXML: nvFW.pageContents.filtroComEstados,
                                    nro_campo_tipo: 1,
                                    filtroWhere: '<nro_com_estado type="igual">%campo_value%</nro_com_estado>'
                                })
                                //campos_defs.set_value('nro_com_estados', 2)
                            </script>
                        </td>
                    ...
                    <tr>
                        <td>
                            <script>
                                campos_defs.add('fecha_cap_desde', {
                                    enDB: false,
                                    nro_campo_tipo: 103,
                                    filtroWhere: "<fecha type='mas'>convert(datetime, '%campo_value%', 103)</fecha>"
                                });
