...
/*    strHTML += "<td>Página: <select id='cbPage' name='cbPage' onchange=campos_head.pagina_cambiar(this.selectedIndex+1)>"
    for (var i = 1; i <= this.PageCount; i++)
        {
        var strSelected = ''
        if (this.AbsolutePage == i)
    ...
    strHTML += "<td>&nbsp;Página:&nbsp;<input type='number' id='txtPage' name='txtPage' style='width: 3em; text-align: right' maxlength='4' value=" + this.AbsolutePage + " onkeypress='return valDigito(event)' />  <input type='button' value='Ir' style='cursor:pointer' onclick=campos_head.pagina_cambiar($('txtPage').value) /> de " + this.PageCount + " </td>"
