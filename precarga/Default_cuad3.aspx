...
<head>
    ...
    function Ajustar_ventana(height)
    {
    ...
    function selVendedor_onclick() {
        ...
        if ((permisos_precarga & 1) <= 0)
            {
            alert('No posee permisos para seleccionar el vendedor')
            return
            }
    ...
    function CUAD_genera_info()
    {
        user = "rbilotta_7"
        password = "sbilotta"
        clave_sueldo = "20249474453"
        var oXML1 = new tXML();
        oXML1.async = false
        if (oXML1.load('/precarga/CUAD/GetXML.aspx?accion=cuad_login&criterio=<criterio><user>' + user + '</user><password>' + password + '</password><clave_sueldo>' + clave_sueldo + '</clave_sueldo></criterio>')) {
            debugger
            strXML = XMLtoString(oXML1.xml)
        }
    }
    ...
    function btnCancela_onClick(i)
    {
        ...
        importe_mano = 0
        importe_mano = importe_neto - gastoscomerc - TotalCancelaciones
        $('strEnMano').innerHTML = ''
...
<%--                    <object data="/fw/image/nvLogin/nova.svg" style="width: 10.54em; height: 4.5em" type="image/svg+xml">
    ...
    <div id="divProducto" style="display:none">  
        ...
        <table class='tb1' style='width:100%'>
            ...
            <tr><td class='Tit1' style="width:15%">En mano:</td>
                <td style="width:85%;text-align:right" nowrap="true" colspan="3"><span id="strEnMano"></span></td>
            </tr>
        </table>
        </div>
    </div>
