...
<% 
    ...
    Dim strSql = "select cr.nro_tabla,isnull(case when p.valor_char is null then cast(p.valor_float as varchar(10)) else p.valor_char end,'') as tipos_validaciones from vercreditos cr left outer join tabla_parametros p on cr.nro_tabla=p.nro_tabla and  p.parametro='TIPOS.VALIDACION' where cr.nro_credito =" & crparam.ToString
