  ...
  <xsl:output method="html" encoding="ISO-8859-1" omit-xml-declaration="yes" />
    ...
    <xsl:template match="z:row">
        ...
        <tr>
            ...
            <td class="ocultar_mobile" style='text-align: left; width:11%;'>
                <xsl:value-of select="@id_calificacion"/>
            </td>
            <td class="requerido" style='text-align: left;'>
              <xsl:value-of select="foo:FechaToSTR(string(@fecha))"/>
            </td>
            <td class="ocultar_mobile" style='text-align: left; width:12%;'>
              <xsl:value-of select="@cuil"/>
            </td>
            <td class="requerido" style='text-align: left;'>
              <xsl:value-of select="@nro_docu"/>
            </td>
            <td class="ocultar_mobile" style='text-align: left; width:18%;'>
              <xsl:value-of select="@apellido"/>
            </td>
            <td class="ocultar_mobile" style='text-align: left; width:24%;'>
              <xsl:value-of select="@nombres"/>
            </td>
          
            <td class="requerido" style='text-align: left;'>
              <xsl:choose>
                <xsl:when test="@dictamen = 'Aprobado'">
                  <xsl:attribute name="style">color: #038a0c;</xsl:attribute>
                </xsl:when>
                <xsl:when test="@dictamen = 'Manual'">
                  <xsl:attribute name="style">color: #e2b00a;</xsl:attribute>
                </xsl:when>
                <xsl:when test="@dictamen = 'Rechazar'">
                  <xsl:attribute name="style">color: #d40202;</xsl:attribute>
                </xsl:when>
                <xsl:otherwise>
            ...
            <td style='width: 2% !important;'><xsl:attribute name='id'>tdScroll<xsl:value-of select="$pos"/></xsl:attribute>&#160;</td>
        </tr>
    </xsl:template>
</xsl:stylesheet>