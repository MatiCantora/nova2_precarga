  ...
  <msxsl:script language="javascript" implements-prefix="foo">
	...
	<xsl:template match="/">
		...
		<head>
            ...
            <title>HTML FW Permisos verPerfiles acceso lineal</title>
           ...
           <table class="tb1 highlightOdd highlightTROver layout_fixed" id="tbCabe">
                 <tr class="tbLabel">
                   <td nowrap='nowrap' style='width:15%;'>
                     <script>
                       campos_head.agregar('(Nro. Perfil) ', true, 'tipo_operador'); campos_head.agregar('Perfil', true, 'tipo_operador_desc')
                     </script>
                   </td>
                   <td nowrap='nowrap' style='width:30%;space-write:noprap'>
                     <script>
                       campos_head.agregar('(Nro. Permiso Grupo) ', true, 'nro_permiso_grupo'); campos_head.agregar('Permiso Grupo', true, 'permiso_grupo')
                     </script>
                   </td>
                   <td nowrap='nowrap'>
                     <script>
    ...
    <tr>
          ...
          <xsl:if test="string(@tipo_operador) != ''">
            ...
            <!--<img>
              <xsl:attribute name="onclick">
                parent.abm_perfil('<xsl:value-of select="@tipo_operador"/>','<xsl:value-of select="@tipo_operador_desc"/>')
              </xsl:attribute>
        ...
        <td>
          <xsl:if test="string(@nro_permiso_grupo) != ''">
            ...
            <xsl:attribute name="style">space-white:nowrap;width:30%;text-decoration:underline;color:blue;cursor:hand;cursor:pointer</xsl:attribute>
            <xsl:attribute name="onclick">parent.permiso_mostrar('lineal', <xsl:value-of select="@tipo_operador"/>)</xsl:attribute>
            <xsl:value-of select="@nro_permiso_grupo"/> - <xsl:value-of select="@permiso_grupo"/>
          </xsl:if>
        </td>
      <td>
        <xsl:if test="string(@nro_permiso) != ''">
          ...
          <xsl:attribute name="style">space-white:nowrap;text-decoration:underline;color:blue;cursor:hand;cursor:pointer</xsl:attribute>
          <xsl:attribute name="onclick">
            parent.permiso_mostrar('lineal', <xsl:value-of select="@tipo_operador"/>)
          </xsl:attribute>
          <xsl:value-of select="@nro_permiso"/> - <xsl:value-of select="@Permitir"/>
        </xsl:if>
      </td>
      <td>
        <xsl:attribute name="style">width:15%;space-white:nowrap</xsl:attribute>
        <xsl:if test="string(@path) != 'No Asignado'">
          <xsl:attribute name="style">space-white:nowrap;width:15%;text-decoration:underline;color:blue;cursor:hand;cursor:pointer</xsl:attribute>
          <xsl:attribute name="onclick">
            parent.permiso_mostrar('arbol', <xsl:value-of select="@tipo_operador"/>)
          </xsl:attribute>
        </xsl:if>
        <xsl:attribute name='title'>
