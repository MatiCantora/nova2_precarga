  ...
  <xsl:output method="html" version="1.0" encoding="Latin-1" omit-xml-declaration="yes"/>
        ...
        <html>
                    <head>
                      ...
                      <script type="text/javascript"  language="javascript" >
                          ...
                          function editar_img_depthcolor(nro_depthcolor)
                          {
                            parent.img_depthcolor_abm(nro_depthcolor)
                          }

                          function eliminar_img_depthcolor(nro_depthcolor)
                          {
                            parent.eliminar_img_depthcolor(nro_depthcolor)
                          }
                        ...
                        <table width="100%" class="tb1" id="tbCabe">
                               <tr class="tbLabel">
                                  <td style='text-align: center; width:10%'>
                                     <script>campos_head.agregar('ID', true, 'nro_depthcolor')</script>
                                  </td>
                                  <td style='text-align: center; width:23%'>
                                     <script>campos_head.agregar('Depthcolor', true, 'depthcolor')</script>
                                  </td>
    ...
    <xsl:template match="z:row">
        ...
        <tr>
            ...
            <td style='text-align: center; width:10%'>
                <xsl:value-of select="@nro_depthcolor"/>
            </td>
            <td style='text-align: left; width:23%'>
              <xsl:attribute name='title'>
                <xsl:value-of select="@depthcolor"/>
              </xsl:attribute>
              <xsl:choose>
                <xsl:when test="string-length(@depthcolor) &#62; 70">
                  <xsl:value-of select="substring(@depthcolor,1,70)"/>...
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="@depthcolor"/>
                </xsl:otherwise>
            ...
            <td style='text-align: center; width:3%'>
              <a>
                <xsl:attribute name="onclick">editar_img_depthcolor('<xsl:value-of select="@nro_depthcolor"/>')</xsl:attribute>
                <img title="Editar Color" src="/fw/image/icons/editar.png" style="cursor:pointer" border="0"/>
              </a>
            </td>
            <td style='text-align: center; width:3%'>
              <a>
                <xsl:attribute name="onclick">eliminar_img_depthcolor('<xsl:value-of select="@nro_depthcolor"/>')</xsl:attribute>
                <img title="Eliminar Color" src="/fw/image/icons/eliminar.png" style="cursor:pointer" border="0"/>
              </a>
            </td>
