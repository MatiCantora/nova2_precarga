...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
                ...
                <head>
                  ...
                  <script type="text/javascript" language='javascript' src="/FW/script/tcampo_head.js"></script>
                    ...
                    <script type="text/javascript"  language="javascript" >
                      ...
                      function mostrar_mov(pos, nro) {
                        if ($('movi'+ nro).style.display == 'none') {
            
                          $('icono'+ nro).src = '/FW/image/icons/menos.gif'
                          $('movi'+ nro).style.display = 'block'
                          parent.filtrar()
                          parent.mostrar_mov($('entidades'+ nro), nro)
                        } else {
            
                          $('icono'+ nro).src = '/FW/image/icons/mas.gif'
                          $('movi'+ nro).style.display = 'none'
                        }
                      }                           
                        
                      function exportarExcelOp(nro) 
                      {
                        parent.exportarExcelOp(nro)
                      }
                    ...
                    <table width="100%" class="tb1" id="tbCabe">
                            <tr class="tbLabel">
                              ...
                              <td style='text-align: center; width:3%'>
                                  <script>campos_head.agregar('Nro.', true, 'id_mov')</script>
                              </td>
    ...
    <xsl:template match="z:row">
        ...
        <xsl:variable name="id_mov" select="@id_mov"/>
      ...
      <tr>
            ...
            <td style='text-align:center; width:2%'>
              <xsl:attribute name="onclick">
                mostrar_mov(this, <xsl:value-of select="@id_mov"/>)
              </xsl:attribute>
              <img style="cursor:pointer" id="icono{$id_mov}" src="/FW/image/icons/mas.gif" />
            </td>
            <td style='text-align:center; width:3%'>
        ...
        <!--<td style='text-align:center; width:5%'>
            ...
            <td style='width:4%'>
              <xsl:choose>
                ...
                <xsl:attribute name='style'>text-align: center; width:4%; color:Green !Important;</xsl:attribute>
                  Confirmado
                </xsl:when>
                <xsl:when test='@estado_mov = "P"'>
                <xsl:attribute name='style'>text-align: center; width:4%; color:Blue !Important;</xsl:attribute>
                  Pendiente
                </xsl:when>
                <xsl:when test='@estado_mov = "A"'>
                <xsl:attribute name='style'>text-align: center; width:4%; color:Red !Important;</xsl:attribute>
                  Anulado
                </xsl:when>
              </xsl:choose>           
            </td>
            ...
            <td style='text-align: center; width:3%'>
              <a>
                <xsl:attribute name="onclick">exportarExcelOp('<xsl:value-of select="@id_mov"/>')</xsl:attribute>
                <img title="Exportar Movimiento" src="/fw/image/icons/excel.png" style="cursor:pointer" border="0"/>
              </a>
            </td>
        ...
        <tr>
          <td colspan='11' >
            <iframe src="/fw/enBlanco.htm" name="movi{$id_mov}" id="movi{$id_mov}" scope='row' style='width:100%; height: 300px; display:none; border: none' />
          </td>
        </tr>      
    </xsl:template>
</xsl:stylesheet>