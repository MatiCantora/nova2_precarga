    ...
    <html>
        ...
        <script type="text/javascript"  language="javascript" >
          <xsl:comment>
            <![CDATA[ 
                      ...
                      function editar_vinculos(id,tipo,grupo,modo)
                      {
                        parent.editar_tipo_vinculo(id,tipo,grupo,modo)
                      }
                      ...
                      function eliminar_vinculos(id)
                      {
                        parent.eliminar_tipo_vinculo(id)
                      }
      ...
      <body onload="window_onload()" onresize="window_onresize()" style="width:100%;height:100%;overflow:hidden">
        <table width="100%" class="tb1" id="tbCabe">
          <tr class="tbLabel">
            <td style='text-align: center; width:10%' nowrap='true'>
              <script>campos_head.agregar('Nro. Tipo', true, 'nro_vinc_tipo')</script>
            </td>
            <td style='text-align: center;' nowrap='true'>
              <script>campos_head.agregar('Descripción', true, 'vinc_tipo')</script>
            </td>
            <td style='text-align: center; width:13%' nowrap='true'>
              <script>campos_head.agregar('Nro. Grupo', true, 'nro_vinc_grupo')</script>
            </td>
            <td style='text-align: center; width:25%' nowrap='true'>
              <script>campos_head.agregar('Grupo', true, 'vinc_grupo')</script>
            </td>
        ...
        <div id="divDetalle" style="width:100%;overflow:auto">
          <table class="tb1 highlightOdd highlightTROver layout_fixed" id="tbDetalle" style="width:100%;">
  ...
  <xsl:template match="z:row">
    ...
    <tr>
      ...
      <td style='text-align: center; width:10%'>
        <xsl:value-of select="@nro_vinc_tipo"/>
      </td>
      <td>
        ...
        <xsl:choose>
          <xsl:when test="string-length(@vinc_tipo) &#62; 70">
            <xsl:value-of select="substring(@vinc_tipo,1,70)"/>...
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="@vinc_tipo"/>
          </xsl:otherwise>
      ...
      <td style='text-align: center; width:13%'>
        <xsl:value-of select="@nro_vinc_grupo"/>
      </td>
      <td style='text-align: left; width:25%'>
        <!--<xsl:if test="string-length(@vinc_grupo) > 18">
          <xsl:attribute name='title'>
      ...
      <td style='text-align: center; width:5%'>
        <a>
          <xsl:attribute name="onclick">
            editar_vinculos('<xsl:value-of select="@nro_vinc_tipo"/>','<xsl:value-of select="@vinc_tipo"/>','<xsl:value-of select="@nro_vinc_grupo"/>','E')
          </xsl:attribute>
      ...
      <td style='text-align: center; width:5%'>
        <a>
          <xsl:attribute name="onclick">
            eliminar_vinculos('<xsl:value-of select="@nro_vinc_tipo"/>')
          </xsl:attribute>
          <img title="Eliminar Definición de Archivos" src="/fw/image/icons/eliminar.png" style="cursor:pointer" border="0"/>
        </a>
      </td>
      <!--<td style="width:1.7%"><xsl:attribute name='id'>tdScroll<xsl:value-of select="$pos"/></xsl:attribute>&#160;&#160;</td>-->
    </tr>
  </xsl:template>
</xsl:stylesheet>