							...
							<span>
								<xsl:attribute name="style">
									...
									<img src='/fw/image/comentario/comentario.png' style='cursor:pointer' border='0' align='absmiddle' hspace='1'>
										<xsl:attribute name='onclick'>return parent.ABMRegistro('<xsl:value-of select='@nro_entidad' />', '<xsl:value-of select='@id_tipo' />', <xsl:value-of select='@nro_com_id_tipo' />, <xsl:value-of select='@nro_registro' />, <xsl:value-of select='@nro_com_tipo' />, <xsl:value-of select='@nro_com_estado' />, '<xsl:value-of select='@bloq_menu'/>')</xsl:attribute>
									</img>
								&#160;<u>
