  ...
  <msxsl:script language="javascript" implements-prefix="foo">
                   ...
                   function window_onresize()
					          {
					              try
					              {
					               ...
					               var total = body_height - tbCabe_height - dif // - divPie_height 
 					               ...
 					               $('table_container_left').setStyle({height: total + 'px'})
                         $('table_container_right').setStyle({height: total + 'px'})
                         
                        //$('divRow').setStyle({height: total + 'px'})
                         // campos_head.resize("tbCabe", "tbRow")
                          
                          campos_head.resize("tbCabe", "left_table")
                          campos_head.resize("tbCabe", "right_table")
					              }
					               catch(e){}
					          } 
                    ...
                    var leftDiv = document.querySelector('#table_container_left');
                    var rightDiv = document.querySelector('#table_container_right');
                    ...
                    function ver_editor(id) 
                    {
                      ...
                      win_editar = parent.nvFW.createWindow({ className: 'alphacube',
                                title: '<b>Valor: ' + $(id).value.substring(0,20) +'</b>',
                                minimizable: false,
                                maximizable: false,
                                draggable: true,
                                resizable: false,
                      ...
                      var html = "<html><head></head><body style='width: 100%; height: 100%;'><form><table class='tb1'><tr><td align='center'><textarea readonly='readonly' style='overflow: auto; resize: none; width: 100%;' rows='5' cols='1'>" + texto + "</textarea></td></tr><tr><td align='right'></td></tr></table></form></body>";
    ...
    <body onload="window_onload()" onresize="window_onresize()" style="width:100%;height:100%;overflow:hidden">
         <table class="tb1 highlightOdd highlightTROver layout_fixed" id="tbCabe" >
					 <tr class="tbLabel">
             <td>
               <div id="table_container_left_cabe" style="width:20%">
                 <table id="left_table_cabe">
                   <tr>
                     <td style="width:5%; cursor:pointer;text-align:center"><script>campos_head.agregar_exportar()</script></td>
                     <td class='locked_left'><script>campos_head.agregar('Parametro', true, 'parametro')</script></td>
                   </tr>
                 </table>
               </div>
               <div id="table_container_right_cabe" style="width:80%" >
                 <table id="right_table_cabe">
                   <tr>
                    <td style="width:20%"><script>campos_head.agregar('Etiqueta', true, 'etiqueta')</script></td>
						        <td style="width:20%"><script>campos_head.agregar('Tipo Dato', true, 'tipo_dato')</script></td>
         ...
         <div id="divRow" style="width:100%;overflow:hidden">
         <div id="table_container_left" style="width:20%">
            <table id="left_table" class="tb1 highlightOdd highlightTROver">
              <xsl:apply-templates select="xml/rs:data/z:row" mode="igual_parametro" />
            </table>
         </div>
         <div id="table_container_right" style="width:80%" >
            <table id="right_table" class="tb1 highlightOdd highlightTROver">
                <xsl:apply-templates select="xml/rs:data/z:row" mode="distinto_parametro" />
            </table>
         </div>
	...
	<xsl:template match="z:row" mode ="igual_parametro">
	  <tr>
       ...
       <td style="width:25%" class='locked_left'>
	...
	<xsl:template match="z:row"  mode ="distinto_parametro">
 	
	  <tr>
       <xsl:variable name="pos" select="position()"/>
       <xsl:attribute name="id">tr_ver<xsl:value-of select="$pos"/>1</xsl:attribute>
	    <xsl:attribute name="style">cursor:hand;cursor:pointer</xsl:attribute>
      <td style="width:20%">
		  ...
		  <td>
		     <xsl:if test='string(@valor) != ""'>
			              ...
			              <input><xsl:attribute name="id">valor<xsl:value-of select="$pos"/></xsl:attribute><xsl:attribute name="type">hidden</xsl:attribute></input>
                    <script type="text/javascript" language="javascript">
                    <xsl:comment>$('valor<xsl:value-of select="$pos"/>').value = '<xsl:value-of select="foo:replace(string(@valor),'\','\\')" />'</xsl:comment>
         </script>
		  </td> 
		</tr>
	</xsl:template>
