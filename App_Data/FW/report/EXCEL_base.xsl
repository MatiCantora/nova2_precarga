  ...
  <xsl:template match="/">
	...
	<xsl:template match="s:AttributeType" mode="titulo">
		<Cell ss:StyleID="cell_cab">
			<Data ss:Type="String">
				<xsl:choose >
					<xsl:when test="@name = 'strNombreCompleto'">Apellido y nombre</xsl:when>
					<xsl:when test="@name = 'strDomicilioCompleto'">Domicilio</xsl:when>
					<xsl:when test="@name = 'nro_credito'">Nº crédito</xsl:when>
					<xsl:when test="@name = 'importe_neto'">$ Neto</xsl:when>
					<xsl:when test="@name = 'importe_documentado'">$ Documentado</xsl:when>
					<xsl:when test="@name = 'importe_bruto'">$ Bruto</xsl:when>
					<xsl:when test="@name = 'importe_cuota'">$ Cuota</xsl:when>
					<xsl:otherwise>
    ...
    <xsl:if test="foo:test_ultima_fila(position(), 65536)">
      ...
      <xsl:text disable-output-escaping="yes">&lt;Table x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="60"&gt;&lt;Row ss:StyleID="s22"&gt;</xsl:text>
        <xsl:apply-templates select="/xml/s:Schema/s:ElementType/s:AttributeType" mode="titulo"/>
