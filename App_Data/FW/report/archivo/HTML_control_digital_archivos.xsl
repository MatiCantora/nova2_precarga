        ...
        <script type="text/javascript">
          <![CDATA[ 				
            ...
            function fisicoMasivo() {
                var cont = 0
                var doc = $$("#tbDetalle tbody tr td")[cont]
                var desc = ""
                var id = ""
                
                var a = []
                while (cont < 1000) {
                    doc = $$("#tbDetalle tbody tr td")[cont]
                    desc = $$("#tbDetalle tbody tr td")[cont + 3]
                    id = $$("#tbDetalle tbody tr td")[cont + 1]
        ...
        <table class="tb1" id="tbCabe" >
          <tr class="tbLabel">
            <td style='width:3%; padding:0;'>
              <!--<input type="button" value="Físico" onclick="fisicoMasivo()" style="width:100%;cursor:pointer;"/>-->
  ...
  <xsl:template match="z:row">
    ...
    <tr>
      ...
      <td style='width:3%; text-align:center;'>
        <xsl:choose>
          <xsl:when test="@f_nro_ubi != '' ">
            <input type="checkbox" value="off" style="display:none" checked="false !Important" />
          </xsl:when>
          <xsl:otherwise>
      ...
      <td style='width:7%; text-align: left;'>
        <xsl:choose>
          <xsl:when test="format-number(@nro_archivo,'000000000') = 'NaN'">
            <xsl:attribute name="style">width:7%;text-align: center;</xsl:attribute>
            -
          </xsl:when>
          <xsl:otherwise>
