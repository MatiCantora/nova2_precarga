				...
				<table class="tb1 layout_fixed" id="tbCabecera">
					<tr class='tbLabel0'>
						...
						<td style='width:9%; nowrap:true; text-align:center'>
							...
							<script>campos_head.agregar('Tipo Alarma', true,'TIPO_ALARMA')</script>
						</td>
						...
						<td style='width:5%; nowrap:true; text-align:center'>
							<xsl:attribute name="title">Tipo de perí­odo</xsl:attribute>
							<script>campos_head.agregar('Tipo peri­odo', true,'tipo_periodo')</script>
						</td>
						<td style='width:5%; nowrap:true; text-align:center'>
							<xsl:attribute name="title">Tipo de tope</xsl:attribute>
	...
	<xsl:template match="z:row">
		<tr>
			...
			<td style='width: 9%; nowrap:true;'>
				<xsl:attribute name="title">
					<xsl:value-of  select="@TIPO_ALARMA" />
				</xsl:attribute>
				<xsl:value-of  select="@TIPO_ALARMA" />
			</td>
			<td style='width: 5%; nowrap:true'>
				<xsl:attribute name="title">
					<xsl:value-of  select="@mes_calc_desc" />
				</xsl:attribute>
				<xsl:value-of  select="@mes_calc_desc" />
			</td>
			<td style='width: 4%; text-align:right; nowrap:true'>
				<xsl:attribute name="title">
