...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="z:row">
		...
		<xsl:value-of select="string(@DmcCodT)" />
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@TdoCodT)" />
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcPaiCodT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcProCodT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcLocCodT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcCPAT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="foo:limpiar_cadena_domicilio(string(@DmcNomCalT))"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcNroPueT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcPisT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcDepT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcCal1T)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcCal2T)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcParT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcUbiT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcBarT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:value-of select="string(@DmcCodPosT)"/>
		<xsl:text><![CDATA[|]]></xsl:text>
		<xsl:text>&#xD;&#xA;</xsl:text>
	</xsl:template>
</xsl:stylesheet>