...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <html>
      ...
      <body onload="window_onload()"  style="width:100%;height:100%;overflow:hidden">
        <table width="100%" class="tb1" id="tbCabe1">
                ...
                <tr class="tbLabel">
                  ...
                  <td style='text-align: center;width:19%'>Acciones</td>
                  <td style="width:1%">&#160;</td>
                </tr>
              </table>
  ...
  <xsl:template match="z:row">
    ...
    <tr>
      ...
      <td style='text-align: center; width:10%'>
        <xsl:value-of select="foo:FechaToSTR(string(@fecha_desde))"/> 
      </td>
      <td style='text-align: center; width:10%'> 
        <xsl:value-of select="foo:FechaToSTR(string(@fecha_hasta))"/>  
      </td>
      <td style='text-align: center; width:29%'>
        ...
        <xsl:attribute name="style">
          <xsl:choose>
            <xsl:when test="@det_estado = 'Esperando Dependencia'">color:#AAA60F !Important;text-align: left ;cursor:pointer</xsl:when>
            <xsl:when test="@det_estado = 'Pendiente' or @det_estado='Anulado'">color:#ED8714 !Important;text-align: left ;cursor:pointer</xsl:when>
            <xsl:when test="@det_estado ='Iniciado'">color:green !Important;text-align: left ;cursor:pointer</xsl:when>
            <xsl:when test="@det_estado ='Controlado' ">color:blue !Important;text-align: left;  cursor:pointer</xsl:when>
