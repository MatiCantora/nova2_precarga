...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <html>
      <head>
        ...
        <script type="text/javascript">
          <![CDATA[ 				
            ...
            function valueParam(nro_archivo, parametro, def_archivos, documentos, razon_social, tip_doc, param_valor, nro_doc, nro_registro, tdf) {
                var td = $(tdf + nro_registro)
                var name = 'venc_'
                var venc = $(name + nro_registro)
                parent.valueParam(nro_archivo, parametro, def_archivos, documentos, razon_social, tip_doc, param_valor, nro_doc, td, venc )
            }     
      ...
      <body onload="window_onload()" onresize="window_onresize()" style="width:100%;overflow:auto;margin-bottom:0px;padding-bottom:0px" hover="background :rgba(grey, 0.5);">
        <table class="tb1 layout_fixed" id="tbCabe">
          <tr class="tbLabel">
            <td style='width:39px; padding:0; text-align:center'>
            ...
            <td style='text-align:center; width:180px' nowrap='true'>
              <script type="text/javascript">
            ...
            <td style='width:242px; text-align:center' nowrap='true' >
              <script type="text/javascript">
            ...
            <td style='text-align:center; width:100px' nowrap='true' >
              <script type="text/javascript">
                campos_head.agregar('Forzado', 'false', 'reclamable')
              </script>
            </td>
            <td style='width:124px; text-align:center' nowrap='true'>
              <script type="text/javascript">
  ...
  <xsl:template match="z:row">
    ...
    <tr>
      ...
      <td style='width:180px'>
        <xsl:choose>
          <xsl:when test='@Razon_social != ""'>
            <xsl:attribute name='title'>
              <xsl:value-of select="concat(@Razon_social, ' - (', @documento,':', @nro_docu, ')')"/>
            </xsl:attribute>
            <a>
              <xsl:attribute name="onclick">
                abrir_entidad(event,<xsl:value-of select="@nro_archivo_id_tipo"/>,<xsl:value-of select="@id_tipo"/>)
          ...
          <xsl:when test='@Razon_social_sol != ""'>
            <xsl:attribute name='title'>
              <xsl:value-of select="concat(@Razon_social_sol, ' - (', @documento,':', @cuil, ')')"/>
            </xsl:attribute>
            <a>
              <xsl:attribute name="onclick">
                abrir_entidad(event,<xsl:value-of select="@nro_archivo_id_tipo"/>,<xsl:value-of select="@id_tipo"/>)
      ...
      <td style='text-align: center; width:124px'>
        <xsl:attribute name="id">docu_<xsl:value-of select="$pos"/></xsl:attribute>
        <xsl:attribute name='title'>
          <xsl:value-of select="@fecha_docu" />
        </xsl:attribute>
                <xsl:choose>
          <xsl:when test='@nro_archivo_id_tipo = 2'>        
            <xsl:attribute name="onclick">
              valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_fe_documento','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social"/>','<xsl:value-of select="@documento"/>','<xsl:value-of select="foo:FechaToSTR(string(@fecha_docu))" />','<xsl:value-of select="@nro_docu"/>','<xsl:value-of select="$pos"/>','docu_')
            </xsl:attribute>
          </xsl:when>
          <xsl:when test='@nro_archivo_id_tipo = 1'>          
            <xsl:attribute name="onclick">
              valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_fe_documento','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social_sol"/>','<xsl:value-of select="@documento"/>','<xsl:value-of select="foo:FechaToSTR(string(@fecha_docu))" />','<xsl:value-of select="@cuil"/>','<xsl:value-of select="$pos"/>','docu_')
            </xsl:attribute>
          </xsl:when>
          <xsl:otherwise>
          </xsl:otherwise>
        </xsl:choose>
        <xsl:attribute name="style">cursor:pointer; text-decoration: underline; text-align: center !Important; width:124px</xsl:attribute>
        <xsl:value-of select="@fecha_docu" />
      </td>
      <td style='text-align: center; width:124px'>
        <xsl:attribute name="id">forz_<xsl:value-of select="$pos"/></xsl:attribute>
        <xsl:attribute name='title'>
        ...
        <xsl:choose>
          <xsl:when test='@nro_archivo_id_tipo = 2'>        
            <xsl:attribute name="onclick">
              valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_fe_forzar','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social"/>','<xsl:value-of select="@documento"/>','<xsl:value-of select="foo:FechaToSTR(string(@fecha_forzada))" />','<xsl:value-of select="@nro_docu"/>','<xsl:value-of select="$pos"/>','forz_')
            </xsl:attribute>
          </xsl:when>
          <xsl:when test='@nro_archivo_id_tipo = 1'>          
            <xsl:attribute name="onclick">
              valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_fe_forzar','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social_sol"/>','<xsl:value-of select="@documento"/>','<xsl:value-of select="foo:FechaToSTR(string(@fecha_forzada))" />','<xsl:value-of select="@cuil"/>','<xsl:value-of select="$pos"/>','forz_')
            </xsl:attribute>
          </xsl:when>
          <xsl:otherwise>
          </xsl:otherwise>
      ...
      <td style='width:100px'>
        <xsl:attribute name="id">rec_<xsl:value-of select="$pos"/></xsl:attribute>
        <xsl:choose>
          <xsl:when test='@nro_archivo_id_tipo = 2'>
            <xsl:choose>
              <xsl:when test='@reclamable = "RECLAMAR"'>
                <xsl:attribute name="onclick">
                  valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_param_seg','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social"/>','<xsl:value-of select="@documento"/>','RECLAMAR','<xsl:value-of select="@nro_docu"/>','<xsl:value-of select="$pos"/>','rec_')
                </xsl:attribute>
              </xsl:when>
              <xsl:otherwise>
                <xsl:attribute name="onclick">
                  valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_param_seg','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social"/>','<xsl:value-of select="@documento"/>','NO RECLAMAR','<xsl:value-of select="@nro_docu"/>','<xsl:value-of select="$pos"/>','rec_')
                </xsl:attribute>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:when>
          <xsl:when test='@nro_archivo_id_tipo = 1'>
            <xsl:choose>
              <xsl:when test='@reclamable = "RECLAMAR"'>
                <xsl:attribute name="onclick">
                  valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_param_seg','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social_sol"/>','<xsl:value-of select="@documento"/>','RECLAMAR','<xsl:value-of select="@cuil"/>','<xsl:value-of select="$pos"/>','rec_')
                </xsl:attribute>
              </xsl:when>
              <xsl:otherwise>
                <xsl:attribute name="onclick">
                  valueParam(<xsl:value-of select="@nro_archivo"/>,'ar_param_seg','<xsl:value-of select="@def_archivo"/>','<xsl:value-of select="@archivo_descripcion"/>','<xsl:value-of select="@Razon_social_sol"/>','<xsl:value-of select="@documento"/>','NO RECLAMAR','<xsl:value-of select="@cuil"/>','<xsl:value-of select="$pos"/>','rec_')
                </xsl:attribute>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:when>
        ...
        <xsl:attribute name="style">cursor:pointer; text-decoration: underline; text-align: center !Important; width:100px</xsl:attribute>
        <xsl:attribute name='title'>
      ...
      <td style='text-align: center; width:125px'>
        <xsl:attribute name="id">venc_<xsl:value-of select="$pos"/></xsl:attribute>
        <xsl:attribute name='title'>
