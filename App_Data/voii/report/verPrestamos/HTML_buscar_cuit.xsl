...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:comment>
                  ...
                  <![CDATA[
                        function cargarDatosClienteUnico()
                        {
                            ...
                            $frame_buscar.cargarDatosCliente(nro_entidad, tipocli, nrodoc, tipdoc)
                        }
                    ]]>
                </xsl:comment>
              </script>
    ...
    <xsl:template match="z:row">
      ...
      <xsl:variable name="nro_entidad">
        <xsl:choose>
          <xsl:when test="@tiporel >= 0">
            <xsl:value-of select="''"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="string(@nro_entidad)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
        ...
        <tr id="tr_ver{$pos}">
            <td style="width: 25px; text-align: center;">
                <img alt="Seleccionar" src="/FW/image/icons/ver.png" style="cursor: pointer;" title="Ver Entidad [{@tipdoc_desc}: {@nrodoc}]" onclick="return cargarDatosCliente('{$nro_entidad}', {@tipocli}, {@nrodoc}, {@tipdoc})" />
            </td>
