...
<xsl:output method="text" />
	...
	<xsl:template match="/">	
		<xsl:comment>(Cod admin)</xsl:comment>
		<xsl:value-of select="string(/xml/rs:data/z:row/@codAdmin)"/>
		<xsl:comment>(Relleno)</xsl:comment>
		<xsl:value-of select="string('|')" />
		<xsl:comment>(nro de mutual para el prestamo)</xsl:comment>
   ...
   <xsl:comment>(nro credito)</xsl:comment>
   <xsl:value-of select="string(@nroreferencia)" />
	 <xsl:comment>(Relleno)</xsl:comment>
	 <xsl:value-of select="string('|')" />
   <xsl:comment>(Fecha de vencimiento de cuota)</xsl:comment>
   <xsl:value-of select="string(@feVenc) " />
   <xsl:comment>(Relleno)</xsl:comment>
   <xsl:value-of select="string('|')" />
   <xsl:comment>(Importe pagado)</xsl:comment>
