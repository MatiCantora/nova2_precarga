...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    <html>
    <head>
      <title>Listado de configs de Contraseña</title>
      ...
      <script type="text/javascript">
        ...
        {
            parent.editPwdConfig(ids_pwdcfg_id);
        }
        ]]>
      </script>
    </head>
      ...
      <xsl:choose>
        ...
        <xsl:when test="count(xml/rs:data/z:row) = 0">
          <div class="sin-datos">
            ...
            <span class="mensaje">Actualmente no hay configuraciones de Contraseñas.</span>
          </div>
        </xsl:when>
        ...
        <xsl:otherwise>
          ...
          <table class="tb1 layout_fixed" id="tbCabecera">
            <tr class="tbLabel">
              ...
              <td style="text-align: center" title="Configuración de Email">
                <script>campos_head.agregar('Config Password', 'true', 'ids_pwdcfg');</script>
              </td>
  ...
  <xsl:template match="z:row" mode="row1">
    ...
    <tr>
      ...
      <td style="width: 30px; text-align: center;">
        <span class="icon edit" title="Editar configuración {@ids_pwdcfg}" onclick="editPwdConfig('{@ids_pwdcfg_id}')"></span>
      </td>
      
      <td>
        &#160;<xsl:value-of select="@ids_pwdcfg" />
      </td>
