  ...
  <xsl:template match="/">
    ...
    <head>
      ...
      <script type="text/javascript">
        ...
        function editEmailConfig(mail_server_id)
        {
            parent.editEmailConfig(mail_server_id);
        }
        ]]>
      </script>
    </head>
      ...
      <xsl:choose>
        ...
        <xsl:when test="count(xml/rs:data/z:row) = 0">
          <div class="sin-datos">
            ...
            <span class="mensaje">Actualmente no hay configuraciones de Email.</span>
          </div>
        </xsl:when>
          ...
          <table class="tb1 layout_fixed" id="tbCabecera">
            <tr class="tbLabel">
              ...
              <td style="text-align: center" title="Configuración de Email">
                <script>campos_head.agregar('Config Email', 'true', 'mail_cfgs_id');</script>
              </td>
  ...
  <xsl:template match="z:row" mode="row1">
    ...
    <tr>
      ...
      <td style="width: 30px; text-align: center;">
        <span class="icon edit" title="Editar configuración {@mail_cfgs}" onclick="editEmailConfig('{@mail_cfgs_id}')"></span>
      </td>
      
      <td>
        &#160;<xsl:value-of select="@mail_cfgs" />
      </td>
